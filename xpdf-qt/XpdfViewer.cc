...
static const char *aboutHTML =
    ...
    "<a "
    "href=\"http://www.glyphandcog.com/XpdfWidgetQt.html\">www.glyphandcog.com/XpdfWidgetQt.html</"
    "a><br>"
    "<br>" xpdfCopyright ".<br>"
    ...
    "<ul>"
    "FreeType is copyright 2006-2019 David Turner, Robert Wilhelm, and Werner Lemberg.  FreeType "
    "is used here under the terms of the FreeType Project License."
    "<li>The Qt Toolkit is Copyright 2015 The Qt Company Ltd.  Qt is used here under the terms of "
    "the LGPL v2.1."
    "</ul>";
...
static int zoomComboBoxVals[nZoomComboBoxVals] = {25,  50,  75,  100, 110, 125, 150,
                                                  175, 200, 300, 400, 600, 800};
...
public:
    XpdfErrorEvent(int eventType, QString msgA) : QEvent((Type)eventType), msg(msgA) {}
    QString getMessage() {
        return msg;
    }
...
public:
    XpdfTabInfo(QListWidgetItem *listItemA, XpdfWidget *pdfA, QTreeView *outlineTreeA, QTreeView *layerTreeA, QTableWidget *attachmentListA)
        : listItem(listItemA), pdf(pdfA), outlineTree(outlineTreeA), layerTree(layerTreeA),
...
XpdfViewer *
XpdfViewer::create(XpdfApp *app, QString fileName, int page, QString destName, QString password, GBool fullScreen) {
    XpdfViewer *viewer;
...
GBool XpdfViewer::open(QString fileName, int page, QString destName, QString password) {
...
GBool XpdfViewer::openInNewTab(QString fileName, int page, QString destName, QString password, GBool switchToTab) {
    GBool ok;
    int oldTabIndex;
...
void XpdfViewer::cmdOpenFileAtPageIn(GString *args[], int nArgs, QInputEvent *event) {
    if(!args[2]->cmp("win")) {
        ...
        openInNewTab(args[0]->getCString(), atoi(args[1]->getCString()), "", "", gTrue);
    } else {
        open(args[0]->getCString(), atoi(args[1]->getCString()), "", "");
    }
}
                    ...
                    if(!gotMouse) {
                        if(event->type() == QEvent::MouseButtonPress || event->type() == QEvent::MouseButtonRelease ||
                           ...
                           event->type() == QEvent::MouseMove || event->type() == QEvent::Wheel) {
                            currentTab->pdf->convertWindowToPDFCoords(mouseX(event), mouseY(event),
                                                                      &mPage, &mX, &mY);
                        } else {
...
void XpdfViewer::cmdSetSelection(GString *args[], int nArgs, QInputEvent *event) {
    currentTab->pdf->setCurrentSelection(atoi(args[0]->getCString()), atof(args[1]->getCString()),
                                         atof(args[2]->getCString()), atof(args[3]->getCString()),
    ...
    if((newTab || newWindow) && currentTab->pdf->getLinkTarget(pg, x, y, targetFileName, targetPage, targetDest)) {
        ...
        if(!currentTab->pdf->gotoLinkAt(pg, x, y)) {
            QMessageBox::warning(NULL, "Xpdf Error",
                                 "Couldn't follow link: '" + currentTab->pdf->getLinkInfo(pg, x, y) + "'");
        }
    }
}
...
void XpdfViewer::mouseRelease(QMouseEvent *e) {
    ...
    if(abs(mouseX(e) - lastMousePressX) + abs(mouseY(e) - lastMousePressY) <= QApplication::startDragDistance()) {
        keyCode = xpdfKeyCodeMouseClick1 + (btn - 1);
        if((cmds = globalParams->getKeyBinding(keyCode, getModifiers(e->modifiers()),
...
int XpdfViewer::getContext(Qt::KeyboardModifiers qtMods) {
    ...
    contin = mode == XpdfWidget::pdfDisplayContinuous || mode == XpdfWidget::pdfDisplaySideBySideContinuous ||
             mode == XpdfWidget::pdfDisplayHorizontalContinuous;
    context = ((windowState() & Qt::WindowFullScreen) ? xpdfKeyContextFullScreen : xpdfKeyContextWindow) |
              (contin ? xpdfKeyContextContinuous : xpdfKeyContextSinglePage) |
              (currentTab->pdf->mouseOverLink() ? xpdfKeyContextOverLink : xpdfKeyContextOffLink) |
...
void XpdfViewer::printStatus(int nextPage, int firstPage, int lastPage) {
    if(!printStatusDialog) {
        ...
        printStatusDialog->move(pos().x() + (width() - printStatusDialog->width()) / 2,
...
void XpdfViewer::pageNumberChanged() {
    ...
    if(viewPageLabelsMenuItem->isChecked() &&
       currentTab->pdf->getCore()->getDoc()->getCatalog()->hasPageLabels()) {
        ...
        pg = currentTab->pdf->getCore()->getDoc()->getCatalog()->getPageNumFromPageLabel(ts);
        delete ts;
        if(pg <= 0) {
            return;
        }
...
void XpdfViewer::createToolBar() {
    ...
    selectModeBtn = addToolBarButton(QIcon(":/selectModeLinear-button"),
                                     SLOT(selectModeButtonPressed()), "toggle selection mode");
    ...
    connect(zoomComboBox, SIGNAL(currentIndexChanged(const QString &)), this,
            SLOT(zoomIndexChanged(const QString &)));
    ...
    addToolBarSpacing(4);
    fitWidthBtn = addToolBarButton(QIcon(":/fitWidth-button"), SLOT(fitWidthButtonPressed()),
...
void XpdfViewer::createMainMenu() {
    ...
    QAction *action;
    action = displayModeSubmenu->addAction("Single page", this, SLOT(singlePageModeMenuAction()));
    action->setCheckable(true);
    displayModeGroup->addAction(action);
    action = displayModeSubmenu->addAction("Continuous", this, SLOT(continuousModeMenuAction()));
    action->setCheckable(true);
    displayModeGroup->addAction(action);
    action = displayModeSubmenu->addAction("Side-by-side single", this,
                                           SLOT(sideBySideSingleModeMenuAction()));
    action->setCheckable(true);
    displayModeGroup->addAction(action);
    action = displayModeSubmenu->addAction("Side-by-side continuous", this,
                                           SLOT(sideBySideContinuousModeMenuAction()));
    action->setCheckable(true);
    displayModeGroup->addAction(action);
    action = displayModeSubmenu->addAction("Horizontal continuous", this,
                                           SLOT(horizontalContinuousModeMenuAction()));
    action->setCheckable(true);
    displayModeGroup->addAction(action);
...
QWidget *XpdfViewer::createTabPane() {
    ...
    connect(tabList, SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)), this,
            SLOT(switchTab(QListWidgetItem *, QListWidgetItem *)));
...
QWidget *XpdfViewer::createInfoPane() {
    ...
    connect(infoComboBox, SIGNAL(activated(int)), this, SLOT(infoComboBoxChanged(int)));
...
void XpdfViewer::addTab() {
    ...
    connect(outlineTree, SIGNAL(clicked(const QModelIndex &)), this,
            SLOT(outlineItemClicked(const QModelIndex &)));
...
void XpdfViewer::updatePageNumberOrLabel(int pg) {
    ...
    if(viewPageLabelsMenuItem->isChecked() &&
       (ts = currentTab->pdf->getCore()->getDoc()->getCatalog()->getPageLabel(pg))) {
        u = ts->getUnicode();
...
void XpdfViewer::updateOutline(int pg) {
    ...
    idx = ((OutlineModel *)currentTab->outlineTree->model())->findPageIndex(pg, currentTab->outlineTree);
    if(idx.isValid()) {
...
} saveImageFormats[nSaveImageFormats] = {{"JPEG", "JPEG files (*.jpg)", "JPEG"},

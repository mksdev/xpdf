...
XpdfApp::XpdfApp(int &argc, char **argv) : QApplication(argc, argv) {
    XpdfViewer *viewer;
    QLocalSocket *sock;
    QString sockName;
    ...
    if(reverseVideo) {
        paperColor = QColor(255 - paperColor.red(), 255 - paperColor.green(), 255 - paperColor.blue());
    }
    if(matteColorArg[0]) {
    ...
    if(argc >= 2) {
        ...
        while(i < argc) {
            ...
            if(viewers->getLength() > 0) {
                ok = ((XpdfViewer *)viewers->get(0))->openInNewTab(qFileName, pg, dest, passwordArg, gFalse);
            } else {
...
XpdfViewer *XpdfApp::newWindow(GBool fullScreen, const char *remoteServerName) {
...
GBool XpdfApp::openInNewWindow(QString fileName, int page, QString dest, QString password, GBool fullScreen, const char *remoteServerName) {
    XpdfViewer *viewer;

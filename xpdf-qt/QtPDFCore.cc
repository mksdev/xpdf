...
QtPDFCore::QtPDFCore(QWidget *viewportA,
                     QScrollBar *hScrollBarA,
                     QScrollBar *vScrollBarA,
                     SplashColorPtr paperColor,
                     SplashColorPtr matteColor,
                     GBool reverseVideo)
...
void QtPDFCore::doLinkCbk(LinkAction *action) {
    ...
    switch(action->getKind()) {
        ...
        case actionLaunch:
            ...
            s = cmd->getCString();
            if(strcmp(s + cmd->getLength() - 4, ".pdf") && strcmp(s + cmd->getLength() - 4, ".PDF") &&
...
void QtPDFCore::copySelection(GBool toClipboard) {
    ...
    if(hasSelection()) {
        QApplication::clipboard()->setText(getSelectedTextQString(),
                                           toClipboard ? QClipboard::Clipboard : QClipboard::Selection);
    }
}
...
GBool QtPDFCore::doAction(LinkAction *action) {
    ...
    switch(kind = action->getKind()) {
        ...
        case actionLaunch:
            ...
            if(fileName->getLength() >= 4 && (!strcmp(s + fileName->getLength() - 4, ".pdf") ||
                ...
                if(globalParams->getLaunchCommand()) {
                    ...
                    msg->append(cmd);
                    if(QMessageBox::question(viewport, "PDF Launch Link", msg->getCString(),
                    ...
                    if(obj1.dictLookup("F", &obj2)) {
                        if((fileName = LinkAction::getFileSpecName(&obj2))) {
                            if(!isAbsolutePath(fileName->getCString())) {
                                fileName2 = appendToPath(grabPath(doc->getFileName()->getCString()),
                                                         fileName->getCString());
                                delete fileName;
                                fileName = fileName2;
                            }
...
void QtPDFCore::paintEvent(int x, int y, int w, int h) {
    ...
    if(scaleFactor == 1) {
        ...
        painter.drawImage(QRectF(x, y, w, h), image,
                          QRectF(x * scaleFactor, y * scaleFactor, w * scaleFactor, h * scaleFactor));
    }
    if(paintDoneCbk) {
...
void QtPDFCore::updateScrollbars() {
    ...
    if(state->getZoom() >= 0 && vScrollBarVisible && hScrollBarVisible &&
       horizMax <= vScrollBar->width() && vertMax <= hScrollBar->height()) {
        horizMax = 0;
        vertMax = 0;
    }
    ...
    if(state->getZoom() == zoomWidth ||
       (state->getZoom() == zoomPage && (state->getDisplayMode() == displayContinuous ||
        ...
        // the horizontal scrollbar.
              ...
              (state->getZoom() == zoomPage && state->getDisplayMode() == displayHorizontalContinuous)) {
        if(horizMax == 0) {
            horizMax = 1;
        }
    }
...
GString *QtPDFCore::getPassword() {
    ...
    s = QInputDialog::getText(viewport, "PDF Password", "This document requires a password",

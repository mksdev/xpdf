...
void XpdfWidget::setup(const QColor &paperColor, const QColor &matteColor, bool reverseVideo) {
    ...
    try {
        core = new QtPDFCore(viewport(), horizontalScrollBar(), verticalScrollBar(), paperColor2,
                             matteColor2, (GBool)reverseVideo);
...
void XpdfWidget::init(const QString &configFileName) {
    ...
    if(!globalParams) {
        try {
            ...
            delete dir;
#else
            globalParams = new GlobalParams((char *)configFileName.toLocal8Bit().constData());
            globalParams->setErrQuiet(gTrue);
        ...
        switch(action->getKind()) {
            ...
            case actionGoToR:
                ...
                if(isAbsolutePath(s)) {
                    ...
                    path = appendToPath(grabPath(core->getDoc()->getFileName()->getCString()), s);
    ...
    try {
        ...
        if(!printerForDialog) {
            ...
            if((defaultPrinter = globalParams->getDefaultPrinter())) {
                printerForDialog->setPrinterName(QString::fromUtf8(defaultPrinter->getCString()));
...
QImage XpdfWidget::convertRegionToImage(int page, double x0, double y0, double x1, double y1, double dpi) {
    PDFDoc *doc;
    PDFRectangle *box;
    SplashColor paperColor;
    SplashOutputDev *out;
    ...
    try {
        ...
        out = new SplashOutputDev(splashModeRGB8, 4, gFalse, paperColor);
        out->startDoc(doc->getXRef());
        doc->displayPageSlice(out, page, dpi, dpi, core->getRotate(), gFalse, gTrue, gFalse, sliceX,
                              sliceY, sliceW, sliceH);
        bitmap = out->getBitmap();
        img = new QImage((const uchar *)bitmap->getDataPtr(), bitmap->getWidth(),
                         bitmap->getHeight(), QImage::Format_RGB888);
        // force a copy
...
bool XpdfWidget::find(const QString &text, int flags) {
    ...
    try {
        ...
        ret = (bool)core->findU(u, len, (flags & findCaseSensitive) ? gTrue : gFalse,
                                (flags & findNext) ? gTrue : gFalse, (flags & findBackward) ? gTrue : gFalse,
...
XpdfLayerOrderHandle XpdfWidget::getLayerOrderRoot() const {
    try {
        ...
        return (XpdfLayerOrderHandle)core->getDoc()->getOptionalContent()->getDisplayRoot();
    } catch(GMemException e) {
        return NULL;
    }
}
...
void XpdfWidget::paintEvent(QPaintEvent *eventA) {
    core->paintEvent(eventA->rect().left(), eventA->rect().top(), eventA->rect().width(),
                     eventA->rect().height());
}

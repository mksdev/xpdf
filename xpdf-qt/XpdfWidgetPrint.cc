...
XpdfWidget::ErrorCode printPDF(PDFDoc *doc, QPrinter *prt, int hDPI, int vDPI, XpdfWidget *widget) {
  ...
  if (colorMode == QPrinter::GrayScale) {
    ...
    if(pageOrientation == QPageLayout::Landscape) {
        ...
        paperRect2 = CGRectMake(paperRect.top, paperRect.left, paperRect.bottom - paperRect.top,
                                paperRect.right - paperRect.left);
    } else {
        PMSetOrientation(pageFormat, kPMPortrait, kPMUnlocked);
        paperRect2 = CGRectMake(paperRect.left, paperRect.top, paperRect.right - paperRect.left,
    ...
    if(psFileName) {
        ...
        switch(paperSource) {
            case QPrinter::LargeCapacity:
                paperSourceStr = "LargeCapacity";
                break;
            case QPrinter::Lower:
                paperSourceStr = "Lower";
                break;
            default:
                paperSourceStr = NULL;
                break;
        ...
        if(!cupsPrintFile(printerName->getCString(), psFileName->getCString(),
                          doc->getFileName() ? doc->getFileName()->getCString() : "Xpdf printing",
                          nOptions, options)) {

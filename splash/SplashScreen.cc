...
void SplashScreen::buildDispersedMatrix(int i, int j, int val, int delta, int offset) {
    if(delta == 0) {
        ...
        mat[(i << log2Size) + j] = (Guchar)(1 + (254 * (val - 1)) / (size * size - 1));
    } else {
...
void SplashScreen::buildSCDMatrix(int r) {
    ...
    for(i = 0; i < size * size; ++i) {
        j = i + (int)((double)(size * size - i) * (double)rand() / ((double)RAND_MAX + 1.0));
        x = pts[i].x;
        y = pts[i].y;
        pts[i].x = pts[j].x;
        pts[i].y = pts[j].y;
    ...
    for(i = 0; i < dotsLen; ++i) {
        ...
        for(j = 0; j < n; ++j) {
            ...
            mat[(pts[j].y << log2Size) + pts[j].x] = (Guchar)(255 - (254 * j) / (n - 1));
        }
    }

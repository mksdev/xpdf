...
inline void
SplashXPath::transform(SplashCoord *matrix, SplashCoord xi, SplashCoord yi, SplashCoord *xo, SplashCoord *yo) {
    //                          [ m[0] m[1] 0 ]
...
SplashXPath::SplashXPath(SplashPath *path,
                         SplashCoord *matrix,
    ...
    if(path->hints) {
        adjusted = strokeAdjust(pts, path->hints, path->hintsLength, strokeAdjMode);
    } else {
        adjusted = gFalse;
    }
        ...
        if(path->flags[i] & splashPathFirst) {
            ...
            if(path->flags[i] & splashPathCurve) {
                ...
                addCurve(x0, y0, x1, y1, x2, y2, x3, y3, flatness,
                         (path->flags[i - 1] & splashPathFirst), (path->flags[i + 2] & splashPathLast),
                         !closeSubpaths && (path->flags[i - 1] & splashPathFirst) &&
                             !(path->flags[i - 1] & splashPathClosed),
                         !closeSubpaths && (path->flags[i + 2] & splashPathLast) &&
            ...
            if(path->flags[i - 1] & splashPathLast) {
                if(closeSubpaths &&
                   (pts[i - 1].x != pts[curSubpath].x || pts[i - 1].y != pts[curSubpath].y)) {
...
#endif
        if(segs[0].y0 == segs[0].y1 && segs[1].x0 == segs[1].x1 && segs[2].x0 == segs[2].x1 &&
           segs[3].y0 == segs[3].y1) {
            isRect = gTrue;
            rectX0 = segs[1].x0;
            rectX1 = segs[2].x0;
            rectY0 = segs[0].y0;
            rectY1 = segs[3].y0;
        } else if(segs[0].x0 == segs[0].x1 && segs[1].y0 == segs[1].y1 &&
                  segs[2].x0 == segs[2].x1 && segs[3].y0 == segs[3].y1) {
            isRect = gTrue;
            rectX0 = segs[0].x0;
            rectX1 = segs[2].x0;
            rectY0 = segs[1].y0;
            rectY1 = segs[3].y0;
        } else if(segs[0].x0 == segs[0].x1 && segs[1].x0 == segs[1].x1 &&
                  segs[2].y0 == segs[2].y1 && segs[3].y0 == segs[3].y1) {
            isRect = gTrue;
            rectX0 = segs[0].x0;
            rectX1 = segs[1].x0;
            rectY0 = segs[2].y0;
...
GBool SplashXPath::strokeAdjust(SplashXPathPoint *pts, SplashPathHint *hints, int nHints, SplashStrokeAdjustMode strokeAdjMode) {
...
void SplashXPath::addCurve(SplashCoord x0,
                           SplashCoord y0,
                           SplashCoord x1,
                           SplashCoord y1,
                           SplashCoord x2,
...
static GBool
joinAngleIsFlat(SplashCoord x0, SplashCoord y0, SplashCoord x1, SplashCoord y1, SplashCoord x2, SplashCoord y2) {
...
// distance.
static GBool
pointCloseToSegment(SplashCoord x0, SplashCoord y0, SplashCoord x1, SplashCoord y1, SplashCoord x2, SplashCoord y2) {
...
void SplashXPath::mergeSegments(int first) {
    ...
    while(in < length) {
        ...
        for(i = in + 1; i < length; ++i) {
            ...
            if((horiz && segs[in].y0 != segs[in].y1) || (vert && segs[in].x0 != segs[in].x1)) {
                break;
            }
            ...
            if(!joinAngleIsFlat(segs[prev].x0, segs[prev].y0, segs[i].x0, segs[i].y0, segs[i].x1,
                                segs[i].y1)) {
                break;
            }
            ...
            for(j = in; j < i; ++j) {
                if(!pointCloseToSegment(segs[in].x0, segs[in].y0, segs[j].x1, segs[j].y1,
                                        segs[i].x1, segs[i].y1)) {
                    break;
                }
            }
...
void SplashXPath::finishSegments() {
    ...
    for(i = 0; i < length; ++i) {
        ...
        if(seg->y0 == seg->y1 || seg->x0 == seg->x1 ||
           !FixedPoint::divCheck(seg->x1 - seg->x0, seg->y1 - seg->y0, &seg->dxdy) ||
           !FixedPoint::divCheck(seg->y1 - seg->y0, seg->x1 - seg->x0, &seg->dydx)) {
            seg->dxdy = 0;
            seg->dydx = 0;
        }
#else

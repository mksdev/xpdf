...
SplashError SplashClip::clipToPath(SplashPath *path,
                                   SplashCoord *matrix,
    ...
    xPath = new SplashXPath(path, matrix, flatness, gTrue, enablePathSimplification, strokeAdjust);
...
SplashClipResult
SplashClip::testRect(int rectXMin, int rectYMin, int rectXMax, int rectYMax, SplashStrokeAdjustMode strokeAdjust) {
    ...
    if(strokeAdjust != splashStrokeAdjustOff && isSimple) {
        ...
        if(isSimple && (SplashCoord)rectXMin >= xMin && (SplashCoord)(rectXMax + 1) <= xMax &&

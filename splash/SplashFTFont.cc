...
SplashFTFont::SplashFTFont(SplashFTFontFile *fontFileA, SplashCoord *matA, SplashCoord *textMatA)
    ...
    x = (int)(mat[0] * (scale * (face->bbox.xMin / div)) + mat[2] * (scale * (face->bbox.yMin / div)));
    xMin = xMax = x;
    y = (int)(mat[1] * (scale * (face->bbox.xMin / div)) + mat[3] * (scale * (face->bbox.yMin / div)));
    yMin = yMax = y;
    x = (int)(mat[0] * (scale * (face->bbox.xMin / div)) + mat[2] * (scale * (face->bbox.yMax / div)));
    if(x < xMin) {
        xMin = x;
    } else if(x > xMax) {
        xMax = x;
    }
    y = (int)(mat[1] * (scale * (face->bbox.xMin / div)) + mat[3] * (scale * (face->bbox.yMax / div)));
    if(y < yMin) {
        yMin = y;
    } else if(y > yMax) {
        yMax = y;
    }
    x = (int)(mat[0] * (scale * (face->bbox.xMax / div)) + mat[2] * (scale * (face->bbox.yMin / div)));
    if(x < xMin) {
        xMin = x;
    } else if(x > xMax) {
        xMax = x;
    }
    y = (int)(mat[1] * (scale * (face->bbox.xMax / div)) + mat[3] * (scale * (face->bbox.yMin / div)));
    if(y < yMin) {
        yMin = y;
    } else if(y > yMax) {
        yMax = y;
    }
    x = (int)(mat[0] * (scale * (face->bbox.xMax / div)) + mat[2] * (scale * (face->bbox.yMax / div)));
    if(x < xMin) {
        xMin = x;
    } else if(x > xMax) {
        xMax = x;
    }
    y = (int)(mat[1] * (scale * (face->bbox.xMax / div)) + mat[3] * (scale * (face->bbox.yMax / div)));
    if(y < yMin) {
        yMin = y;
    } else if(y > yMax) {
        yMax = y;
    ...
    // transform the four corners of the font bounding box -- the min
    // and max values form the bounding box of the transformed font
    x = (int)((mat[0] * (SplashCoord)face->bbox.xMin + mat[2] * (SplashCoord)face->bbox.yMin) /
              (div * face->units_per_EM));
    xMin = xMax = x;
    y = (int)((mat[1] * (SplashCoord)face->bbox.xMin + mat[3] * (SplashCoord)face->bbox.yMin) /
              (div * face->units_per_EM));
    yMin = yMax = y;
    x = (int)((mat[0] * (SplashCoord)face->bbox.xMin + mat[2] * (SplashCoord)face->bbox.yMax) /
              (div * face->units_per_EM));
    if(x < xMin) {
        xMin = x;
    } else if(x > xMax) {
    ...
    y = (int)((mat[1] * (SplashCoord)face->bbox.xMin + mat[3] * (SplashCoord)face->bbox.yMax) /
              (div * face->units_per_EM));
    if(y < yMin) {
        yMin = y;
    } else if(y > yMax) {
    ...
    x = (int)((mat[0] * (SplashCoord)face->bbox.xMax + mat[2] * (SplashCoord)face->bbox.yMin) /
              (div * face->units_per_EM));
    if(x < xMin) {
        xMin = x;
    } else if(x > xMax) {
    ...
    y = (int)((mat[1] * (SplashCoord)face->bbox.xMax + mat[3] * (SplashCoord)face->bbox.yMin) /
              (div * face->units_per_EM));
    if(y < yMin) {
        yMin = y;
    } else if(y > yMax) {
    ...
    x = (int)((mat[0] * (SplashCoord)face->bbox.xMax + mat[2] * (SplashCoord)face->bbox.yMax) /
              (div * face->units_per_EM));
    if(x < xMin) {
        xMin = x;
    } else if(x > xMax) {
    ...
    y = (int)((mat[1] * (SplashCoord)face->bbox.xMax + mat[3] * (SplashCoord)face->bbox.yMax) /
              (div * face->units_per_EM));
    if(y < yMin) {
        yMin = y;
    } else if(y > yMax) {
...
static int glyphPathCubicTo(const FT_Vector *ctrl1, const FT_Vector *ctrl2, const FT_Vector *pt, void *path) {
    ...
    p->path->curveTo((SplashCoord)ctrl1->x * p->textScale / 64.0, (SplashCoord)ctrl1->y * p->textScale / 64.0,
                     (SplashCoord)ctrl2->x * p->textScale / 64.0, (SplashCoord)ctrl2->y * p->textScale / 64.0,

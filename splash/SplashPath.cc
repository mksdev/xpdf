...
SplashError
SplashPath::curveTo(SplashCoord x1, SplashCoord y1, SplashCoord x2, SplashCoord y2, SplashCoord x3, SplashCoord y3) {
    if(noCurrentPoint()) {
        return splashErrNoCurPt;
    }
    flags[length - 1] &= (Guchar)~splashPathLast;
...
SplashError SplashPath::close(GBool force) {
    ...
    if(force || curSubpath == length - 1 || pts[length - 1].x != pts[curSubpath].x ||
...
void SplashPath::addStrokeAdjustHint(int ctrl0, int ctrl1, int firstPt, int lastPt, GBool projectingCap) {

...
void SplashFont::initCache() {
    ...
    for(cacheSets = splashFontCacheMaxSets;
        cacheSets > 1 && glyphSize > splashFontCacheSize / (cacheSets * cacheAssoc); cacheSets >>= 1)
        ;
...
GBool SplashFont::getGlyph(int c, int xFrac, int yFrac, SplashGlyphBitmap *bitmap) {
    ...
    for(j = 0; j < cacheAssoc; ++j) {
        if((cacheTags[i + j].mru & 0x80000000) && cacheTags[i + j].c == c &&
           (int)cacheTags[i + j].xFrac == xFrac && (int)cacheTags[i + j].yFrac == yFrac) {

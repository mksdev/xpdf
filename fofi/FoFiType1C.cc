...
GBool Type1COp::isZero() {
    switch(kind) {
        ...
        case type1COpRational:
            return rat.num == 0;
        default:
            return gFalse; // shouldn't happen
    }
}
...
FoFiType1C::~FoFiType1C() {
    ...
    if(charset && charset != fofiType1CISOAdobeCharset && charset != fofiType1CExpertCharset &&
...
void FoFiType1C::convertToType1(char *psName, const char **newEncoding, GBool ascii, FoFiOutputFunc outputFunc, void *outputStream) {
    int psNameLen;
    Type1CEexecBuf eb;
    Type1CIndex subrIdx;
    Type1CIndexVal val;
    ...
    buf = GString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] readonly "
                          "def\n",
    ...
    buf = GString::format("/FontBBox [{0:.4g} {1:.4g} {2:.4g} {3:.4g}] readonly def\n", topDict.fontBBox[0],
                          topDict.fontBBox[1], topDict.fontBBox[2], topDict.fontBBox[3]);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
    ...
    if(privateDicts[0].nBlueValues) {
        ...
        for(i = 0; i < privateDicts[0].nBlueValues; ++i) {
            buf = GString::format("{0:s}{1:d}", i > 0 ? " " : "", privateDicts[0].blueValues[i]);
    ...
    if(privateDicts[0].nOtherBlues) {
        ...
        for(i = 0; i < privateDicts[0].nOtherBlues; ++i) {
            buf = GString::format("{0:s}{1:d}", i > 0 ? " " : "", privateDicts[0].otherBlues[i]);
            eexecWrite(&eb, buf->getCString());
            delete buf;
        }
        eexecWrite(&eb, "] def\n");
    ...
    if(privateDicts[0].nFamilyBlues) {
        ...
        for(i = 0; i < privateDicts[0].nFamilyBlues; ++i) {
            buf = GString::format("{0:s}{1:d}", i > 0 ? " " : "", privateDicts[0].familyBlues[i]);
            eexecWrite(&eb, buf->getCString());
            delete buf;
        }
        eexecWrite(&eb, "] def\n");
    ...
    if(privateDicts[0].nFamilyOtherBlues) {
        ...
        for(i = 0; i < privateDicts[0].nFamilyOtherBlues; ++i) {
            buf = GString::format("{0:s}{1:d}", i > 0 ? " " : "", privateDicts[0].familyOtherBlues[i]);
            eexecWrite(&eb, buf->getCString());
            delete buf;
        }
        eexecWrite(&eb, "] def\n");
    ...
    if(privateDicts[0].nStemSnapH) {
        ...
        if(i == privateDicts[0].nStemSnapH) {
            ...
            for(i = 0; i < privateDicts[0].nStemSnapH; ++i) {
                buf = GString::format("{0:s}{1:.4g}", i > 0 ? " " : "", privateDicts[0].stemSnapH[i]);
                eexecWrite(&eb, buf->getCString());
                delete buf;
            }
            eexecWrite(&eb, "] def\n");
    ...
    if(privateDicts[0].nStemSnapV) {
        ...
        if(i == privateDicts[0].nStemSnapV) {
            ...
            for(i = 0; i < privateDicts[0].nStemSnapV; ++i) {
                buf = GString::format("{0:s}{1:.4g}", i > 0 ? " " : "", privateDicts[0].stemSnapV[i]);
                eexecWrite(&eb, buf->getCString());
                delete buf;
            }
            eexecWrite(&eb, "] def\n");
...
void FoFiType1C::convertToCIDType0(char *psName, int *codeMap, int nCodes, FoFiOutputFunc outputFunc, void *outputStream) {
    int *cidMap;
    GString *charStrings;
    ...
    for(i = 0; i < nCIDs; ++i) {
        ...
        if((gid = cidMap[i]) >= 0) {
            ...
            if(ok) {
                ...
                cvtGlyph(val.pos, val.len, charStrings, &subrIdx,
                         &privateDicts[fdSelect ? fdSelect[gid] : 0], gTrue);
            }
        }
    }
    ...
    for(i = 0; i < nFDs; ++i) {
        ...
        if(privateDicts[i].hasFontMatrix) {
            buf = GString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] "
                                  "def\n",
        ...
        if(privateDicts[i].nBlueValues) {
            ...
            for(j = 0; j < privateDicts[i].nBlueValues; ++j) {
                buf = GString::format("{0:s}{1:d}", j > 0 ? " " : "", privateDicts[i].blueValues[j]);
                (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                delete buf;
            }
            (*outputFunc)(outputStream, "] def\n", 6);
        ...
        if(privateDicts[i].nOtherBlues) {
            ...
            for(j = 0; j < privateDicts[i].nOtherBlues; ++j) {
                buf = GString::format("{0:s}{1:d}", j > 0 ? " " : "", privateDicts[i].otherBlues[j]);
                (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                delete buf;
            }
            (*outputFunc)(outputStream, "] def\n", 6);
        ...
        if(privateDicts[i].nFamilyBlues) {
            ...
            for(j = 0; j < privateDicts[i].nFamilyBlues; ++j) {
                buf = GString::format("{0:s}{1:d}", j > 0 ? " " : "", privateDicts[i].familyBlues[j]);
                (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                delete buf;
            }
            (*outputFunc)(outputStream, "] def\n", 6);
        ...
        if(privateDicts[i].nFamilyOtherBlues) {
            ...
            for(j = 0; j < privateDicts[i].nFamilyOtherBlues; ++j) {
                buf = GString::format("{0:s}{1:d}", j > 0 ? " " : "", privateDicts[i].familyOtherBlues[j]);
                (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                delete buf;
            }
            (*outputFunc)(outputStream, "] def\n", 6);
        ...
        if(privateDicts[i].nStemSnapH) {
            ...
            if(j == privateDicts[i].nStemSnapH) {
                ...
                for(j = 0; j < privateDicts[i].nStemSnapH; ++j) {
                    buf = GString::format("{0:s}{1:.4g}", j > 0 ? " " : "", privateDicts[i].stemSnapH[j]);
                    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                    delete buf;
                }
                (*outputFunc)(outputStream, "] def\n", 6);
        ...
        if(privateDicts[i].nStemSnapV) {
            ...
            if(j == privateDicts[i].nStemSnapV) {
                ...
                for(j = 0; j < privateDicts[i].nStemSnapV; ++j) {
                    buf = GString::format("{0:s}{1:.4g}", j > 0 ? " " : "", privateDicts[i].stemSnapV[j]);
                    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                    delete buf;
                }
                (*outputFunc)(outputStream, "] def\n", 6);
...
void FoFiType1C::convertToType0(char *psName, int *codeMap, int nCodes, FoFiOutputFunc outputFunc, void *outputStream) {
    int *cidMap;
    Type1CIndex subrIdx;
    Type1CIndexVal val;
    int nCIDs;
    ...
    for(i = 0; i < nCIDs; i += 256) {
        ...
        if(privateDicts[fd].nBlueValues) {
            ...
            for(k = 0; k < privateDicts[fd].nBlueValues; ++k) {
                buf = GString::format("{0:s}{1:d}", k > 0 ? " " : "", privateDicts[fd].blueValues[k]);
                eexecWrite(&eb, buf->getCString());
                delete buf;
            }
            eexecWrite(&eb, "] def\n");
        ...
        if(privateDicts[fd].nOtherBlues) {
            ...
            for(k = 0; k < privateDicts[fd].nOtherBlues; ++k) {
                buf = GString::format("{0:s}{1:d}", k > 0 ? " " : "", privateDicts[fd].otherBlues[k]);
                eexecWrite(&eb, buf->getCString());
                delete buf;
            }
            eexecWrite(&eb, "] def\n");
        ...
        if(privateDicts[fd].nFamilyBlues) {
            ...
            for(k = 0; k < privateDicts[fd].nFamilyBlues; ++k) {
                buf = GString::format("{0:s}{1:d}", k > 0 ? " " : "", privateDicts[fd].familyBlues[k]);
                eexecWrite(&eb, buf->getCString());
                delete buf;
            }
            eexecWrite(&eb, "] def\n");
        ...
        if(privateDicts[fd].nFamilyOtherBlues) {
            ...
            for(k = 0; k < privateDicts[fd].nFamilyOtherBlues; ++k) {
                buf = GString::format("{0:s}{1:d}", k > 0 ? " " : "", privateDicts[fd].familyOtherBlues[k]);
                eexecWrite(&eb, buf->getCString());
                delete buf;
            }
            eexecWrite(&eb, "] def\n");
        ...
        if(privateDicts[fd].nStemSnapH) {
            ...
            if(k == privateDicts[fd].nStemSnapH) {
                ...
                for(k = 0; k < privateDicts[fd].nStemSnapH; ++k) {
                    buf = GString::format("{0:s}{1:.4g}", k > 0 ? " " : "", privateDicts[fd].stemSnapH[k]);
                    eexecWrite(&eb, buf->getCString());
                    delete buf;
                }
                eexecWrite(&eb, "] def\n");
        ...
        if(privateDicts[fd].nStemSnapV) {
            ...
            if(k == privateDicts[fd].nStemSnapV) {
                ...
                for(k = 0; k < privateDicts[fd].nStemSnapV; ++k) {
                    buf = GString::format("{0:s}{1:.4g}", k > 0 ? " " : "", privateDicts[fd].stemSnapV[k]);
                    eexecWrite(&eb, buf->getCString());
                    delete buf;
                }
                eexecWrite(&eb, "] def\n");
        ...
        for(j = 0; j < 256 && i + j < nCIDs; ++j) {
            if(cidMap[i + j] >= 0) {
                ...
                if(ok) {
                    ...
                    eexecCvtGlyph(&eb, buf->getCString(), val.pos, val.len, &subrIdx, &privateDicts[fd]);
                    delete buf;
                }
            }
        }
...
void FoFiType1C::eexecCvtGlyph(Type1CEexecBuf *eb,
    ...
    eexecWrite(eb, buf->getCString());
    delete buf;
    eexecWriteCharstring(eb, (Guchar *)charBuf->getCString(), charBuf->getLength());
    eexecWrite(eb, " ND\n");

    delete charBuf;
}

void FoFiType1C::cvtGlyph(int offset, int nBytes, GString *charBuf, Type1CIndex *subrIdx, Type1CPrivateDict *pDict, GBool top) {
    Type1CIndexVal val;
    Type1COp zero, tmp;
    GBool ok, dInt;
    double d, dx, dy;
            ...
            switch(ops[nOps].op) {
                ...
                case 0x0c24: // hflex1
                    if(nOps == 9) {
                        ...
                        cvtNum(ops[7], charBuf);
                        cvtNum(ops[8], charBuf);
                        if(ops[1].kind == type1COpInteger && ops[3].kind == type1COpInteger &&
                ...
                case 0x0c25: // flex1
                    if(nOps == 11) {
                        ...
                        if(fabs(dx) > fabs(dy)) {
                            cvtNum(ops[10], charBuf);
                            if(ops[1].kind == type1COpInteger && ops[3].kind == type1COpInteger &&
                               ops[5].kind == type1COpInteger && ops[7].kind == type1COpInteger &&
                            ...
                            if(ops[0].kind == type1COpInteger && ops[2].kind == type1COpInteger &&
                               ops[4].kind == type1COpInteger && ops[6].kind == type1COpInteger &&
...
void FoFiType1C::cvtGlyphWidth(GBool useOp, GString *charBuf, Type1CPrivateDict *pDict) {
    Type1COp zero, w;
    int i;
...
void FoFiType1C::eexecWriteCharstring(Type1CEexecBuf *eb, Guchar *s, int n) {
    Guchar x;
    int i;
...
void FoFiType1C::convertToOpenType(FoFiOutputFunc outputFunc,
    ...
    Guchar *hmtxTable;
    static const char *tableTag[9] = {"CFF ", "OS/2", "cmap", "head", "hhea",
                                      "hmtx", "maxp", "name", "post"};
    Guchar *tableData[9];
    int tableLength[9];
...
void FoFiType1C::readTopDict() {
    ...
    while(pos < topDictPtr.pos + topDictPtr.len) {
        ...
        if(ops[nOps - 1].kind == type1COpOperator) {
            ...
            switch(ops[nOps].op) {
                case 0x0000:
                    topDict.versionSID = ops[0].toInt();
                    break;
                case 0x0001:
                    topDict.noticeSID = ops[0].toInt();
                    break;
                case 0x0c00:
                    topDict.copyrightSID = ops[0].toInt();
                    break;
                case 0x0002:
                    topDict.fullNameSID = ops[0].toInt();
                    break;
                case 0x0003:
                    topDict.familyNameSID = ops[0].toInt();
                    break;
                case 0x0004:
                    topDict.weightSID = ops[0].toInt();
                    break;
                case 0x0c01:
                    topDict.isFixedPitch = ops[0].toInt();
                    break;
                case 0x0c02:
                    topDict.italicAngle = ops[0].toFloat();
                    break;
                case 0x0c03:
                    topDict.underlinePosition = ops[0].toFloat();
                    break;
                case 0x0c04:
                    topDict.underlineThickness = ops[0].toFloat();
                    break;
                case 0x0c05:
                    topDict.paintType = ops[0].toInt();
                    break;
                case 0x0c06:
                    topDict.charstringType = ops[0].toInt();
                    break;
                case 0x0c07:
                ...
                case 0x0005:
                    ...
                    break;
                case 0x0c08:
                    topDict.strokeWidth = ops[0].toFloat();
                    break;
                case 0x000f:
                    topDict.charsetOffset = ops[0].toInt();
                    break;
                case 0x0010:
                    topDict.encodingOffset = ops[0].toInt();
                    break;
                case 0x0011:
                    topDict.charStringsOffset = ops[0].toInt();
                    break;
                case 0x0012:
                ...
                case 0x0c1e:
                    ...
                    break;
                case 0x0c24:
                    topDict.fdArrayOffset = ops[0].toInt();
                    break;
                case 0x0c25:
                    topDict.fdSelectOffset = ops[0].toInt();
                    break;
            }
            nOps = 0;
        }
...
void FoFiType1C::getIndexVal(Type1CIndex *idx, int i, Type1CIndexVal *val, GBool *ok) {
    int pos0, pos1;

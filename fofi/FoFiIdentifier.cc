...
GBool MemReader::getU32LE(int pos, Guint *val) {
    ...
    *val = (buf[pos] & 0xff) + ((buf[pos + 1] & 0xff) << 8) + ((buf[pos + 2] & 0xff) << 16) +
           ((buf[pos + 3] & 0xff) << 24);
    return gTrue;
}
...
GBool FileReader::getU32BE(int pos, Guint *val) {
    ...
    *val = ((buf[pos - bufPos] & 0xff) << 24) + ((buf[pos - bufPos + 1] & 0xff) << 16) +
           ((buf[pos - bufPos + 2] & 0xff) << 8) + (buf[pos - bufPos + 3] & 0xff);
    return gTrue;
}
...
GBool FileReader::getU32LE(int pos, Guint *val) {
    ...
    *val = (buf[pos - bufPos] & 0xff) + ((buf[pos - bufPos + 1] & 0xff) << 8) +
           ((buf[pos - bufPos + 2] & 0xff) << 16) + ((buf[pos - bufPos + 3] & 0xff) << 24);
    return gTrue;
}
...
static FoFiIdentifierType identify(Reader *reader) {
    ...
    //----- TrueType
    if((reader->getByte(0) == 0x00 && reader->getByte(1) == 0x01 && reader->getByte(2) == 0x00 &&
...
static FoFiIdentifierType identifyOpenType(Reader *reader) {
    ...
    for(i = 0; i < nTables; ++i) {
        if(reader->cmp(12 + i * 16, "CFF ")) {
            if(reader->getU32BE(12 + i * 16 + 8, &offset) && offset < (Guint)INT_MAX) {
...
static FoFiIdentifierType identifyCFF(Reader *reader, int start) {
    ...
    if(n == 0) {
        ...
        if(!reader->getUVarBE(pos + 3 + n * offSize1, offSize1, &offset1) || offset1 > (Guint)INT_MAX) {
            return fofiIdUnknown;
        }
    ...
    if(!reader->getUVarBE(pos + 3, offSize1, &offset0) || offset0 > (Guint)INT_MAX ||
       !reader->getUVarBE(pos + 3 + offSize1, offSize1, &offset1) || offset1 > (Guint)INT_MAX ||
       offset0 > offset1) {
        return fofiIdUnknown;
    }
...
static GList *getTTCFontList(FILE *f) {
    ...
    for(i = 0; i < nFonts; ++i) {
        ...
        for(j = 0; j < nTables; ++j) {
            if(buf2[16 * j + 0] == 'n' && buf2[16 * j + 1] == 'a' && buf2[16 * j + 2] == 'm' &&
               buf2[16 * j + 3] == 'e') {
        ...
        if(nameTabOffset + 6 + 12 * nNames < 0 || nameTabOffset + 6 + 12 * nNames > fileLength ||
...
static GList *getDfontFontList(FILE *f) {
    ...
    if(resMapOffset < 0 || resMapOffset >= fileLength || resMapLength < 0 ||
       resMapOffset + resMapLength > fileLength || resMapOffset + resMapLength < 0) {
        goto err1;
    }
    ...
    for(i = 0; i < nTypes; ++i) {
        ...
        if(resMap[offset] == 0x73 && // 'sfnt'
           resMap[offset + 1] == 0x66 && resMap[offset + 2] == 0x6e && resMap[offset + 3] == 0x74) {

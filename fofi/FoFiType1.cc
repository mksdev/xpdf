...
void FoFiType1::writeEncoded(const char **newEncoding, FoFiOutputFunc outputFunc, void *outputStream) {
    ...
    for(line = (char *)file; line && line + 9 <= (char *)file + len && strncmp(line, "/Encoding", 9);
    ...
    if(line) {
        for(line2 = line, i = 0;
            i < 20 && line2 && line2 + 9 <= (char *)file + len && strncmp(line2, "/Encoding", 9);
...
void FoFiType1::parse() {
    ...
    for(i = 1, line = (char *)file; i <= 100 && line && (!name || !encoding || !gotMatrix); ++i) {
        ...
        if(!name && line + 9 <= (char *)file + len && !strncmp(line, "/FontName", 9)) {
            ...
            for(j = 0, line = getNextLine(line); j < 300 && line && (line1 = getNextLine(line));
                ++j, line = line1) {

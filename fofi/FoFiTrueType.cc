...
static T42Table t42Tables[nT42Tables] = {{"cvt ", gTrue},  {"fpgm", gTrue}, {"glyf", gTrue},
                                         {"head", gTrue},  {"hhea", gTrue}, {"hmtx", gTrue},
                                         {"loca", gTrue},  {"maxp", gTrue}, {"prep", gTrue},
                                         {"vhea", gFalse}, {"vmtx", gFalse}};
...
FoFiTrueType *FoFiTrueType::make(char *fileA, int lenA, int fontNum, GBool allowHeadlessCFF) {
    FoFiTrueType *ff;
...
FoFiTrueType *FoFiTrueType::load(char *fileName, int fontNum, GBool allowHeadlessCFF) {
    FoFiTrueType *ff;
    char *fileA;
    int lenA, n;
    GBool isDfontA;
    ...
    ff = new FoFiTrueType(fileA, lenA, gTrue, fontNum, isDfontA, allowHeadlessCFF);
    if(!ff->parsedOk) {
        delete ff;
        return NULL;
    }
...
FoFiTrueType::FoFiTrueType(char *fileA, int lenA, GBool freeFileDataA, int fontNum, GBool isDfontA, GBool allowHeadlessCFF)
...
int FoFiTrueType::mapCodeToGID(int i, int c) {
    ...
    switch(cmaps[i].fmt) {
        ...
        case 2:
            ...
            if(getU16BE(pos + 6, &ok) != 0 || getU16BE(pos + 518 + 0, &ok) != 0 ||
               getU16BE(pos + 518 + 2, &ok) != 256 || getU16BE(pos + 518 + 4, &ok) != 0) {
                return 0;
            }
...
static FoFiTrueTypeTrickyFont trickyFonts[] = {{0x000002e4, 0x05bcf058, // MingLiU 1995
                                                0x000087c4, 0x28233bf1, 0x000001e1, 0xa344a1ea},
                                               {0x000002e4, 0x05bcf058, // MingLiU 1996-
                                                0x000087c4, 0x28233bf1, 0x000001e1, 0xa344a1eb},
                                               {0x00000350, 0x12c3ebb2, // DFGothic-EB
                                                0x000087a7, 0xb680ee64, 0x00000758, 0xce939563},
                                               {0x00000350, 0x11e5ead4, // DFGyoSho-Lt
                                                0x0000bc85, 0xce5956e9, 0x00000045, 0x8272f416},
                                               {0x00000350, 0x1262eb4e, // DFHSGothic-W5
                                                0x00007940, 0xe86a5d64, 0x000005ff, 0x7850f729},
                                               {0x00000350, 0x122deb0a, // DFHSMincho-W3
                                                0x0000859b, 0x3d16328a, 0x000002cb, 0xa93fc33b},
                                               {0x00000350, 0x125feb26, // DFHSMincho-W7
                                                0x00007ee1, 0xa5acc982, 0x0000041f, 0x90999196},
                                               {0x00000350, 0x11e5ead4, // DFKaiShu
                                                0x00009063, 0x5a30ca3b, 0x0000007e, 0x13a42602},
                                               {0x00000350, 0x11e5ead4, // DFKaiShu2
                                                0x00008998, 0xa6e78c01, 0x0000007e, 0x13a42602},
                                               {0x00000008, 0xfffbfffc, // HuaTianKaiTi
                                                0x0000bea2, 0x9c9e48b8, 0x00000008, 0x70020112},
                                               {0x00000008, 0xfffbfffc, // HuaTianSongTi
                                                0x00017c39, 0x0a5a0483, 0x00000008, 0x70020112},
                                               {0x00000000, 0x00000000, // NEC fadpop7.ttf
                                                0x000000e5, 0x40c92555, 0x0000117c, 0xa39b58e3},
                                               {0x00000000, 0x00000000, // NEC fadrei5.ttf
                                                0x000000e5, 0x33c41652, 0x00000f6a, 0x26d6c52a},
                                               {0x00000000, 0x00000000, // NEC fangot7.ttf
                                                0x0000019d, 0x6db1651d, 0x00002492, 0x6c6e4b03},
                                               {0x00000000, 0x00000000, // NEC fangyo5.ttf
                                                0x000000e5, 0x40c92555, 0x0000117c, 0xde51fad0},
                                               {0x00000000, 0x00000000, // NEC fankyo5.ttf
                                                0x000000e5, 0x85e47664, 0x00001caa, 0xa6c62831},
                                               {0x00000000, 0x00000000, // NEC fanrgo5.ttf
                                                0x0000019d, 0x2d891cfd, 0x00001de8, 0xa0604633},
                                               {0x00000000, 0x00000000, // NEC fangot5.ttc
                                                0x000001cb, 0x40aa774c, 0x00001f9a, 0x9b5caa96},
                                               {0x00000000, 0x00000000, // NEC fanmin3.ttc
                                                0x00000141, 0x0d3de9cb, 0x00002280, 0xd4127766},
                                               {0x00000000, 0x00000000, // NEC FA-Gothic, 1996
                                                0x000001f0, 0x4a692698, 0x00001fca, 0x340d4346},
                                               {0x00000000, 0x00000000, // NEC FA-Minchou, 1996
                                                0x00000166, 0xcd34c604, 0x000022b0, 0x6cf31046},
                                               {0x00000000, 0x00000000, // NEC FA-RoundGothicB, 1996
                                                0x0000019d, 0x5da75315, 0x000022e0, 0x40745a5f},
                                               {0x00000000, 0x00000000, // NEC FA-RoundGothicM, 1996
                                                0x000001c2, 0xf055fc48, 0x00001e18, 0x3900ded3},
                                               {0x00000060, 0x00170003, // MINGLI, 1992
                                                0x000058aa, 0xdbb4306e, 0x00000035, 0xd643482a}};
...
GBool FoFiTrueType::checkForTrickyCJK() {
    ...
    for(i = 0; i < nTrickyFonts; ++i) {
        if(cvtLen == trickyFonts[i].cvtLen && cvtChecksum == trickyFonts[i].cvtChecksum &&
           fpgmLen == trickyFonts[i].fpgmLen && fpgmChecksum == trickyFonts[i].fpgmChecksum &&
           prepLen == trickyFonts[i].prepLen && prepChecksum == trickyFonts[i].prepChecksum) {
            return gTrue;
        }
    }
    return gFalse;
...
void FoFiTrueType::convertToType42(char *psName, char **encoding, int *codeToGID, FoFiOutputFunc outputFunc, void *outputStream) {
    GString *buf;
    int maxUsedGlyph;
    GBool ok;
...
void FoFiTrueType::convertToType1(char *psName, const char **newEncoding, GBool ascii, FoFiOutputFunc outputFunc, void *outputStream) {
    char *start;
    int length;
    FoFiType1C *ff;
            ...
            for(i = 0; i < nCIDs; i += 32768 - 16) {
                ...
                for(j = 0; j < 32768 - 16 && i + j < nCIDs; j += 16) {
                    ...
                    for(k = 0; k < 16 && i + j + k < nCIDs; ++k) {
                        cid = cidMap[i + j + k];
                        buf = GString::format("{0:02x}{1:02x}", (cid >> 8) & 0xff, cid & 0xff);
                        (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
                        delete buf;
                    }
                    (*outputFunc)(outputStream, "\n", 1);
...
void FoFiTrueType::convertToCIDType0(char *psName, int *cidMap, int nCIDs, FoFiOutputFunc outputFunc, void *outputStream) {
    char *start;
    int length;
    FoFiType1C *ff;
...
void FoFiTrueType::convertToType0(char *psName, int *cidMap, int nCIDs, FoFiOutputFunc outputFunc, void *outputStream) {
    char *start;
    int length;
    FoFiType1C *ff;
...
GBool FoFiTrueType::writeTTF(FoFiOutputFunc outputFunc,
    ...
    static char os2Tab[86] = {
        ...
        0,    0,                               // sFamilyClass
        0,    0,          0, 0, 0,             // panose
        0,    0,          0, 0, 0, 0, 0, 0, 0, // ulUnicodeRange1
    ...
    for(i = 0; i <= nGlyphs; ++i) {
        ...
        if(locaTable[i].origOffset < 0 || locaTable[i].origOffset > glyfLen) {
        ...
        // start)
        if(i > 0 && locaTable[i].origOffset - locaTable[i - 1].origOffset > 0 &&
    ...
    for(i = 0; i < nTables; ++i) {
        ...
        } else if(!(tables[i].tag & 0xe0000000) || !(tables[i].tag & 0x00e00000) ||
                  !(tables[i].tag & 0x0000e000) || !(tables[i].tag & 0x000000e0)) {
    ...
    if(!missingCmap && !missingName && !missingPost && !missingOS2 && !unsortedLoca && !emptyCmap &&
       !badCmapLen && !abbrevHMTX && nZeroLengthTables == 0 && nBogusTables == 0 && !name &&
       !codeToGID && !replacementCmapTable && !isDfont && !isTTC) {
...
#else
        qsort(locaTable, nGlyphs + 1, sizeof(TrueTypeLoca), &cmpTrueTypeLocaOffset);
#endif
        for(i = 0; i < nGlyphs; ++i) {
...
#else
        qsort(locaTable, nGlyphs + 1, sizeof(TrueTypeLoca), &cmpTrueTypeLocaIdx);
#endif
    ...
    nNewTables = nTables - nZeroLengthTables - nBogusTables + (missingCmap ? 1 : 0) +
                 (missingName ? 1 : 0) + (missingPost ? 1 : 0) + (missingOS2 ? 1 : 0);
    newTables = (TrueTypeTable *)gmallocn(nNewTables, sizeof(TrueTypeTable));
    j = 0;
    for(i = 0; i < nTables; ++i) {
        if(tables[i].len > 0 && (tables[i].tag & 0xe0000000) && (tables[i].tag & 0x00e00000) &&
           (tables[i].tag & 0x0000e000) && (tables[i].tag & 0x000000e0)) {
            newTables[j] = tables[i];
            newTables[j].origOffset = tables[i].offset;
            if(checkRegion(tables[i].offset, tables[i].len)) {
                newTables[j].checksum = computeTableChecksum(file + tables[i].offset, tables[i].len);
            ...
            if(newTables[j].tag == cmapTag && replacementCmapTable) {
                ...
                newTables[j].checksum = computeTableChecksum(replacementCmapTable, replacementCmapTableLen);
            } else if(newTables[j].tag == cmapTag && codeToGID) {
                newTables[j].len = newCmapLen;
                newTables[j].checksum = computeTableChecksum((Guchar *)newCmapTab, newCmapLen);
            } else if(newTables[j].tag == cmapTag && emptyCmap) {
                newTables[j].checksum = computeTableChecksum((Guchar *)cmapTab, sizeof(cmapTab));
                ...
                newTables[j].checksum = computeTableChecksum((Guchar *)newNameTab, newNameLen);
            } else if(newTables[j].tag == hheaTag && abbrevHMTX) {
                newTables[j].len = newHHEALen;
                newTables[j].checksum = computeTableChecksum((Guchar *)newHHEATab, newHHEALen);
            } else if(newTables[j].tag == hmtxTag && abbrevHMTX) {
                newTables[j].len = newHMTXLen;
                newTables[j].checksum = computeTableChecksum((Guchar *)newHMTXTab, newHMTXLen);
            }
            ++j;
        }
    }
    ...
    if(missingName) {
        ...
        if(name) {
            ...
            newTables[j].checksum = computeTableChecksum((Guchar *)nameTab, sizeof(nameTab));
    ...
    if(missingPost) {
        ...
        newTables[j].checksum = computeTableChecksum((Guchar *)postTab, sizeof(postTab));
        newTables[j].len = sizeof(postTab);
        ++j;
    }
    if(missingOS2) {
        newTables[j].tag = os2Tag;
        newTables[j].checksum = computeTableChecksum((Guchar *)os2Tab, sizeof(os2Tab));
...
#else
    qsort(newTables, nNewTables, sizeof(TrueTypeTable), &cmpTrueTypeTableTag);
#endif
    ...
    fileChecksum = computeTableChecksum((Guchar *)tableDir, 12 + nNewTables * 16);
    for(i = 0; i < nNewTables; ++i) {
    ...
    for(i = 0; i < nNewTables; ++i) {
        if(newTables[i].tag == headTag) {
            if(checkRegion(newTables[i].origOffset, newTables[i].len)) {
                ...
                (*outputFunc)(outputStream, (char *)file + newTables[i].origOffset + 12,
                              newTables[i].len - 12);
            } else {
            ...
            (*outputFunc)(outputStream, (char *)replacementCmapTable, newTables[i].len);
            ...
            if(checkRegion(newTables[i].origOffset, newTables[i].len)) {
                (*outputFunc)(outputStream, (char *)file + newTables[i].origOffset, newTables[i].len);
            } else {
                for(j = 0; j < newTables[i].len; ++j) {
                    (*outputFunc)(outputStream, "\0", 1);
                }
...
void FoFiTrueType::cvtCharStrings(char **encoding, int *codeToGID, FoFiOutputFunc outputFunc, void *outputStream) {
    char *name;
    GString *buf;
    char buf2[16];
    int i, k;
...
void FoFiTrueType::cvtSfnts(FoFiOutputFunc outputFunc, void *outputStream, GString *name, GBool needVerticalMetrics, int *maxUsedGlyph) {
    Guchar headData[54];
    TrueTypeLoca *locaTable;
    Guchar *locaData;
    ...
    for(i = 0; i <= nGlyphs; ++i) {
        ...
        if(locaTable[i].origOffset < 0 || locaTable[i].origOffset > glyfTableLen) {
    ...
    *maxUsedGlyph = -1;
    ...
    for(i = 0; i < nT42Tables; ++i) {
            ...
            for(j = 0; j < nGlyphs; ++j) {
                if(locaTable[j].len > 12) {
                    ...
                    if(checkRegion(glyfPos + locaTable[j].origOffset, locaTable[j].len)) {
                        checksum += computeTableChecksum(file + glyfPos + locaTable[j].origOffset,
                                                         locaTable[j].len);
                    }
                }
            }
        ...
        if(length >= 0) {
            newTables[k].tag = ((t42Tables[i].tag[0] & 0xff) << 24) | ((t42Tables[i].tag[1] & 0xff) << 16) |
                               ((t42Tables[i].tag[2] & 0xff) << 8) | (t42Tables[i].tag[3] & 0xff);
...
void FoFiTrueType::dumpString(Guchar *s, int length, FoFiOutputFunc outputFunc, void *outputStream) {
    GString *buf;
    int pad, i, j;
...
void FoFiTrueType::parse(int fontNum, GBool allowHeadlessCFF) {
    ...
    for(i = 0; i < nTables; ++i) {
        ...
        if(tables[j].offset >= 0 && tables[j].len >= 0 &&
    ...
    if(seekTable("hhea") < 0 || seekTable("maxp") < 0 || seekTable("hmtx") < 0 ||
       (!openTypeCFF && seekTable("loca") < 0) || (!openTypeCFF && seekTable("glyf") < 0) ||
    ...
    if(postFmt == 0x00010000) {
        ...
        for(i = 0; i < n && i < nGlyphs; ++i) {
            ...
            if(j < 258) {
                ...
                j -= 258;
                if(j != stringIdx) {
                    for(stringIdx = 0, stringPos = tablePos + 34 + 2 * n; stringIdx < j;

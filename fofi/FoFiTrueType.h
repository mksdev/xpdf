...
public:
    ...
    // functions.
    static FoFiTrueType *make(char *fileA, int lenA, int fontNum, GBool allowHeadlessCFF = gFalse);

    // Create a FoFiTrueType object from a file on disk.  If
    // <allowHeadlessCFF> is true, OpenType CFF fonts without the 'head'
    // table are permitted -- this is useful when calling the convert*
    // functions.
    static FoFiTrueType *load(char *fileName, int fontNum, GBool allowHeadlessCFF = gFalse);
    ...
    void convertToType42(char *psName, char **encoding, int *codeToGID, FoFiOutputFunc outputFunc, void *outputStream);
    ...
    // useful for OpenType CFF fonts.)
    void convertToType1(char *psName, const char **newEncoding, GBool ascii, FoFiOutputFunc outputFunc, void *outputStream);
    ...
    void convertToCIDType2(char *psName, int *cidMap, int nCIDs, GBool needVerticalMetrics, FoFiOutputFunc outputFunc, void *outputStream);

    // Convert to a Type 0 CIDFont, suitable for embedding in a
    // PostScript file.  <psName> will be used as the PostScript font
    // name.  (Only useful for OpenType CFF fonts.)
    void convertToCIDType0(char *psName, int *cidMap, int nCIDs, FoFiOutputFunc outputFunc, void *outputStream);
    ...
    void convertToType0(char *psName, int *cidMap, int nCIDs, GBool needVerticalMetrics, FoFiOutputFunc outputFunc, void *outputStream);

    // Convert to a Type 0 (but non-CID) composite font, suitable for
    // embedding in a PostScript file.  <psName> will be used as the
    // PostScript font name.  (Only useful for OpenType CFF fonts.)
    void convertToType0(char *psName, int *cidMap, int nCIDs, FoFiOutputFunc outputFunc, void *outputStream);
    ...
    GBool writeTTF(FoFiOutputFunc outputFunc,
                   void *outputStream,
                   char *name = NULL,
                   int *codeToGID = NULL,
...
private:
    FoFiTrueType(char *fileA, int lenA, GBool freeFileDataA, int fontNum, GBool isDfont, GBool allowHeadlessCFF);
    ...
    void cvtSfnts(FoFiOutputFunc outputFunc, void *outputStream, GString *name, GBool needVerticalMetrics, int *maxUsedGlyph);

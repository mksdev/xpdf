...
void PreScanOutputDev::stroke(GfxState *state) {
    ...
    check(state, state->getStrokeColorSpace(), state->getStrokeColor(), state->getStrokeOpacity(),
          state->getBlendMode());
...
void PreScanOutputDev::fill(GfxState *state) {
    check(state, state->getFillColorSpace(), state->getFillColor(), state->getFillOpacity(),
          state->getBlendMode());
}
...
void PreScanOutputDev::tilingPatternFill(GfxState *state,
                                         Gfx *gfx,
                                         Object *strRef,
                                         int paintType,
                                         int tilingType,
...
GBool PreScanOutputDev::functionShadedFill(GfxState *state, GfxFunctionShading *shading) {
...
GBool PreScanOutputDev::axialShadedFill(GfxState *state, GfxAxialShading *shading) {
    if(shading->getColorSpace()->getMode() != csDeviceGray && shading->getColorSpace()->getMode() != csCalGray) {
        gray = gFalse;
    }
    mono = gFalse;
...
GBool PreScanOutputDev::radialShadedFill(GfxState *state, GfxRadialShading *shading) {
    if(shading->getColorSpace()->getMode() != csDeviceGray && shading->getColorSpace()->getMode() != csCalGray) {
        gray = gFalse;
    }
    mono = gFalse;
...
void PreScanOutputDev::beginStringOp(GfxState *state) {
    ...
    simpleTTF = fabs(m11 + m22) < 0.01 && m11 > 0 && fabs(m12) < 0.01 && fabs(m21) < 0.01 &&
                fabs(state->getHorizScaling() - 1) < 0.001 &&
                (font->getType() == fontTrueType || font->getType() == fontTrueTypeOT);
    if(simpleTTF) {
...
GBool PreScanOutputDev::beginType3Char(GfxState *state, double x, double y, double dx, double dy, CharCode code, Unicode *u, int uLen) {
...
void PreScanOutputDev::drawImageMask(GfxState *state,
                                     Object *ref,
                                     Stream *str,
                                     int width,
                                     int height,
...
void PreScanOutputDev::drawImage(GfxState *state,
                                 Object *ref,
                                 Stream *str,
                                 int width,
                                 int height,
...
void PreScanOutputDev::drawMaskedImage(GfxState *state,
                                       Object *ref,
                                       Stream *str,
                                       int width,
                                       int height,
...
void PreScanOutputDev::drawSoftMaskedImage(GfxState *state,
                                           Object *ref,
                                           Stream *str,
                                           int width,
                                           int height,
...
void PreScanOutputDev::beginTransparencyGroup(GfxState *state,
                                              double *bbox,
    ...
    if(colorSpace->getMode() == csPattern) {
        ...
        if(rgb.r != rgb.g || rgb.g != rgb.b || rgb.b != rgb.r) {
            ...
            gray = gFalse;
                    ...
                    (rgb.r == gfxColorComp1 && rgb.g == gfxColorComp1 && rgb.b == gfxColorComp1))) {
            mono = gFalse;
        }
    }

...
enum SelectMode { selectModeBlock, selectModeLinear };
...
class PDFCore {
public:
    PDFCore(SplashColorMode colorMode, int bitmapRowPad, GBool reverseVideo, SplashColorPtr paperColor);
    virtual ~PDFCore();
    ...
    // Load a new file.  Returns pdfOk or error code.
    virtual int
    loadFile(wchar_t *fileName, int fileNameLen, GString *ownerPassword = NULL, GString *userPassword = NULL);
#endif
    ...
    virtual void displayPage(int page, GBool scrollToTop, GBool scrollToBottom, GBool addToHist = gTrue);
    ...
    GBool getSelection(int *pg, double *ulx, double *uly, double *lrx, double *lry);
    GBool hasSelection();
    ...
    GString *extractText(int pg, double xMin, double yMin, double xMax, double yMax);
    ...
    virtual GBool
    findU(Unicode *u, int len, GBool caseSensitive, GBool next, GBool backward, GBool wholeWord, GBool onePageOnly);
    ...
    GBool canGoBack() {
        return historyBLen > 1;
    }
    GBool canGoForward() {
        return historyFLen > 0;
    }
    int getScrollX();
    int getScrollY();
    int getWindowWidth();
...
protected:
    ...
    void getSelectRectListBBox(GList *rects, int *wxMin, int *wyMin, int *wxMax, int *wyMax);

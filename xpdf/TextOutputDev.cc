...
enum TextBlockTag { blkTagMulticolumn, blkTagColumn, blkTagSuperLine, blkTagLine };
...
class TextLink {
public:
    TextLink(double xMinA, double yMinA, double xMaxA, double yMaxA, GString *uriA) {
        xMin = xMinA;
        yMin = yMinA;
        xMax = xMaxA;
        yMax = yMaxA;
...
TextWord::TextWord(GList *chars, int start, int lenA, int rotA, int dirA, GBool spaceAfterA) {
    TextChar *ch;
    int i;
...
void TextWord::getCharBBox(int charIdx, double *xMinA, double *yMinA, double *xMaxA, double *yMaxA) {
...
TextLine::TextLine(GList *wordsA, double xMinA, double yMinA, double xMaxA, double yMaxA, double fontSizeA) {
    TextWord *word;
    int i, j, k;
...
TextColumn::TextColumn(GList *paragraphsA, double xMinA, double yMinA, double xMaxA, double yMaxA) {
...
int TextPosition::operator==(TextPosition pos) {
    return colIdx == pos.colIdx && parIdx == pos.parIdx && lineIdx == pos.lineIdx && charIdx == pos.charIdx;
}
...
int TextPosition::operator<(TextPosition pos) {
    return colIdx < pos.colIdx ||
           (colIdx == pos.colIdx &&
            (parIdx < pos.parIdx ||
             (parIdx == pos.parIdx &&
              (lineIdx < pos.lineIdx || (lineIdx == pos.lineIdx && charIdx < pos.charIdx)))));
}
...
void TextPage::updateFont(GfxState *state) {
    ...
    if(gfxFont && gfxFont->getType() == fontType3) {
        ...
        for(code = 0; code < 256; ++code) {
            ...
            if(letterCode < 0 && name &&
               ((name[0] >= 'A' && name[0] <= 'Z') || (name[0] >= 'a' && name[0] <= 'z')) && name[1] == '\0') {
                letterCode = code;
            }
            if(anyCode < 0 && name && ((Gfx8BitFont *)gfxFont)->getWidth((Guchar)code) > 0) {
                anyCode = code;
            }
        }
        if(mCode >= 0 && (w = ((Gfx8BitFont *)gfxFont)->getWidth((Guchar)mCode)) > 0) {
            // 0.6 is a generic average 'm' width -- yes, this is a hack
            curFontSize *= w / 0.6;
        } else if(letterCode >= 0 && (w = ((Gfx8BitFont *)gfxFont)->getWidth((Guchar)letterCode)) > 0) {
            // even more of a hack: 0.5 is a generic letter width
            curFontSize *= w / 0.5;
        } else if(anyCode >= 0 && (w = ((Gfx8BitFont *)gfxFont)->getWidth((Guchar)anyCode)) > 0) {
...
void TextPage::addChar(GfxState *state, double x, double y, double dx, double dy, CharCode c, int nBytes, Unicode *u, int uLen) {
    ...
    if(x1 + w1 < control.marginLeft || x1 > pageWidth - control.marginRight || y1 + h1 < control.marginTop ||
       y1 > pageHeight - control.marginBottom || w1 > pageWidth || h1 > pageHeight) {
        charPos += nBytes;
        return;
    }
    ...
    if(uLen == 1 && (u[0] == (Unicode)0x20 || u[0] == (Unicode)0x09 || u[0] == (Unicode)0xa0)) {
        charPos += nBytes;
        if(chars->getLength() > 0) {
            ((TextChar *)chars->get(chars->getLength() - 1))->spaceAfter = (char)gTrue;
        }
        return;
    }
    ...
    if(uBufLen > 0) {
        ...
        for(i = 0; i < uBufLen; ++i) {
            ...
            if(control.clipText || control.discardClippedText) {
                ...
                xMid = 0.5 * (xMin + xMax);
                yMid = 0.5 * (yMin + yMax);
                if(xMid < clipXMin || xMid > clipXMax || yMid < clipYMin || yMid > clipYMax) {
                    clipped = gTrue;
                }
            }
            ...
            chars->append(new TextChar(uBuf[j], charPos, nBytes, xMin, yMin, xMax, yMax, curRot,
                                       clipped, state->getRender() == 3 || alpha < 0.001, curFont,
                                       curFontSize, colToDbl(rgb.r), colToDbl(rgb.g), colToDbl(rgb.b)));
        }
    }
...
void TextPage::endActualText(GfxState *state) {
    ...
    if(actualTextNBytes) {
        ...
        // addChar()
        addChar(state, actualTextX0, actualTextY0, actualTextX1 - actualTextX0,
...
void TextPage::addLink(double xMin, double yMin, double xMax, double yMax, Link *link) {
    GString *uri;
...
void TextPage::write(void *outputStream, TextOutputFunc outputFunc) {
    ...
    switch(control.mode) {
        case textOutReadingOrder:
            writeReadingOrder(outputStream, outputFunc, uMap, space, spaceLen, eol, eolLen);
            break;
        case textOutPhysLayout:
        case textOutTableLayout:
            writePhysLayout(outputStream, outputFunc, uMap, space, spaceLen, eol, eolLen);
            break;
        case textOutSimpleLayout:
            writeSimpleLayout(outputStream, outputFunc, uMap, space, spaceLen, eol, eolLen);
            break;
        case textOutLinePrinter:
            writeLinePrinter(outputStream, outputFunc, uMap, space, spaceLen, eol, eolLen);
            break;
  ...
  dumpColumns(columns);
    ...
    for(colIdx = 0; colIdx < columns->getLength(); ++colIdx) {
        ...
        for(parIdx = 0; parIdx < col->paragraphs->getLength() && y < ph; ++parIdx) {
            par = (TextParagraph *)col->paragraphs->get(parIdx);
            for(lineIdx = 0; lineIdx < par->lines->getLength() && y < ph; ++lineIdx) {
                line = (TextLine *)par->lines->get(lineIdx);
                if(!out[y]) {
                    out[y] = new GString();
                }
...
void TextPage::writeLinePrinter(void *outputStream,
    ...
    if(chars->getLength()) {
        ...
        yShift = yMin0 - (int)(yMin0 / lineSpacing + 0.5) * lineSpacing - 0.5 * lineSpacing;
    } else {
        yShift = 0;
    }
    ...
    for(y = yShift; y < pageHeight; y += lineSpacing) {
        ...
        line = new GList;
        while(i < chars->getLength() && ((TextChar *)chars->get(i))->yMin < y + lineSpacing) {
                    ...
                    default:
                        if(fabs(ch2->yMin - ch->yMin) > rawModeLineDelta * ch->fontSize ||
                           ch2->xMin - ch->xMax < -rawModeCharOverlap * ch->fontSize) {
                            s->append(eol, eolLen);
                        } else if(ch->spaceAfter || ch2->xMin - ch->xMax > rawModeWordSpacing * ch->fontSize) {
                            s->append(space, spaceLen);
                        }
                        break;
                    case 1:
                        if(fabs(ch->xMax - ch2->xMax) > rawModeLineDelta * ch->fontSize ||
                           ch2->yMin - ch->yMax < -rawModeCharOverlap * ch->fontSize) {
                            s->append(eol, eolLen);
                        } else if(ch->spaceAfter || ch2->yMin - ch->yMax > rawModeWordSpacing * ch->fontSize) {
                            s->append(space, spaceLen);
                        }
                        break;
                    case 2:
                        if(fabs(ch->yMax - ch2->yMax) > rawModeLineDelta * ch->fontSize ||
                           ch->xMin - ch2->xMax < -rawModeCharOverlap * ch->fontSize) {
                            s->append(eol, eolLen);
                        } else if(ch->spaceAfter || ch->xMin - ch2->xMax > rawModeWordSpacing * ch->fontSize) {
                            s->append(space, spaceLen);
                        }
                        break;
                    case 3:
                        if(fabs(ch2->xMin - ch->xMin) > rawModeLineDelta * ch->fontSize ||
                           ch->yMin - ch2->yMax < -rawModeCharOverlap * ch->fontSize) {
                            s->append(eol, eolLen);
                        } else if(ch->spaceAfter || ch->yMin - ch2->yMax > rawModeWordSpacing * ch->fontSize) {
                            s->append(space, spaceLen);
                        }
                        break;
                }
...
void TextPage::encodeFragment(Unicode *text, int len, UnicodeMap *uMap, GBool primaryLR, GString *s) {
        ...
        for(i = 0; i < charsA->getLength(); ++i) {
            ...
            while(j < charsA->getLength()) {
                ...
                if(ch2->c == ch->c && fabs(ch2->xMin - ch->xMin) < xDelta &&
                   fabs(ch2->xMax - ch->xMax) < xDelta && fabs(ch2->yMax - ch->yMax) < yDelta) {
                    if(ch2->spaceAfter) {
        ...
        for(i = 0; i < charsA->getLength(); ++i) {
            ...
            while(j < charsA->getLength()) {
                ...
                if(ch2->c == ch->c && fabs(ch2->xMax - ch->xMax) < xDelta &&
                   fabs(ch2->yMin - ch->yMin) < yDelta && fabs(ch2->yMax - ch->yMax) < yDelta) {
                    charsA->del(j);
                } else {
                    ++j;
                }
...
TextBlock *TextPage::splitChars(GList *charsA) {
    ...
    for(rot = 0; rot < 4; ++rot) {
        ...
        for(i = 0; i < charsA->getLength(); ++i) {
            ch = (TextChar *)charsA->get(i);
            if(ch->rot == rot && !(control.discardInvisibleText && ch->invisible) &&
...
TextBlock *TextPage::split(GList *charsA, int rot) {
    ...
    findGaps(charsA, rot, &xMin, &yMin, &xMax, &yMax, &avgFontSize, horizGaps, vertGaps);
    ...
    if(rot & 1) {
        ...
        } else if(nHorizGaps > 0 && (horizGapSize > vertGapSize || control.mode == textOutTableLayout) &&
                  horizGapSize > vertGapThreshold && (minHorizChunkWidth > minChunk || nVertGaps == 0)) {
            doHorizSplit = gTrue;
        } else if(nVertGaps > 0) {
            doVertSplit = gTrue;
        ...
        } else if(nVertGaps > 0 && (vertGapSize > horizGapSize || control.mode == textOutTableLayout) &&
                  vertGapSize > vertGapThreshold && (minVertChunkWidth > minChunk || nHorizGaps == 0)) {
            doVertSplit = gTrue;
        } else if(nHorizGaps > 0) {
            doHorizSplit = gTrue;
...
GList *TextPage::getChars(GList *charsA, double xMin, double yMin, double xMax, double yMax) {
    GList *ret;
    TextChar *ch;
    double x, y;
    int i;
...
void TextPage::findGaps(GList *charsA,
                        int rot,
                        double *xMinOut,
                        double *yMinOut,
                        double *xMaxOut,
    ...
    *avgFontSizeOut = avgFontSize;
    ...
    if(xMin * invSplitPrecision < 0.5 * INT_MIN || xMax * invSplitPrecision > 0.5 * INT_MAX ||
       yMin * invSplitPrecision < 0.5 * INT_MIN || xMax * invSplitPrecision > 0.5 * INT_MAX) {
        return;
    }
...
void TextPage::insertLargeChars(GList *largeChars, TextBlock *blk) {
    ...
    for(i = 1; i < largeChars->getLength(); ++i) {
        ...
        minOverlap = 0.5 * (ch->fontSize < ch2->fontSize ? ch->fontSize : ch2->fontSize);
...
void TextPage::insertColumnIntoTree(TextBlock *column, TextBlock *tree) {
    ...
    for(i = 0; i < tree->children->getLength(); ++i) {
        ...
        if(child->tag == blkTagMulticolumn && column->xMin >= child->xMin &&
           column->yMin >= child->yMin && column->xMax <= child->xMax && column->yMax <= child->yMax) {
...
TextBlock *TextPage::findClippedCharLeaf(TextChar *ch, TextBlock *tree) {
    ...
    if(tree->type == blkLeaf) {
        if(tree->rot == 0) {
            if(y > tree->yMin && y < tree->yMax && ch->xMin <= tree->xMax + clippedTextMaxWordSpace * ch->fontSize) {
                return tree;
            }
        }
    } else {
...
TextColumn *TextPage::buildColumn(TextBlock *blk) {
            ...
            if(indent1 - indent0 > minParagraphIndent * fontSize0 &&
               ...
               getLineSpacing(line0, line1) <= spaceThresh) {
                ...
                for(++i; i < lines->getLength(); ++i) {
                    ...
                    if(getLineSpacing((TextLine *)lines->get(i - 1), line1) > spaceThresh) {
                        break;
                    }
                    parLines->append(line1);
                }
                ...
                // drop cap
                      ...
                      indent1 - indent0 > minParagraphIndent * fontSize1 && getLineSpacing(line0, line1) < 0) {
                dropCap = gTrue;
                parLines->append(line1);
                fontSize0 = fontSize1;
                for(++i; i < lines->getLength(); ++i) {
    ...
    return new TextColumn(paragraphs, blk->xMin, blk->yMin, blk->xMax, blk->yMax);
}
...
double TextPage::getAverageLineSpacing(GList *lines) {
    ...
    for(i = 1; i < lines->getLength(); ++i) {
        sp = getLineSpacing((TextLine *)lines->get(i - 1), (TextLine *)lines->get(i));
        if(sp > 0) {
            avg += sp;
            ++n;
        }
...
TextLine *TextPage::buildLine(TextBlock *blk) {
    ...
    while(i < charsA->getLength()) {
        ...
        for(j = i + 1; j < charsA->getLength(); ++j) {
            ...
            if(ch->font != ch2->font || fabs(ch->fontSize - ch2->fontSize) > 0.01 ||
               (dir && dir2 && dir2 != dir) ||
        ...
        word = new TextWord(charsA, i, j - i, blk->rot, dir, (blk->rot >= 2) ? spaceBefore : spaceAfter);
    ...
    return new TextLine(words, blk->xMin, blk->yMin, blk->xMax, blk->yMax, lineFontSize);
}
...
double TextPage::computeWordSpacingThreshold(GList *charsA, int rot) {
    ...
    if(maxGap - minGap < uniformSpacing * avgFontSize) {
        if(minSpGap <= maxSpGap) {
            if(minAdjGap <= maxAdjGap && minSpGap - maxAdjGap > 0.01) {
                return 0.5 * (maxAdjGap + minSpGap);
            } else if(minAdjGap > maxAdjGap && maxSpGap - minSpGap < uniformSpacing * avgFontSize) {
                return minSpGap - 1;
            }
        }
        return maxGap + 1;
        ...
        if(minAdjGap <= maxAdjGap && minSpGap <= maxSpGap && minSpGap - maxAdjGap > uniformSpacing * avgFontSize) {
        ...
        for(parIdx = 0; parIdx < col->paragraphs->getLength(); ++parIdx) {
            ...
            for(lineIdx = 0; lineIdx < par->lines->getLength(); ++lineIdx) {
                ...
                if(control.fixedPitch > 0) {
                    ...
                    line->px = (int)((line->xMin - col->xMin) / (physLayoutSpaceWidth * line->fontSize));
                }
...
void TextPage::assignSimpleLayoutPositions(GList *superLines, UnicodeMap *uMap) {
    GList *lines;
    TextLine *line0, *line1;
    double xMin, xMax;
    int px, px2, sp, i, j;
        ...
        for(parIdx = 0; parIdx < col->paragraphs->getLength(); ++parIdx) {
            ...
            for(lineIdx = 0; lineIdx < par->lines->getLength(); ++lineIdx) {
                ...
                for(wordIdx = 0; wordIdx < line->words->getLength(); ++wordIdx) {
                    ...
                    for(i = 0; i < underlines->getLength(); ++i) {
                        ...
                        if(underline->horiz) {
                            if(word->rot == 0 || word->rot == 2) {
                                if(fabs(underline->y0 - base) < ubSlack && underline->x0 < word->xMin + uSlack &&
                            ...
                            if(word->rot == 1 || word->rot == 3) {
                                if(fabs(underline->x0 - base) < ubSlack && underline->y0 < word->yMin + uSlack &&
                    ...
                    for(i = 0; i < links->getLength(); ++i) {
                        link = (TextLink *)links->get(i);
                        if(link->xMin < word->xMin + hSlack && word->xMax - hSlack < link->xMax &&
                           link->yMin < word->yMin + hSlack && word->yMax - hSlack < link->yMax) {
                            word->link = link;
                        }
                    }
                }
...
GBool TextPage::findText(Unicode *s,
    ...
    for(colIdx = backward ? findCols->getLength() - 1 : 0;
        backward ? colIdx >= 0 : colIdx < findCols->getLength(); colIdx += backward ? -1 : 1) {
        column = (TextColumn *)findCols->get(colIdx);

        // check: is the column above the top limit?
        if(!startAtTop && (backward ? column->yMin > yStart : column->yMax < yStart)) {
            continue;
        }

        // check: is the column below the bottom limit?
        if(!stopAtBottom && (backward ? column->yMax < yStop : column->yMin > yStop)) {
            continue;
        }

        for(parIdx = backward ? column->paragraphs->getLength() - 1 : 0;
            backward ? parIdx >= 0 : parIdx < column->paragraphs->getLength(); parIdx += backward ? -1 : 1) {
            par = (TextParagraph *)column->paragraphs->get(parIdx);

            // check: is the paragraph above the top limit?
            if(!startAtTop && (backward ? par->yMin > yStart : par->yMax < yStart)) {
                continue;
            }

            // check: is the paragraph below the bottom limit?
            if(!stopAtBottom && (backward ? par->yMax < yStop : par->yMin > yStop)) {
                continue;
            }

            for(lineIdx = backward ? par->lines->getLength() - 1 : 0;
                backward ? lineIdx >= 0 : lineIdx < par->lines->getLength(); lineIdx += backward ? -1 : 1) {
                line = (TextLine *)par->lines->get(lineIdx);

                // check: is the line above the top limit?
                if(!startAtTop && (backward ? line->yMin > yStart : line->yMax < yStart)) {
                    continue;
                }

                // check: is the line below the bottom limit?
                if(!stopAtBottom && (backward ? line->yMax < yStop : line->yMin > yStop)) {
                    continue;
                }
                ...
                while(backward ? j >= 0 : j <= m - len) {
                    if(!wholeWord || ((j == 0 || !unicodeTypeWord(txt[j - 1])) &&
...
GString *TextPage::getText(double xMin, double yMin, double xMax, double yMax, GBool forceEOL) {
    UnicodeMap *uMap;
    char space[8], eol[16];
    int spaceLen, eolLen;
    GList *chars2;
...
GBool TextPage::findCharRange(int pos, int length, double *xMin, double *yMin, double *xMax, double *yMax) {
    TextChar *ch;
...
GBool TextPage::checkPointInside(double x, double y) {
    ...
    for(colIdx = 0; colIdx < findCols->getLength(); ++colIdx) {
        ...
        if(x >= col->getXMin() && x <= col->getXMax() && y >= col->getYMin() && y <= col->getYMax()) {
            return gTrue;
        }
    }
    return gFalse;
...
void TextPage::findPointInColumn(TextColumn *col, double x, double y, TextPosition *pos) {
    TextParagraph *par;
    TextLine *line;
    GList *pars, *lines;
...
void TextPage::convertPosToPointUpper(TextPosition *pos, double *x, double *y) {
    TextColumn *col;
    TextParagraph *par;
    TextLine *line;
...
void TextPage::convertPosToPointLower(TextPosition *pos, double *x, double *y) {
    TextColumn *col;
    TextParagraph *par;
    TextLine *line;
...
void TextPage::convertPosToPointLeftEdge(TextPosition *pos, double *x, double *y) {
    TextColumn *col;
    TextParagraph *par;
    TextLine *line;
...
void TextPage::convertPosToPointRightEdge(TextPosition *pos, double *x, double *y) {
    TextColumn *col;
    TextParagraph *par;
    TextLine *line;
...
TextWordList *TextPage::makeWordListForRect(double xMin, double yMin, double xMax, double yMax) {
    TextWordList *words;
    GList *chars2;
    TextChar *ch;
    double xx, yy;
...
TextOutputDev::TextOutputDev(char *fileName, TextOutputControl *controlA, GBool append) {
    text = NULL;
    control = *controlA;
    ok = gTrue;
...
TextOutputDev::TextOutputDev(TextOutputFunc func, void *stream, TextOutputControl *controlA) {
    outputFunc = func;
    outputStream = stream;
    needClose = gFalse;
    control = *controlA;
...
GBool TextOutputDev::findText(Unicode *s,
                              ...
                              double *xMax,
                              double *yMax) {
    return text->findText(s, len, startAtTop, stopAtBottom, startAtLast, stopAtLast, caseSensitive,
                          backward, wholeWord, xMin, yMin, xMax, yMax);
}

GString *TextOutputDev::getText(double xMin, double yMin, double xMax, double yMax) {
    return text->getText(xMin, yMin, xMax, yMax);
}

GBool TextOutputDev::findCharRange(int pos, int length, double *xMin, double *yMin, double *xMax, double *yMax) {
...
TextWordList *TextOutputDev::makeWordListForRect(double xMin, double yMin, double xMax, double yMax) {

...
Object *Parser::getObj(Object *obj,
    ...
    if(!simpleOnly && recursion < recursionLimit && buf1.isCmd("[")) {
        ...
        while(!buf1.isCmd("]") && !buf1.isEOF())
            obj->arrayAdd(getObj(&obj2, gFalse, fileKey, encAlgorithm, keyLength, objNum, objGen, recursion + 1));
        if(buf1.isEOF())
        ...
        while(!buf1.isCmd(">>") && !buf1.isEOF()) {
            if(!buf1.isName()) {
                ...
                obj->dictAdd(key, getObj(&obj2, gFalse, fileKey, encAlgorithm, keyLength, objNum,
                                         objGen, recursion + 1));
            }
        }
        if(buf1.isEOF())
        ...
        s2 = new GString();
        obj2.initNull();
        decrypt = new DecryptStream(new MemStream(s->getCString(), 0, s->getLength(), &obj2),
...
Stream *
Parser::makeStream(Object *dict, Guchar *fileKey, CryptAlgorithm encAlgorithm, int keyLength, int objNum, int objGen, int recursion) {
    Object obj;
    BaseStream *baseStr;
    Stream *str, *str2;

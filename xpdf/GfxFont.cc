...
static Base14FontMapEntry base14FontMap[] = {{"Arial", "Helvetica"},
...
static const char *base14SubstFonts[14] = {"Courier", "Courier-Oblique", "Courier-Bold",
                                           "Courier-BoldOblique", "Helvetica", "Helvetica-Oblique",
                                           "Helvetica-Bold", "Helvetica-BoldOblique", "Times-Roman",
...
GfxFont::GfxFont(const char *tagA, Ref idA, GString *nameA, GfxFontType typeA, Ref embFontIDA) {
    ok = gFalse;
...
GfxFontLoc *GfxFont::locateFont(XRef *xref, GBool ps) {
    ...
    //----- system font
    if(name && (path = globalParams->findSystemFontFile(name, &sysFontType, &fontNum))) {
        fontLoc = new GfxFontLoc();
    ...
    if(ps && name &&
       ((psFont16 = globalParams->getPSResidentFont16(name, ((GfxCIDFont *)this)->getWMode())))) {
        fontLoc = new GfxFontLoc();
        fontLoc->locType = gfxFontLocResident;
    ...
    if(ps && ((psFont16 = globalParams->getPSResidentFontCC(((GfxCIDFont *)this)->getCollection(),
                                                            ((GfxCIDFont *)this)->getWMode())))) {
        error(errSyntaxWarning, -1, "Substituting font '{0:t}' for '{1:t}'", psFont16->psFontName, name);
        fontLoc = new GfxFontLoc();
        fontLoc->locType = gfxFontLocResident;
        fontLoc->fontType = fontCIDType0; // this is not used
        fontLoc->path = psFont16->psFontName->copy();
    ...
    if((path = globalParams->findCCFontFile(((GfxCIDFont *)this)->getCollection()))) {
        if((fontLoc = getExternalFont(path, 0, 0, gTrue))) {
            error(errSyntaxWarning, -1, "Substituting font '{0:t}' for '{1:t}'", fontLoc->path, name);
            return fontLoc;
        }
    }
...
GfxFontLoc *GfxFont::getExternalFont(GString *path, int fontNum, double oblique, GBool cid) {
    FoFiIdentifierType fft;
...
Gfx8BitFont::Gfx8BitFont(XRef *xref, const char *tagA, Ref idA, GString *nameA, GfxFontType typeA, Ref embFontIDA, Dict *fontDict)
        ...
        if(obj2.isArray()) {
            ...
            for(i = 0; i < obj2.arrayGetLength(); ++i) {
                ...
                if(obj3.isInt()) {
                    ...
                    error(errSyntaxError, -1, "Wrong type in font encoding resource differences ({0:s})",
                          obj3.getTypeName());
                }
                obj3.free();
            }
        ...
        if((charName = enc[code])) {
            if(!(toUnicode[code] = globalParams->mapNameToUnicode(charName)) &&
               strcmp(charName, ".notdef")) {
                ...
                if((strlen(charName) == 3 && isalpha(charName[0] & 0xff) &&
                    isxdigit(charName[1] & 0xff) && isxdigit(charName[2] & 0xff) &&
                    ((charName[1] >= 'a' && charName[1] <= 'f') || (charName[1] >= 'A' && charName[1] <= 'F') ||
                     (charName[2] >= 'a' && charName[2] <= 'f') || (charName[2] >= 'A' && charName[2] <= 'F'))) ||
                   (strlen(charName) == 2 && isxdigit(charName[0] & 0xff) && isxdigit(charName[1] & 0xff) &&
                    ((charName[0] >= 'a' && charName[0] <= 'f') || (charName[0] >= 'A' && charName[0] <= 'F') ||
                     (charName[1] >= 'a' && charName[1] <= 'f') || (charName[1] >= 'A' && charName[1] <= 'F')))) {
                    hex = gTrue;
                }
                missing = gTrue;
    ...
    if(missing && globalParams->getMapNumericCharNames()) {
        for(code = 0; code < 256; ++code) {
            if((charName = enc[code]) && !toUnicode[code] && strcmp(charName, ".notdef")) {
                ...
                code2 = -1;
                if(hex && n == 3 && isalpha(charName[0] & 0xff) && isxdigit(charName[1] & 0xff) &&
                   isxdigit(charName[2] & 0xff)) {
                    sscanf(charName + 1, "%x", &code2);
                } else if(hex && n == 2 && isxdigit(charName[0] & 0xff) && isxdigit(charName[1] & 0xff)) {
                    sscanf(charName, "%x", &code2);
                } else if(!hex && n >= 2 && n <= 4 && isdigit(charName[0] & 0xff) &&
                          isdigit(charName[1] & 0xff)) {
                    code2 = atoi(charName);
                ...
                } else if(n >= 7 && charName[0] == 'u' && charName[1] == 'n' && charName[2] == 'i' &&
                          isxdigit(charName[3] & 0xff) && isxdigit(charName[4] & 0xff) &&
                          isxdigit(charName[5] & 0xff) && isxdigit(charName[6] & 0xff)) {
...
GBool Gfx8BitFont::problematicForUnicode() {
    ...
    if(name) {
        ...
        symbolic = strstr(nameLC->getCString(), "dingbat") ||
                   strstr(nameLC->getCString(), "wingding") || strstr(nameLC->getCString(), "commpi");
        delete nameLC;
        if(symbolic) {
            return gFalse;
        }
...
GfxCIDFont::GfxCIDFont(XRef *xref, const char *tagA, Ref idA, GString *nameA, GfxFontType typeA, Ref embFontIDA, Dict *fontDict)
    : GfxFont(tagA, idA, nameA, typeA, embFontIDA) {
    Dict *desFontDict;
    ...
    identityEnc = obj1.isName("Identity-H") && !collection->cmp("Adobe-Identity");
            ...
            if(obj2.isInt() && obj3.isInt() && i + 2 < obj1.arrayGetLength()) {
                if(obj1.arrayGet(i + 2, &obj4)->isNum()) {
                    if(widths.nExceps == excepsSize) {
                        excepsSize += 16;
                        widths.exceps = (GfxFontCIDWidthExcep *)greallocn(widths.exceps, excepsSize,
    ...
    if(desFontDict->lookup("W2", &obj1)->isArray()) {
        ...
        while(i + 1 < obj1.arrayGetLength()) {
            ...
            if(obj2.isInt() && obj3.isInt() && i + 4 < obj1.arrayGetLength()) {
                if(obj1.arrayGet(i + 2, &obj4)->isNum() && obj1.arrayGet(i + 3, &obj5)->isNum() &&
                   obj1.arrayGet(i + 4, &obj6)->isNum()) {
                    if(widths.nExcepsV == excepsSize) {
                        excepsSize += 16;
                        widths.excepsV = (GfxFontCIDWidthExcepV *)greallocn(widths.excepsV, excepsSize,
                ...
                for(k = 0; k + 2 < obj3.arrayGetLength(); k += 3) {
                    if(obj3.arrayGet(k, &obj4)->isNum() && obj3.arrayGet(k + 1, &obj5)->isNum() &&
...
GfxFont *GfxFontDict::lookupByRef(Ref ref) {
    ...
    for(i = 0; i < uniqueFonts->getLength(); ++i) {
        ...
        if(font->getID()->num == ref.num && font->getID()->gen == ref.gen) {
            return font;
        }
    }
    return NULL;

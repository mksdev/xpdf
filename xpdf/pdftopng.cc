...
static ArgDesc argDesc[] = {
    ...
    {"-mono", argFlag, &mono, 0, "generate a monochrome PNG file"},
    {"-gray", argFlag, &gray, 0, "generate a grayscale PNG file"},
    {"-alpha", argFlag, &pngAlpha, 0, "include an alpha channel in the PNG file"},
#if HAVE_FREETYPE_H
...
static void
setupPNG(png_structp *png, png_infop *pngInfo, FILE *f, int bitDepth, int colorType, double res, SplashBitmap *bitmap);
...
int main(int argc, char *argv[]) {
    ...
    for(pg = firstPage; pg <= lastPage; ++pg) {
        ...
        if(mono) {
            ...
            setupPNG(&png, &pngInfo, f, 8, pngAlpha ? PNG_COLOR_TYPE_GRAY_ALPHA : PNG_COLOR_TYPE_GRAY,
            ...
            setupPNG(&png, &pngInfo, f, 8, pngAlpha ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB,
                     resolution, splashOut->getBitmap());
            writePNGData(png, splashOut->getBitmap());
            finishPNG(&png, &pngInfo);
            fclose(f);
...
setupPNG(png_structp *png, png_infop *pngInfo, FILE *f, int bitDepth, int colorType, double res, SplashBitmap *bitmap) {
    ...
    png_init_io(*png, f);
    png_set_IHDR(*png, *pngInfo, bitmap->getWidth(), bitmap->getHeight(), bitDepth, colorType,

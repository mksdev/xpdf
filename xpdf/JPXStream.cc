...
void JPXStream::fillReadBuf() {
    ...
    do {
        ...
        tileIdx = (((curY << reduction) - img.yTileOffset) / img.yTileSize) * img.nXTiles +
...
#endif
        tx = jpxFloorDiv(curX - jpxCeilDivPow2(img.tiles[tileIdx].x0, reduction), tileComp->hSep);
        ty = jpxFloorDiv(curY - jpxCeilDivPow2(img.tiles[tileIdx].y0, reduction), tileComp->vSep);
    ...
    if(bufStr->lookChar() == 0xff) {
        ...
        while(readBoxHdr(&boxType, &boxLen, &dataLen)) {
            if(boxType == 0x6a703268) { // JP2 header
                ...
                cover(1);
                if(readULong(&dummy) && readULong(&dummy) && readUWord(&dummy) &&
                   readUByte(&bpc1) && readUByte(&dummy) && readUByte(&dummy) && readUByte(&dummy)) {
...
void JPXStream::getImageParams2(int *bitsPerComponent, StreamColorSpaceMode *csMode) {
    ...
    while(readMarkerHdr(&segType, &segLen)) {
        if(segType == 0x51) { // SIZ - image and tile size
            cover(5);
            if(readUWord(&dummy) && readULong(&dummy) && readULong(&dummy) && readULong(&dummy) &&
               readULong(&dummy) && readULong(&dummy) && readULong(&dummy) && readULong(&dummy) &&
               readULong(&dummy) && readUWord(&nComps1) && readUByte(&bpc1)) {
    ...
    while(readBoxHdr(&boxType, &boxLen, &dataLen)) {
        switch(boxType) {
            ...
            case 0x69686472: // image header
                cover(9);
                if(!readULong(&h) || !readULong(&w) || !readUWord(&n) || !readUByte(&bpc1) ||
                   !readUByte(&compression) || !readUByte(&unknownColorspace) || !readUByte(&ipr)) {
            ...
            case 0x70636c72: // palette
                ...
                for(i = 0; i < palette.nEntries; ++i) {
                    for(j = 0; j < palette.nComps; ++j) {
                        if(!readNBytes(((palette.bpc[j] & 0x7f) + 7) >> 3, (palette.bpc[j] & 0x80) ? gTrue : gFalse,
            ...
            case 0x636d6170: // component mapping
                ...
                for(i = 0; i < compMap.nChannels; ++i) {
                    if(!readUWord(&compMap.comp[i]) || !readUByte(&compMap.type[i]) ||
            ...
            case 0x63646566: // channel definition
                ...
                channelDefn.idx = (Guint *)gmallocn(channelDefn.nChannels, sizeof(Guint));
                channelDefn.type = (Guint *)gmallocn(channelDefn.nChannels, sizeof(Guint));
                channelDefn.assoc = (Guint *)gmallocn(channelDefn.nChannels, sizeof(Guint));
                for(i = 0; i < channelDefn.nChannels; ++i) {
                    if(!readUWord(&channelDefn.idx[i]) || !readUWord(&channelDefn.type[i]) ||
    ...
    do {
        ...
        switch(segType) {
            ...
            case 0x51: // SIZ - image and tile size
                ...
                if(!readUWord(&capabilities) || !readULong(&img.xSize) || !readULong(&img.ySize) ||
                   !readULong(&img.xOffset) || !readULong(&img.yOffset) || !readULong(&img.xTileSize) ||
                ...
                if(img.xSize == 0 || img.ySize == 0 || img.xOffset >= img.xSize || img.yOffset >= img.ySize ||
                   img.xTileSize == 0 || img.yTileSize == 0 || img.xTileOffset > img.xOffset ||
                   img.yTileOffset > img.yOffset || img.xTileSize + img.xTileOffset <= img.xOffset ||
            ...
            case 0x52: // COD - coding style default
                ...
                if(!readUByte(&style) || !readUByte(&progOrder) || !readUWord(&nLayers) ||
                   !readUByte(&multiComp) || !readUByte(&nDecompLevels) || !readUByte(&codeBlockW) ||
                   !readUByte(&codeBlockH) || !readUByte(&codeBlockStyle) || !readUByte(&transform)) {
                    error(errSyntaxError, getPos(), "Error in JPX COD marker segment");
                    return jpxDecodeFatalError;
                }
                if(nDecompLevels < 1 || nDecompLevels > 31 || codeBlockW > 8 || codeBlockH > 8) {
                    error(errSyntaxError, getPos(), "Error in JPX COD marker segment");
                    return jpxDecodeFatalError;
                }
#if 1 //~ progression orders 2-4 are unimplemented
                if(progOrder >= 2) {
                    error(errUnimplemented, -1, "JPX progression order {0:d} is unimplemented", progOrder);
                }
#endif
                codeBlockW += 2;
                codeBlockH += 2;
            ...
            case 0x53: // COC - coding style component
                ...
                if((img.nComps > 256 && !readUWord(&comp)) ||
                   ...
                   !readUByte(&style) || !readUByte(&nDecompLevels) || !readUByte(&codeBlockW) ||
                   !readUByte(&codeBlockH) || !readUByte(&codeBlockStyle) || !readUByte(&transform)) {
                ...
                for(i = 0; i < img.nXTiles * img.nYTiles; ++i) {
                    ...
                    img.tiles[i].tileComps[comp].transform = transform;
                    img.tiles[i].tileComps[comp].resLevels =
                        (JPXResLevel *)greallocn(img.tiles[i].tileComps[comp].resLevels,
                                                 nDecompLevels + 1, sizeof(JPXResLevel));
                    for(r = 0; r <= nDecompLevels; ++r) {
                        img.tiles[i].tileComps[comp].resLevels[r].precincts = NULL;
                    }
                }
                ...
                if((img.tiles[0].tileComps[0].quantStyle & 0x1f) == 0x00) {
                    ...
                    img.tiles[0].tileComps[0].quantSteps =
                        (Guint *)greallocn(img.tiles[0].tileComps[0].quantSteps,
                                           img.tiles[0].tileComps[0].nQuantSteps, sizeof(Guint));
            ...
            case 0x5d: // QCC - quantization component
                ...
                if((img.nComps > 256 && !readUWord(&comp)) || (img.nComps <= 256 && !readUByte(&comp)) ||
                   comp >= img.nComps || !readUByte(&img.tiles[0].tileComps[comp].quantStyle)) {
                ...
                if((img.tiles[0].tileComps[comp].quantStyle & 0x1f) == 0x00) {
                    ...
                    img.tiles[0].tileComps[comp].quantSteps =
                        (Guint *)greallocn(img.tiles[0].tileComps[comp].quantSteps,
                                           img.tiles[0].tileComps[comp].nQuantSteps, sizeof(Guint));
...
#else
                if((img.nComps > 256 && !readUWord(&comp)) || (img.nComps <= 256 && !readUByte(&comp)) ||
                   comp >= img.nComps || !readUByte(&compInfo[comp].defROI.style) ||
            ...
            case 0x5f: // POC - progression order change
                ...
                for(i = 0; i < nProgs; ++i) {
                    if(!readUByte(&progs[i].startRes) || !(img.nComps > 256 && readUWord(&progs[i].startComp)) ||
                       !(img.nComps <= 256 && readUByte(&progs[i].startComp)) ||
                       !readUWord(&progs[i].endLayer) || !readUByte(&progs[i].endRes) ||
                       !(img.nComps > 256 && readUWord(&progs[i].endComp)) ||
                       !(img.nComps <= 256 && readUByte(&progs[i].endComp)) || !readUByte(&progs[i].progOrder)) {
                        error(errSyntaxError, getPos(), "Error in JPX POC marker segment");
                        return jpxDecodeFatalError;
                    }
                }
...
GBool JPXStream::readTilePart() {
    ...
    if(tileIdx >= img.nXTiles * img.nYTiles || tilePartIdx != img.tiles[tileIdx].nextTilePart ||
       (tilePartIdx > 0 && !img.tiles[tileIdx].init) || (tilePartIdx == 0 && img.tiles[tileIdx].init)) {
        ...
        switch(segType) {
            case 0x52: // COD - coding style default
                ...
                if(img.tiles[tileIdx].progOrder >= 2) {
                    error(errUnimplemented, -1, "JPX progression order {0:d} is unimplemented",
                ...
                for(comp = 0; comp < img.nComps; ++comp) {
                    ...
                    img.tiles[tileIdx].tileComps[comp].resLevels =
                        (JPXResLevel *)greallocn(img.tiles[tileIdx].tileComps[comp].resLevels,
                                                 nDecompLevels + 1, sizeof(JPXResLevel));
                    for(r = 0; r <= nDecompLevels; ++r) {
                ...
                if((img.tiles[tileIdx].tileComps[0].quantStyle & 0x1f) == 0x00) {
                    ...
                    img.tiles[tileIdx].tileComps[0].quantSteps =
                        (Guint *)greallocn(img.tiles[tileIdx].tileComps[0].quantSteps,
                                           img.tiles[tileIdx].tileComps[0].nQuantSteps, sizeof(Guint));
            ...
            case 0x5d: // QCC - quantization component
                cover(37);
                if((img.nComps > 256 && !readUWord(&comp)) || (img.nComps <= 256 && !readUByte(&comp)) ||
                   comp >= img.nComps || !readUByte(&img.tiles[tileIdx].tileComps[comp].quantStyle)) {
                    error(errSyntaxError, getPos(), "Error in JPX QCC marker segment");
                    return gFalse;
                }
                if((img.tiles[tileIdx].tileComps[comp].quantStyle & 0x1f) == 0x00) {
                    ...
                    img.tiles[tileIdx].tileComps[comp].quantSteps =
                        (Guint *)greallocn(img.tiles[tileIdx].tileComps[comp].quantSteps,
                                           img.tiles[tileIdx].tileComps[comp].nQuantSteps, sizeof(Guint));
            ...
            case 0x5e: // RGN - region of interest
                ...
                if((img.nComps > 256 && !readUWord(&comp)) ||
                   (img.nComps <= 256 && !readUByte(&comp)) || comp >= img.nComps ||
                   !readUByte(&compInfo[comp].roi.style) || !readUByte(&compInfo[comp].roi.shift)) {
                    error(errSyntaxError, getPos(), "Error in JPX RGN marker segment");
                    return gFalse;
                }
#endif
            ...
            case 0x5f: // POC - progression order change
                ...
                for(i = 0; i < nTileProgs; ++i) {
                    if(!readUByte(&tileProgs[i].startRes) ||
                       ...
                       !readUWord(&tileProgs[i].endLayer) || !readUByte(&tileProgs[i].endRes) ||
    ...
    if(tilePartIdx == 0) {
        ...
        for(comp = 0; comp < img.nComps; ++comp) {
            ...
            tileComp->w = jpxCeilDivPow2(tileComp->x1, reduction) - jpxCeilDivPow2(tileComp->x0, reduction);
            tileComp->h = jpxCeilDivPow2(tileComp->y1, reduction) - jpxCeilDivPow2(tileComp->y0, reduction);
            if(tileComp->w == 0 || tileComp->h == 0) {
                error(errSyntaxError, getPos(), "Invalid tile size or sample separation in JPX stream");
                return gFalse;
            }
            ...
            for(r = 0; r <= tileComp->nDecompLevels; ++r) {
                ...
                if(r == 0) {
                    ...
                    resLevel->empty = resLevel->empty && (resLevel->bx0[0] == resLevel->bx1[0] ||
                ...
                for(pre = 0; pre < 1; ++pre) {
                    ...
                    for(sb = 0; sb < nSBs; ++sb) {
                        ...
                        subband->nXCBs = jpxCeilDivPow2(subband->x1, tileComp->codeBlockW) -
                                         jpxFloorDivPow2(subband->x0, tileComp->codeBlockW);
                        subband->nYCBs = jpxCeilDivPow2(subband->y1, tileComp->codeBlockH) -
                                         jpxFloorDivPow2(subband->y0, tileComp->codeBlockH);
                        ...
                        subband->cbs = (JPXCodeBlock *)gmallocn(subband->nXCBs * subband->nYCBs,
                                                                sizeof(JPXCodeBlock));
                        for(k = 0; k < subband->nXCBs * subband->nYCBs; ++k) {
                        ...
                        if(r == 0) { // (NL)LL
                            ...
                            sbCoeffs =
                                tileComp->data + (resLevel->by1[0] - resLevel->by0[0]) * tileComp->w;
                        } else { // (NL-r+1)HH
                            sbCoeffs = tileComp->data +
                        ...
                        for(cbY = 0; cbY < subband->nYCBs; ++cbY) {
                            for(cbX = 0; cbX < subband->nXCBs; ++cbX) {
                                ...
                                if(r <= tileComp->nDecompLevels - reduction) {
                                    cb->coeffs = sbCoeffs + (cb->y0 - subband->y0) * tileComp->w +
                                                 (cb->x0 - subband->x0);
                                    cb->touched =
                                        (char *)gmalloc(1 << (tileComp->codeBlockW + tileComp->codeBlockH));
                                    cb->len = 0;
                                    ...
                                    memset(cb->touched, 0,
                                           ((size_t)1 << (tileComp->codeBlockW + tileComp->codeBlockH)));
                                } else {
                                    cb->coeffs = NULL;
                                    cb->touched = NULL;
                                    cb->len = 0;
...
GBool JPXStream::readTilePartData(Guint tileIdx, Guint tilePartLen, GBool tilePartToEOC) {
    JPXTile *tile;
    JPXTileComp *tileComp;
    JPXResLevel *resLevel;
    JPXPrecinct *precinct;
                        ...
                        if(cb->included) {
                            ...
                            if(tileComp->codeBlockStyle & 0x04) {
                                if(cb->nCodingPasses > cb->dataLenSize) {
                                    ...
                                    cb->dataLen =
                                        (Guint *)greallocn(cb->dataLen, cb->dataLenSize, sizeof(Guint));
                                }
...
GBool JPXStream::readCodeBlockData(JPXTileComp *tileComp,
    ...
    for(i = 0; i < cb->nCodingPasses; ++i) {
        ...
        switch(cb->nextPass) {
            ...
            case jpxPassSigProp:
                cover(65);
                for(y0 = cb->y0, coeff0 = cb->coeffs, touched0 = cb->touched; y0 < cb->y1;
                    y0 += 4, coeff0 += 4 * tileComp->w, touched0 += 4 << tileComp->codeBlockW) {
                    for(x = cb->x0, coeff1 = coeff0, touched1 = touched0; x < cb->x1; ++x, ++coeff1, ++touched1) {
                        for(y1 = 0, coeff = coeff1, touched = touched1; y1 < 4 && y0 + y1 < cb->y1;
            ...
            case jpxPassCleanup:
                ...
                for(y0 = cb->y0, coeff0 = cb->coeffs, touched0 = cb->touched; y0 < cb->y1;
                    ...
                    for(x = cb->x0, coeff1 = coeff0, touched1 = touched0; x < cb->x1; ++x, ++coeff1, ++touched1) {
                        y1 = 0;
                        if(y0 + 3 < cb->y1 && !(*touched1) && !(touched1[tileComp->cbW]) &&
                           !(touched1[2 * tileComp->cbW]) && !(touched1[3 * tileComp->cbW]) &&
                           (x == cb->x0 || y0 == cb->y0 || !coeff1[-(int)tileComp->w - 1]) &&
                           (y0 == cb->y0 || !coeff1[-(int)tileComp->w]) &&
                           (x == cb->x1 - 1 || y0 == cb->y0 || !coeff1[-(int)tileComp->w + 1]) &&
                           (x == cb->x0 || (!coeff1[-1] && !coeff1[tileComp->w - 1] &&
                                            !coeff1[2 * tileComp->w - 1] && !coeff1[3 * tileComp->w - 1])) &&
                           (x == cb->x1 - 1 || (!coeff1[1] && !coeff1[tileComp->w + 1] &&
                                                !coeff1[2 * tileComp->w + 1] && !coeff1[3 * tileComp->w + 1])) &&
                           ((tileComp->codeBlockStyle & 0x08) ||
                            ((x == cb->x0 || y0 + 4 == cb->y1 || !coeff1[4 * tileComp->w - 1]) &&
                             (y0 + 4 == cb->y1 || !coeff1[4 * tileComp->w]) &&
                             (x == cb->x1 - 1 || y0 + 4 == cb->y1 || !coeff1[4 * tileComp->w + 1])))) {
...
void JPXStream::inverseTransform(JPXTileComp *tileComp) {
    ...
    for(cbY = 0; cbY < subband->nYCBs; ++cbY) {
        for(cbX = 0; cbX < subband->nXCBs; ++cbX) {
            for(y = cb->y0, coeff0 = cb->coeffs, touched0 = cb->touched; y < cb->y1;
...
void JPXStream::inverseTransformLevel(JPXTileComp *tileComp, Guint r, JPXResLevel *resLevel) {
    JPXPrecinct *precinct;
    JPXSubband *subband;
    JPXCodeBlock *cb;
    int *coeff0, *coeff;
...
void JPXStream::inverseTransform1D(JPXTileComp *tileComp, int *data, Guint offset, Guint n) {
    Guint end, i;
...
GBool JPXStream::inverseMultiCompAndDC(JPXTile *tile) {
    ...
    if(tile->multiComp == 1) {
        cover(86);
        if(img.nComps < 3 || tile->tileComps[0].hSep != tile->tileComps[1].hSep ||
...
void JPXStream::skipEPH() {
    ...
    k = bitBufSkip ? 1 : 0;
    if(byteCount >= (Guint)(k + 2) && bufStr->lookChar(k) == 0xff && bufStr->lookChar(k + 1) == 0x92) {

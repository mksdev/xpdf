...
public:
    CachedTileDesc(TileDesc *tile)
        : TileDesc(tile->page, tile->rotate, tile->dpi, tile->tx, tile->ty, tile->tw, tile->th),
          state(cachedTileUnstarted), active(gTrue), bitmap(NULL), freeBitmap(gFalse) {}
    ~CachedTileDesc();
...
void TileCache::rasterizeTile(CachedTileDesc *ct) {
    ...
    out = new SplashOutputDev(state->getColorMode(), 1, state->getReverseVideo(), state->getPaperColor());
    info.tileCache = this;
    info.ct = ct;
    info.out = out;
    out->setStartPageCallback(&TileCache::startPageCbk, &info);
    out->startDoc(state->getDoc()->getXRef());
    state->getDoc()->displayPageSlice(out, ct->page, ct->dpi, ct->dpi, ct->rotate, gFalse, gTrue,
                                      gFalse, ct->tx, ct->ty, ct->tw, ct->th, &abortCheckCbk, ct);
    if(ct->state == cachedTileCanceled) {
        threadPool->lockMutex();
        removeTile(ct);
        threadPool->unlockMutex();

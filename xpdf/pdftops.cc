...
static ArgDesc argDesc[] = {
    ...
    {"-level2gray", argFlag, &level2Gray, 0, "generate Level 2 grayscale PostScript"},
    {"-level2sep", argFlag, &level2Sep, 0, "generate Level 2 separable PostScript"},
    {"-level3", argFlag, &level3, 0, "generate Level 3 PostScript"},
    {"-level3gray", argFlag, &level3Gray, 0, "generate Level 3 grayscale PostScript"},
    ...
    {"-pagecrop", argFlag, &pageCrop, 0, "treat the CropBox as the page size"},
    {"-userunit", argFlag, &userUnit, 0, "honor the UserUnit"},
...
int main(int argc, char *argv[]) {
    ...
    if((level1 ? 1 : 0) + (level1Sep ? 1 : 0) + (level2 ? 1 : 0) + (level2Gray ? 1 : 0) +
           (level2Sep ? 1 : 0) + (level3 ? 1 : 0) + (level3Gray ? 1 : 0) + (level3Sep ? 1 : 0) >
       1) {
    ...
    if(level1 || level1Sep || level2 || level2Gray || level2Sep || level3 || level3Gray || level3Sep) {
    ...
    psOut = new PSOutputDev(psFileName->getCString(), doc, firstPage, lastPage, mode, 0, 0, 0, 0,
                            gFalse, NULL, NULL, userUnit);
    if(!psOut->isOk()) {
        delete psOut;
        exitCode = 2;
        goto err2;
    }
    doc->displayPages(psOut, firstPage, lastPage, 72, 72, 0, !globalParams->getPSUseCropBoxAsPage(),

...
static PSSubstFont psBase14SubstFonts[14] = {{"Courier", 0.600},
...
enum PSFontFileLocation { psFontFileResident, psFontFileEmbedded, psFontFileExternal };
...
PSFontFileInfo::PSFontFileInfo(GString *psNameA, GfxFontType typeA, PSFontFileLocation locA) {
    psName = psNameA;
    type = typeA;
    loc = locA;
...
PSOutCustomColor::PSOutCustomColor(double cA, double mA, double yA, double kA, GString *nameA) {
    c = cA;
    m = mA;
    y = yA;
    k = kA;
...
PSOutputDev::PSOutputDev(char *fileName,
    ...
    init(outputToFile, f, fileTypeA, docA, firstPageA, lastPageA, modeA, imgLLXA, imgLLYA, imgURXA,
         imgURYA, manualCtrlA, honorUserUnitA);
}
...
PSOutputDev::PSOutputDev(PSOutputFunc outputFuncA,
    ...
    init(outputFuncA, outputStreamA, psGeneric, docA, firstPageA, lastPageA, modeA, imgLLXA,
         imgLLYA, imgURXA, imgURYA, manualCtrlA, honorUserUnitA);
}

void PSOutputDev::init(PSOutputFunc outputFuncA,
                       void *outputStreamA,
                       PSFileType fileTypeA,
                       PDFDoc *docA,
                       int firstPageA,
    ...
    for(i = 0; i < 14; ++i) {
        ff = new PSFontFileInfo(new GString(psBase14SubstFonts[i].psName), fontType1, psFontFileResident);
    ...
    for(i = 0; i < names->getLength(); ++i) {
        if(!fontFileInfo->lookup((GString *)names->get(i))) {
            ff = new PSFontFileInfo((GString *)names->get(i), fontType1, psFontFileResident);
            fontFileInfo->add(ff->psName, ff);
        }
    }
    delete names;
    ...
    if(!manualCtrl) {
        ...
        if(firstPage <= catalog->getNumPages()) {
            writeHeader(catalog->getPage(firstPage)->getMediaBox(), catalog->getPage(firstPage)->getCropBox(),
...
void PSOutputDev::writeHeader(PDFRectangle *mediaBox, PDFRectangle *cropBox, int pageRotate) {
    ...
    switch(mode) {
        ...
        case psModeEPS:
            ...
            if(floor(x1) != ceil(x1) || floor(y1) != ceil(y1) || floor(x2) != ceil(x2) ||
               floor(y2) != ceil(y2)) {
        ...
        case psModeForm:
            ...
            writePSFmt("/BBox [{0:d} {1:d} {2:d} {3:d}] def\n", (int)floor(mediaBox->x1),
                       (int)floor(mediaBox->y1), (int)ceil(mediaBox->x2), (int)ceil(mediaBox->y2));
...
void PSOutputDev::writeXpdfProcset() {
    ...
    for(p = prolog; *p; ++p) {
        ...
        } else if((level == psLevel1 && lev1 && nonSep) || (level == psLevel1Sep && lev1 && sep) ||
                  (level == psLevel2 && lev2 && nonSep) || (level == psLevel2Gray && lev2 && gray) ||
                  (level == psLevel2Sep && lev2 && sep) || (level == psLevel3 && lev3 && nonSep) ||
                  (level == psLevel3Gray && lev3 && gray) || (level == psLevel3Sep && lev3 && sep)) {
            writePSFmt("{0:s}\n", *p);
        }
    }
...
GBool PSOutputDev::checkIfPageNeedsToBeRasterized(int pg) {
    ...
    if(globalParams->getPSAlwaysRasterize()) {
        ...
        doc->getCatalog()->getPage(pg)->display(scan, 72, 72, 0, gTrue, gTrue, gTrue);
...
void PSOutputDev::setupFont(GfxFont *font, Dict *parentResDict) {
    ...
    if(font->getType() == fontType3) {
            ...
            switch(fontLoc->locType) {
                case gfxFontLocEmbedded:
                    switch(fontLoc->fontType) {
                        ...
                        case fontCIDType2OT:
                            ...
                            fi->ff = setupEmbeddedCIDTrueTypeFont(font, &fontLoc->embFontID, gTrue);
                            break;
                ...
                case gfxFontLocExternal:
                    ...
                    switch(fontLoc->fontType) {
                        ...
                        case fontTrueType:
                        case fontTrueTypeOT:
                            fi->ff = setupExternalTrueTypeFont(font, fontLoc->path, fontLoc->fontNum);
                            break;
                        case fontCIDType2:
                        case fontCIDType2OT:
                            //~ should check to see if font actually uses vertical mode
                            fi->ff = setupExternalCIDTrueTypeFont(font, fontLoc->path, fontLoc->fontNum, gTrue);
                            break;
                        case fontCIDType0COT:
                ...
                case gfxFontLocResident:
                    if(!(fi->ff = (PSFontFileInfo *)fontFileInfo->lookup(fontLoc->path))) {
                        ...
                        fi->ff = new PSFontFileInfo(fontLoc->path->copy(), fontLoc->fontType, psFontFileResident);
        ...
        if(!fi->ff) {
            if(font->isCIDFont()) {
                error(errSyntaxError, -1, "Couldn't find a font for '{0:s}' ('{1:s}' character collection)",
        ...
        if(fontLoc->locType == gfxFontLocResident && fontLoc->substIdx >= 0) {
            ...
            for(code = 0; code < 256; ++code) {
                if((charName = ((Gfx8BitFont *)font)->getCharName(code)) && charName[0] == 'm' &&
                   charName[1] == '\0') {
                    break;
                }
            }
    ...
    if(font->isCIDFont()) {
        if(level >= psLevel3) {
            writePSFmt("/F{0:d}_{1:d} /{2:t} {3:d} pdfMakeFont16L3\n", font->getID()->num,
                       font->getID()->gen, fi->ff->psName, font->getWMode());
        } else {
...
PSFontFileInfo *PSOutputDev::setupEmbeddedType1Font(GfxFont *font, Ref *id) {
    ...
    if(!font->getEmbeddedFontName()) {
        rename = gTrue;
    } else if((ff = (PSFontFileInfo *)fontFileInfo->lookup(font->getEmbeddedFontName()))) {
...
PSFontFileInfo *PSOutputDev::setupEmbeddedTrueTypeFont(GfxFont *font, Ref *id) {
    ...
    fontFileInfo->startIter(&iter);
    while(fontFileInfo->getNext(&iter, &psName, (void **)&ff)) {
        if(ff->loc == psFontFileEmbedded && ff->type == font->getType() &&
           ff->embFontID.num == id->num && ff->embFontID.gen == id->gen &&
           ff->codeToGIDLen == 256 && !memcmp(ff->codeToGID, codeToGID, 256 * sizeof(int))) {
            fontFileInfo->killIter(&iter);
            gfree(codeToGID);
            delete ffTT;
            gfree(fontBuf);
    ...
    ffTT->convertToType42(psName->getCString(),
                          ((Gfx8BitFont *)font)->getHasEncoding() ? ((Gfx8BitFont *)font)->getEncoding() :
                                                                    (char **)NULL,
...
PSFontFileInfo *PSOutputDev::setupExternalTrueTypeFont(GfxFont *font, GString *fileName, int fontNum) {
    GString *psName;
    PSFontFileInfo *ff;
    FoFiTrueType *ffTT;
    int *codeToGID;
    ...
    fontFileInfo->startIter(&iter);
    while(fontFileInfo->getNext(&iter, &psName, (void **)&ff)) {
        if(ff->loc == psFontFileExternal && ff->type == font->getType() && !ff->extFileName->cmp(fileName) &&
           ff->codeToGIDLen == 256 && !memcmp(ff->codeToGID, codeToGID, 256 * sizeof(int))) {
            fontFileInfo->killIter(&iter);
            gfree(codeToGID);
            delete ffTT;
    ...
    if((fontBuf = font->readEmbFontFile(xref, &fontLen))) {
        if((ffT1C = FoFiType1C::make(fontBuf, fontLen))) {
            if(globalParams->getPSLevel() >= psLevel3) {
                ...
                ffT1C->convertToCIDType0(psName->getCString(), ((GfxCIDFont *)font)->getCIDToGID(),
                                         ((GfxCIDFont *)font)->getCIDToGIDLen(), outputFunc, outputStream);
            } else {
                // otherwise: use a non-CID composite font
                ffT1C->convertToType0(psName->getCString(), ((GfxCIDFont *)font)->getCIDToGID(),
                                      ((GfxCIDFont *)font)->getCIDToGIDLen(), outputFunc, outputStream);
            }
            delete ffT1C;
        }
...
PSFontFileInfo *PSOutputDev::setupEmbeddedCIDTrueTypeFont(GfxFont *font, Ref *id, GBool needVerticalMetrics) {
    GString *psName;
    PSFontFileInfo *ff;
    char *fontBuf;
    int fontLen;
    ...
    fontFileInfo->startIter(&iter);
    while(fontFileInfo->getNext(&iter, &psName, (void **)&ff)) {
        if(ff->loc == psFontFileEmbedded && ff->type == font->getType() && ff->embFontID.num == id->num &&
           ff->embFontID.gen == id->gen && ff->codeToGIDLen == codeToGIDLen &&
           ((!ff->codeToGID && !codeToGID) ||
            (ff->codeToGID && codeToGID && !memcmp(ff->codeToGID, codeToGID, codeToGIDLen * sizeof(int))))) {
            fontFileInfo->killIter(&iter);
            return ff;
        }
    }
    ...
    if((fontBuf = font->readEmbFontFile(xref, &fontLen))) {
        if((ffTT = FoFiTrueType::make(fontBuf, fontLen, 0))) {
            if(globalParams->getPSLevel() >= psLevel3) {
                // Level 3: use a CID font
                ffTT->convertToCIDType2(psName->getCString(), codeToGID, codeToGIDLen,
                                        needVerticalMetrics, outputFunc, outputStream);
            } else {
                // otherwise: use a non-CID composite font
                ffTT->convertToType0(psName->getCString(), codeToGID, codeToGIDLen,
                                     needVerticalMetrics, outputFunc, outputStream);
            }
            delete ffTT;
        }
...
PSFontFileInfo *
PSOutputDev::setupExternalCIDTrueTypeFont(GfxFont *font, GString *fileName, int fontNum, GBool needVerticalMetrics) {
    GString *psName;
    PSFontFileInfo *ff;
    FoFiTrueType *ffTT;
    int *codeToGID;
    ...
    if(!(ctu = ((GfxCIDFont *)font)->getToUnicode())) {
        error(errSyntaxError, -1, "Couldn't find a mapping to Unicode for font '{0:s}'",
              font->getName() ? font->getName()->getCString() : "(unnamed)");
        delete ffTT;
        return NULL;
    }
    ...
    while(fontFileInfo->getNext(&iter, &psName, (void **)&ff)) {
        if(ff->loc == psFontFileExternal && ff->type == font->getType() &&
           !ff->extFileName->cmp(fileName) && ff->codeToGIDLen == codeToGIDLen && ff->codeToGID &&
    ...
    if(ffTT->getEmbeddingRights() < 1) {
        error(errSyntaxError, -1, "TrueType font '{0:s}' does not allow embedding",
              font->getName() ? font->getName()->getCString() : "(unnamed)");
        gfree(codeToGID);
        delete ffTT;
        return NULL;
    ...
    if(globalParams->getPSLevel() >= psLevel3) {
        // Level 3: use a CID font
        ffTT->convertToCIDType2(psName->getCString(), codeToGID, codeToGIDLen, needVerticalMetrics,
                                outputFunc, outputStream);
    } else {
        // otherwise: use a non-CID composite font
        ffTT->convertToType0(psName->getCString(), codeToGID, codeToGIDLen, needVerticalMetrics,
                             outputFunc, outputStream);
    }
    delete ffTT;
    ...
    if((fontBuf = font->readEmbFontFile(xref, &fontLen))) {
        if((ffTT = FoFiTrueType::make(fontBuf, fontLen, 0, gTrue))) {
            if(ffTT->isOpenTypeCFF()) {
                if(globalParams->getPSLevel() >= psLevel3) {
                    // Level 3: use a CID font
                    ffTT->convertToCIDType0(psName->getCString(), ((GfxCIDFont *)font)->getCIDToGID(),
                                            ((GfxCIDFont *)font)->getCIDToGIDLen(), outputFunc, outputStream);
                } else {
                    // otherwise: use a non-CID composite font
                    ffTT->convertToType0(psName->getCString(), ((GfxCIDFont *)font)->getCIDToGID(),
                                         ((GfxCIDFont *)font)->getCIDToGIDLen(), outputFunc, outputStream);
                }
            }
            delete ffTT;
...
// mapping.
PSFontFileInfo *PSOutputDev::setupExternalOpenTypeCFFFont(GfxFont *font, GString *fileName) {
    GString *psName;
    PSFontFileInfo *ff;
    FoFiTrueType *ffTT;
    GHashIter *iter;
    ...
    if(globalParams->getPSLevel() >= psLevel3) {
        // Level 3: use a CID font
        ffTT->convertToCIDType0(psName->getCString(), codeToGID, codeToGIDLen, outputFunc, outputStream);
    } else {
        // otherwise: use a non-CID composite font
        ffTT->convertToType0(psName->getCString(), codeToGID, codeToGIDLen, outputFunc, outputStream);
    }
    delete ffTT;
...
PSFontFileInfo *PSOutputDev::setupType3Font(GfxFont *font, Dict *parentResDict) {
    ...
    // generate name
    psName = GString::format("T3_{0:d}_{1:d}", font->getID()->num, font->getID()->gen);
    ...
    if((charProcs = ((Gfx8BitFont *)font)->getCharProcs())) {
        ...
        for(i = 0; i < charProcs->getLength(); ++i) {
            ...
            if(t3String) {
                if(t3Cacheable) {
                    buf = GString::format("{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g} "
                                          "setcachedevice\n",
...
GString *PSOutputDev::fixType1Font(GString *font, int length1, int length2) {
    ...
    if(pfb) {
        ...
        if(!splitType1PFA(fontData, fontSize, length1, length2, out, binSection)) {
            delete out;
            delete binSection;
...
// section.
GBool PSOutputDev::splitType1PFA(Guchar *font, int fontSize, int length1, int length2, GString *textSection, GString *binSection) {
    ...
    if(length2 > 0 && length2 < INT_MAX - 4 && binStart <= fontSize - length2 - 4 &&
       !memcmp(font + binStart + length2, "0000", 4)) {
        ...
        if(!memcmp(font + fontSize - 256, "0000", 4) || !memcmp(font + fontSize - 255, "0000", 4) ||
           !memcmp(font + fontSize - 254, "0000", 4) || !memcmp(font + fontSize - 253, "0000", 4) ||
           !memcmp(font + fontSize - 252, "0000", 4) || !memcmp(font + fontSize - 251, "0000", 4)) {
            i = fontSize - 252;
            lastSpace = -1;
            while(i >= binStart) {
...
// section.
GBool PSOutputDev::splitType1PFB(Guchar *font, int fontSize, GString *textSection, GString *binSection) {
    Guchar *p;
    ...
    if(n >= 18 && !memcmp(textSection->getCString() + n - 18, "currentfile eexec", 17)) {
        // remove the trailing whitespace
        textSection->del(n - 1, 1);
    } else if(n >= 17 && !memcmp(textSection->getCString() + n - 17, "currentfile eexec", 17)) {
...
void PSOutputDev::setupImage(Ref id, Stream *str, GBool mask, Array *colorKeyMask) {
    ...
    writePSFmt("{0:d} array dup /{1:s}Data_{2:d}_{3:d} exch def\n", size,
               (mask || colorKeyMask) ? "Mask" : "Im", id.num, id.gen);
    str->close();
...
GBool PSOutputDev::checkPageSlice(Page *page,
                                  double hDPI,
                                  double vDPI,
                                  int rotateA,
                                  GBool useMediaBox,
    ...
    // start the PS page
    page->makeBox(userUnit * dpi, userUnit * dpi, rotateA, useMediaBox, gFalse, sliceX, sliceY,
    ...
    if(mono) {
        paperColor[0] = 0xff;
        splashOut = new SplashOutputDev(splashModeMono8, 1, gFalse, paperColor, gFalse,
                                        globalParams->getAntialiasPrinting());
#if SPLASH_CMYK
    } else if(level == psLevel1Sep) {
        paperColor[0] = paperColor[1] = paperColor[2] = paperColor[3] = 0;
        splashOut = new SplashOutputDev(splashModeCMYK8, 1, gFalse, paperColor, gFalse,
                                        globalParams->getAntialiasPrinting());
#endif
    } else {
        paperColor[0] = paperColor[1] = paperColor[2] = 0xff;
        splashOut = new SplashOutputDev(splashModeRGB8, 1, gFalse, paperColor, gFalse,
                                        globalParams->getAntialiasPrinting());
    }
    ...
    for(stripeY = sliceY; stripeY < sliceH; stripeY += stripeH) {
        ...
        // rasterize a stripe
        page->makeBox(hDPI2, vDPI2, 0, useMediaBox, gFalse, sliceX, stripeY, sliceW, stripeH, &box, &crop);
        m0 = box.x2 - box.x1;
        m1 = 0;
        m2 = 0;
        m3 = box.y2 - box.y1;
        m4 = box.x1;
        m5 = box.y1;
        page->displaySlice(splashOut, hDPI2, vDPI2, (360 - page->getRotate()) % 360, useMediaBox, crop,
                           sliceX, stripeY, sliceW, stripeH, printing, abortCheckCbk, abortCheckCbkData);
        ...
        case psModePS:
            ...
            if(xScale0 > 0 && yScale0 > 0) {
                ...
                yScale = yScale0 * userUnit;
                       ...
                       (width * userUnit > imgWidth2 || height * userUnit > imgHeight2)) ||
            ...
            if(clipLLX0 < clipURX0 && clipLLY0 < clipURY0) {
                writePSFmt("{0:.6g} {1:.6g} {2:.6g} {3:.6g} re W\n", clipLLX0, clipLLY0,
                           clipURX0 - clipLLX0, clipURY0 - clipLLY0);
            } else {
    ...
    switch(level) {
        ...
        case psLevel3:
            if(state->getFillColorSpace()->getMode() != csPattern) {
                dumpColorSpaceL2(state, state->getFillColorSpace(), gTrue, gFalse, gFalse);
                writePS(" cs\n");
                noStateChanges = gFalse;
            }
            break;
    ...
    switch(level) {
        ...
        case psLevel3:
            if(state->getStrokeColorSpace()->getMode() != csPattern) {
                dumpColorSpaceL2(state, state->getStrokeColorSpace(), gTrue, gFalse, gFalse);
                writePS(" CS\n");
                noStateChanges = gFalse;
            }
            break;
        ...
        case psLevel3Sep:
            if(state->getFillColorSpace()->getMode() == csSeparation) {
                ...
                writePSFmt("{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} ({5:t}) ck\n",
                           ...
                           colToDbl(cmyk.m), colToDbl(cmyk.y), colToDbl(cmyk.k), sepCS->getName());
...
void PSOutputDev::addCustomColor(GfxState *state, GfxSeparationColorSpace *sepCS) {
    ...
    color.c[0] = gfxColorComp1;
    sepCS->getCMYK(&color, &cmyk, state->getRenderingIntent());
    cc = new PSOutCustomColor(colToDbl(cmyk.c), colToDbl(cmyk.m), colToDbl(cmyk.y),
...
void PSOutputDev::addCustomColors(GfxState *state, GfxDeviceNColorSpace *devnCS) {
    ...
    for(i = 0; i < devnCS->getNComps(); ++i) {
        ...
        color.c[i] = 0;
        cc = new PSOutCustomColor(colToDbl(cmyk.c), colToDbl(cmyk.m), colToDbl(cmyk.y),
                                  colToDbl(cmyk.k), devnCS->getColorantName(i)->copy());
        cc->next = customColors;
        customColors = cc;
    }
}
...
void PSOutputDev::updateFont(GfxState *state) {
    if(state->getFont()) {
        if(state->getFont()->getTag() && !state->getFont()->getTag()->cmp("xpdf_default_font")) {
            writePSFmt("/xpdf_default_font {0:.6g} Tf\n",
                       fabs(state->getFontSize()) < 0.0001 ? 0.0001 : state->getFontSize());
        } else {
...
void PSOutputDev::tilingPatternFill(GfxState *state,
                                    Gfx *gfx,
                                    Object *strRef,
                                    int paintType,
                                    int tilingType,
    ...
    if(level <= psLevel1Sep) {
        tilingPatternFillL1(state, gfx, strRef, paintType, tilingType, resDict, mat, bbox, x0, y0,
                            x1, y1, xStep, yStep);
    } else {
        tilingPatternFillL2(state, gfx, strRef, paintType, tilingType, resDict, mat, bbox, x0, y0,
                            x1, y1, xStep, yStep);
    }
}

void PSOutputDev::tilingPatternFillL1(GfxState *state,
                                      Gfx *gfx,
                                      Object *strRef,
                                      int paintType,
                                      int tilingType,
    ...
    writePSFmt("{0:d} 1 {1:d} {{ {2:.6g} exch {3:.6g} mul m {4:d} 1 {5:d} {{ pop (x) show }} for "
               "}} for\n",
...
void PSOutputDev::tilingPatternFillL2(GfxState *state,
                                      Gfx *gfx,
                                      Object *strRef,
                                      int paintType,
                                      int tilingType,
    ...
    writePSFmt("{0:.6g} {1:.6g} {2:.6g} {3:.6g} rectfill\n", x0 * xStep + bbox[0],
               y0 * yStep + bbox[1], (x1 - x0) * xStep + bbox[2], (y1 - y0) * yStep + bbox[3]);

    writePS("grestore\n");
    noStateChanges = gFalse;
}

GBool PSOutputDev::functionShadedFill(GfxState *state, GfxFunctionShading *shading) {
    double x0, y0, x1, y1;
    double *mat;
    int i;
...
GBool PSOutputDev::radialShadedFill(GfxState *state, GfxRadialShading *shading) {
    double xMin, yMin, xMax, yMax;
    ...
    if(enclosed) {
        ...
        if((shading->getExtend0() && r0 > r1) || (shading->getExtend1() && r1 >= r0)) {
            ...
            writePSFmt("{0:.6g} {1:.6g} m {2:.6g} {3:.6g} l {4:.6g} {5:.6g} l {6:.6g} {7:.6g} l h "
                       "f*\n",
    ...
    if(n == 1 && path->getSubpath(0)->getNumPoints() == 5) {
        ...
        if(x4 == x0 && y4 == y0) {
            ...
            if(x0 == x1 && x2 == x3 && y0 == y3 && y1 == y2) {
                ...
                writePSFmt("{0:.6g} {1:.6g} {2:.6g} {3:.6g} re\n", x0 < x1 ? x0 : x1,
                           y0 < y2 ? y0 : y2, fabs(x1 - x0), fabs(y2 - y0));
                return;
            }
        }
    }
        ...
        while(j < m) {
            if(subpath->getCurve(j)) {
                writePSFmt("{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g} c\n", subpath->getX(j),
                           subpath->getY(j), subpath->getX(j + 1), subpath->getY(j + 1),
...
void PSOutputDev::drawString(GfxState *state, GString *s) {
    ...
    while(len > 0) {
        n = font->getNextChar(p, len, &code, u, (int)(sizeof(u) / sizeof(Unicode)), &uLen, &dx, &dy,
                              &originX, &originY);
...
void PSOutputDev::drawImageMask(GfxState *state, Object *ref, Stream *str, int width, int height, GBool invert, GBool inlineImg, GBool interpolate) {
    int len;
...
void PSOutputDev::drawImage(GfxState *state,
    ...
    switch(level) {
        case psLevel1:
            doImageL1(ref, state, colorMap, gFalse, inlineImg, str, width, height, len);
            break;
        case psLevel1Sep:
        ...
        case psLevel2Gray:
        case psLevel2Sep:
            doImageL2(ref, state, colorMap, gFalse, inlineImg, str, width, height, len, maskColors,
                      NULL, 0, 0, gFalse);
            break;
        case psLevel3:
        case psLevel3Gray:
        case psLevel3Sep:
            doImageL3(ref, state, colorMap, gFalse, inlineImg, str, width, height, len, maskColors,
                      NULL, 0, 0, gFalse);
            break;
    }
    t3Cacheable = gFalse;
...
void PSOutputDev::drawMaskedImage(GfxState *state,
                                  Object *ref,
                                  Stream *str,
                                  int width,
                                  int height,
    ...
    switch(level) {
        ...
        case psLevel2Gray:
        case psLevel2Sep:
            doImageL2(ref, state, colorMap, gFalse, gFalse, str, width, height, len, NULL, maskStr,
                      maskWidth, maskHeight, maskInvert);
            break;
        case psLevel3:
        case psLevel3Gray:
        case psLevel3Sep:
            doImageL3(ref, state, colorMap, gFalse, gFalse, str, width, height, len, NULL, maskStr,
                      maskWidth, maskHeight, maskInvert);
            break;
    }
    t3Cacheable = gFalse;
...
void PSOutputDev::doImageL1(Object *ref,
                            GfxState *state,
                            GfxImageColorMap *colorMap,
                            GBool invert,
                            GBool inlineImg,
    ...
    if((inType3Char || preload) && !colorMap) {
        writePSFmt("{0:d} {1:d} {2:s} [{3:d} 0 0 {4:d} 0 {5:d}] pdfImM1a\n", width, height,
                   invert ? "true" : "false", width, -height, height);
    } else if(colorMap) {
...
void PSOutputDev::doImageL1Sep(GfxState *state,
                               GfxImageColorMap *colorMap,
                               GBool invert,
                               GBool inlineImg,
                               Stream *str,
...
void PSOutputDev::doImageL2(Object *ref,
                            GfxState *state,
                            GfxImageColorMap *colorMap,
                            GBool invert,
                            GBool inlineImg,
    ...
    writePSFmt("  /Height {0:d}\n", height);
    writePSFmt("  /ImageMatrix [{0:d} 0 0 {1:d} 0 {2:d}]\n", width, -height, height);
    if(colorMap && (colorMap->getColorSpace()->getMode() == csDeviceN || level == psLevel2Gray ||
                    level == psLevel3Gray)) {
        writePS("  /BitsPerComponent 8\n");
    } else {
...
void PSOutputDev::convertColorKeyMaskToClipRects(GfxImageColorMap *colorMap, Stream *str, int width, int height, int *maskColors) {
    ...
    for(i = 0; i < rectsOutLen; ++i) {
        writePSFmt("{0:d} {1:d} {2:d} {3:d} pr\n", rectsOut[i].x0, rectsOut[i].y0,
                   rectsOut[i].x1 - rectsOut[i].x0, rectsOut[i].y1 - rectsOut[i].y0);
    }
...
// sequence of clip rectangles.
void PSOutputDev::convertExplicitMaskToClipRects(Stream *maskStr, int maskWidth, int maskHeight, GBool maskInvert) {
    ImageStream *imgStr;
    Guchar *line;
    PSOutImgClipRect *rects0, *rects1, *rectsTmp, *rectsOut;
    int rects0Len, rects1Len, rectsSize, rectsOutLen, rectsOutSize;
...
void PSOutputDev::doImageL3(Object *ref,
                            GfxState *state,
                            GfxImageColorMap *colorMap,
                            GBool invert,
                            GBool inlineImg,
...
void PSOutputDev::dumpColorSpaceL2(GfxState *state, GfxColorSpace *colorSpace, GBool genXform, GBool updateColors, GBool map01) {
...
void PSOutputDev::dumpDeviceGrayColorSpace(GfxDeviceGrayColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    writePS("/DeviceGray");
    if(genXform) {
        writePS(" {}");
    }
...
void PSOutputDev::dumpCalGrayColorSpace(GfxCalGrayColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    writePS("[/CIEBasedA <<\n");
...
void PSOutputDev::dumpDeviceRGBColorSpace(GfxDeviceRGBColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    writePS("/DeviceRGB");
    if(genXform) {
        writePS(" {}");
    }
...
void PSOutputDev::dumpCalRGBColorSpace(GfxCalRGBColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    writePS("[/CIEBasedABC <<\n");
    writePSFmt(" /DecodeABC [{{{0:.4g} exp}} bind {{{1:.4g} exp}} bind {{{2:.4g} exp}} bind]\n",
               cs->getGammaR(), cs->getGammaG(), cs->getGammaB());
    writePSFmt(" /MatrixABC [{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} {6:.4g} {7:.4g} "
               "{8:.4g}]\n",
               cs->getMatrix()[0], cs->getMatrix()[1], cs->getMatrix()[2], cs->getMatrix()[3],
               cs->getMatrix()[4], cs->getMatrix()[5], cs->getMatrix()[6], cs->getMatrix()[7],
               cs->getMatrix()[8]);
    writePSFmt(" /WhitePoint [{0:.4g} {1:.4g} {2:.4g}]\n", cs->getWhiteX(), cs->getWhiteY(), cs->getWhiteZ());
    writePSFmt(" /BlackPoint [{0:.4g} {1:.4g} {2:.4g}]\n", cs->getBlackX(), cs->getBlackY(), cs->getBlackZ());
    writePS(">>]");
...
void PSOutputDev::dumpDeviceCMYKColorSpace(GfxDeviceCMYKColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    writePS("/DeviceCMYK");
    if(genXform) {
        writePS(" {}");
    }
...
void PSOutputDev::dumpLabColorSpace(GfxLabColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    writePS("[/CIEBasedABC <<\n");
    if(map01) {
        writePS(" /RangeABC [0 1 0 1 0 1]\n");
        writePSFmt(" /DecodeABC [{{100 mul 16 add 116 div}} bind {{{0:.4g} mul {1:.4g} add}} bind "
                   "{{{2:.4g} mul {3:.4g} add}} bind]\n",
                   (cs->getAMax() - cs->getAMin()) / 500.0, cs->getAMin() / 500.0,
                   (cs->getBMax() - cs->getBMin()) / 200.0, cs->getBMin() / 200.0);
    } else {
        writePSFmt(" /RangeABC [0 100 {0:.4g} {1:.4g} {2:.4g} {3:.4g}]\n", cs->getAMin(),
                   cs->getAMax(), cs->getBMin(), cs->getBMax());
...
void PSOutputDev::dumpICCBasedColorSpace(GfxState *state, GfxICCBasedColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
...
void PSOutputDev::dumpIndexedColorSpace(GfxState *state, GfxIndexedColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
...
void PSOutputDev::dumpDeviceNColorSpaceL2(GfxState *state, GfxDeviceNColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
...
void PSOutputDev::dumpDeviceNColorSpaceL3(GfxState *state, GfxDeviceNColorSpace *cs, GBool genXform, GBool updateColors, GBool map01) {
    GString *tint;
    int i;
...
void PSOutputDev::opiBegin13(GfxState *state, Dict *dict) {
    ...
    if(obj1.isArray() && obj1.arrayGetLength() == 8) {
        ...
        writePSFmt("%ALDImagePosition: {0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} {6:.4g} "
                   "{7:.4g}\n",
    ...
    switch(func->getType()) {
        ...
        case 0: // sampled
            ...
            for(i = 0; i < n; ++i) {
                ...
                writePSFmt("dup {0:.6g} lt {{ pop {1:.6g} }} {{ dup {2:.6g} gt {{ pop {3:.6g} }} "
                           "if }} ifelse\n",
                           func0->getRangeMin(i), func0->getRangeMin(i), func0->getRangeMax(i),
                           func0->getRangeMax(i));
        ...
        case 2: // exponential
            ...
            writePSFmt("{{ dup {0:.6g} lt {{ pop {1:.6g} }} {{ dup {2:.6g} gt {{ pop {3:.6g} }} if "
                       "}} ifelse\n",
                       func2->getDomainMin(0), func2->getDomainMin(0), func2->getDomainMax(0),
                       func2->getDomainMax(0));
            // x
            for(i = 0; i < n; ++i) {
                // x y(0) .. y(i-1)
                writePSFmt("{0:d} index {1:.6g} exp {2:.6g} mul {3:.6g} add\n", i, func2->getE(),
                           func2->getC1()[i] - func2->getC0()[i], func2->getC0()[i]);
                if(func2->getHasRange()) {
                    writePSFmt("dup {0:.6g} lt {{ pop {1:.6g} }} {{ dup {2:.6g} gt {{ pop {3:.6g} "
                               "}} if }} ifelse\n",
                               func2->getRangeMin(i), func2->getRangeMin(i), func2->getRangeMax(i),
                               func2->getRangeMax(i));
                }
            }
            // x y(0) .. y(n-1)
        ...
        case 3: // stitching
            ...
            writePSFmt("{{ dup {0:.6g} lt {{ pop {1:.6g} }} {{ dup {2:.6g} gt {{ pop {3:.6g} }} if "
                       "}} ifelse\n",
                       func3->getDomainMin(0), func3->getDomainMin(0), func3->getDomainMax(0),
                       func3->getDomainMax(0));
            for(i = 0; i < func3->getNumFuncs() - 1; ++i) {
                writePSFmt("dup {0:.6g} lt {{ {1:.6g} sub {2:.6g} mul {3:.6g} add "
                           "xpdfFunc{4:d}_{5:d} }} {{\n",
                           func3->getBounds()[i + 1], func3->getBounds()[i], func3->getScale()[i],
...
void PSOutputDev::writePSName(const char *s) {
    ...
    p = s;
    while((c = *p++)) {
        if(c <= (char)0x20 || c >= (char)0x7f || c == '(' || c == ')' || c == '<' || c == '>' ||
           c == '[' || c == ']' || c == '{' || c == '}' || c == '/' || c == '%') {

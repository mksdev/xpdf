...
public:
    ...
    int getStreamIndex() {
        return lexer->getStreamIndex();
    }

    // Get stream.
    Stream *getStream() {
        return lexer->getStream();
    }
...
private:
    ...
    Stream *
    makeStream(Object *dict, Guchar *fileKey, CryptAlgorithm encAlgorithm, int keyLength, int objNum, int objGen, int recursion);
    void shift();
};

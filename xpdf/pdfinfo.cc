...
static void printInfoString(Object *infoDict,
                            const char *infoKey,
                            ZxDoc *xmp,
                            const char *xmpKey1,
                            const char *xmpKey2,
...
int main(int argc, char *argv[]) {
    ...
    printInfoString(&info, "CreationDate", xmp, "xap:CreateDate", "xmp:CreateDate",
                    "CreationDate:   ", !rawDates, uMap);
    printInfoString(&info, "ModDate", xmp, "xap:ModifyDate", "xmp:ModifyDate",
                    "ModDate:        ", !rawDates, uMap);
    info.free();
    if(xmp) {
        delete xmp;
    }
    ...
    if(doc->isEncrypted()) {
        printf("yes (print:%s copy:%s change:%s addNotes:%s)\n",
               doc->okToPrint(gTrue) ? "yes" : "no", doc->okToCopy(gTrue) ? "yes" : "no",
               doc->okToChange(gTrue) ? "yes" : "no", doc->okToAddNotes(gTrue) ? "yes" : "no");
    } else {
        printf("no\n");
    }
...
static GString *parseXMPDate(GString *s) {
    ...
    if(p[0] == '-' && isdigit(p[1]) && isdigit(p[2])) {
        ...
        if(p[0] == '-' && isdigit(p[1]) && isdigit(p[2])) {
            ...
            day = atoi(buf);
            p += 3;
            if(p[0] == 'T' && isdigit(p[1]) && isdigit(p[2]) && p[3] == ':' && isdigit(p[4]) &&
               isdigit(p[5])) {
                buf[0] = p[1];
                buf[1] = p[2];
                buf[2] = '\0';
                ...
                if((p[0] == '+' || p[0] == '-') && isdigit(p[1]) && isdigit(p[2]) && p[3] == ':' &&

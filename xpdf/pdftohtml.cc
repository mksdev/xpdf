...
int main(int argc, char *argv[]) {
    ...
    if(!createDir(htmlDir, 0755)) {
        error(errIO, -1, "Couldn't create HTML output directory '{0:s}'", htmlDir);
        exitCode = 2;
        goto err1;
    }
    ...
    for(pg = firstPage; pg <= lastPage; ++pg) {
        ...
        err = htmlGen->convertPage(pg, pngURL->getCString(), htmlDir, &writeToFile, htmlFile,
                                   &writeToFile, pngFile);
        delete pngURL;
        fclose(htmlFile);
        fclose(pngFile);

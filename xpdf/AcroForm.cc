...
AcroFormField *AcroFormField::load(AcroForm *acroFormA, Object *fieldRefA) {
    ...
    field = new AcroFormField(acroFormA, fieldRefA, &fieldObjA, typeA, nameA, flagsA, typeFromParentA);
    fieldObjA.free();
    return field;
    ...
    if(fieldLookup("DA", &daObj)->isString()) {
        ...
        for(i = 1; i < daToks->getLength(); ++i) {
            ...
            if(!((GString *)daToks->get(i))->cmp("g")) {
                *red = *green = *blue = atof(((GString *)daToks->get(i - 1))->getCString());
                break;
...
void AcroFormField::drawAnnot(int pageNum, Gfx *gfx, GBool printing, Object *annotRef, Object *annotObj) {
    ...
    obj1.free();
    if((annotFlags & annotFlagHidden) || (printing && !(annotFlags & annotFlagPrint)) ||
...
void AcroFormField::drawNewAppearance(Gfx *gfx, Dict *annot, double xMin, double yMin, double xMax, double yMax) {
    ...
    if(ftObj.isName("Btn")) {
        ...
        if(flags & acroFormFlagRadio) {
            ...
            if(fieldLookup("V", &obj1)->isName(appearanceState->getCString())) {
                if(caption) {
                    drawText(caption, da, fontDict, gFalse, 0, acroFormQuadCenter, gFalse, gTrue,
                             rot, xMin, yMin, xMax, yMax, borderWidth, appearBuf);
                } else {
                    if(mkDict) {
            ...
            if(caption) {
                drawText(caption, da, fontDict, gFalse, 0, acroFormQuadCenter, gFalse, gFalse, rot,
                         xMin, yMin, xMax, yMax, borderWidth, appearBuf);
            }
            // checkbox
        } else {
            fieldLookup("V", &obj1);
            if(obj1.isName() && !(obj1.isName("Off") || obj1.isName("No") || obj1.isName(""))) {
                if(!caption) {
                    caption = new GString("3"); // ZapfDingbats checkmark
                }
                drawText(caption, da, fontDict, gFalse, 0, acroFormQuadCenter, gFalse, gTrue, rot,
                         xMin, yMin, xMax, yMax, borderWidth, appearBuf);
            }
            obj1.free();
        }
        ...
        if(obj1.isString()) {
            ...
            drawText(obj1.getString(), da, fontDict, flags & acroFormFlagMultiline, comb, quadding,
                     gTrue, gFalse, rot, xMin, yMin, xMax, yMax, borderWidth, appearBuf);
        }
        obj1.free();
        ...
        if(flags & acroFormFlagCombo) {
            if(fieldLookup("V", &obj1)->isString()) {
                ...
                obj2.free();
                drawText(val, da, fontDict, gFalse, 0, quadding, gTrue, gFalse, rot, xMin, yMin,
                         xMax, yMax, borderWidth, appearBuf);
                delete val;
            ...
            if(fieldObj.dictLookup("Opt", &obj1)->isArray()) {
                ...
                for(i = 0; i < nOptions; ++i) {
                    ...
                    if(obj2.isString()) {
                        ...
                        for(j = 0; j < obj2.arrayGetLength(); ++j) {
                            if(obj2.arrayGet(j, &obj3)->isString() && !obj3.getString()->cmp(text[i])) {
                                selection[i] = gTrue;
                            }
                            obj3.free();
                        }
                ...
                obj2.free();
                // draw the text
                drawListBox(text, selection, nOptions, topIdx, da, fontDict, quadding, xMin, yMin,
                            xMax, yMax, borderWidth, appearBuf);
                for(i = 0; i < nOptions; ++i) {
                    delete text[i];
                }
        ...
        drawText(caption, da, fontDict, gFalse, 0, acroFormQuadLeft, gFalse, gFalse, rot, xMin,
                 yMin, xMax, yMax, borderWidth, appearBuf);
        delete caption;
    } else {
...
void AcroFormField::drawText(GString *text,
                             GString *da,
                             GfxFontDict *fontDict,
                             GBool multiline,
                             int comb,
...
// Draw the variable text or caption for a field.
void AcroFormField::drawListBox(GString **text,
                                GBool *selection,
                                int nOptions,
                                int topIdx,
                                GString *da,
    ...
    for(i = topIdx; i < nOptions; ++i) {
        ...
        if(selection[i]) {
            ...
            appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} re f\n", border, y - 0.2 * fontSize,
                               xMax - xMin - 2 * border, 1.1 * fontSize);
        }
...
void AcroFormField::getNextLine(GString *text,
                                int start,
                                GfxFont *font,
                                double fontSize,
                                double wMax,
...
void AcroFormField::drawCircle(double cx, double cy, double r, const char *cmd, GString *appearBuf) {
    ...
    appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} {4:.4f} {5:.4f} c\n", cx - bezierCircle * r,
                       cy + r, cx - r, cy + bezierCircle * r, cx - r, cy);
    appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} {4:.4f} {5:.4f} c\n", cx - r,
                       cy - bezierCircle * r, cx - bezierCircle * r, cy - r, cx, cy - r);
    appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} {4:.4f} {5:.4f} c\n", cx + bezierCircle * r,
                       cy - r, cx + r, cy - bezierCircle * r, cx + r, cy);
...
void AcroFormField::drawCircleTopLeft(double cx, double cy, double r, GString *appearBuf) {
    ...
    appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} {4:.4f} {5:.4f} c\n",
                       ...
                       cx - (1 - bezierCircle) * r2, cy + (1 + bezierCircle) * r2, cx - r2, cy + r2);
    appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} {4:.4f} {5:.4f} c\n",
                       cx - (1 + bezierCircle) * r2, cy + (1 - bezierCircle) * r2,
                       cx - (1 + bezierCircle) * r2, cy - (1 - bezierCircle) * r2, cx - r2, cy - r2);
    appearBuf->append("S\n");
...
GList *AcroFormField::tokenize(GString *s) {
    ...
    while(i < s->getLength()) {
        ...
        if(i < s->getLength()) {
            for(j = i + 1; j < s->getLength() && !Lexer::isSpace(s->getChar(j)); ++j)
                ;

...
GBool SecurityHandler::checkEncryption(GString *ownerPassword, GString *userPassword) {
    void *authData;
    GBool ok;
    int i;
...
StandardSecurityHandler::StandardSecurityHandler(PDFDoc *docA, Object *encryptDictA)
    ...
    if(!versionObj.isInt() || !revisionObj.isInt() || !permObj.isInt() || !ownerKeyObj.isString() ||
       !userKeyObj.isString()) {
    ...
    if((encVersion == 4 || encVersion == 5) && (encRevision == 4 || encRevision == 5 || encRevision == 6)) {
        ...
        if(cryptFiltersObj.isDict() && streamFilterObj.isName() && stringFilterObj.isName() &&
    ...
    if(encRevision <= 4) {
        if(ownerKeyObj.getString()->getLength() != 32 || userKeyObj.getString()->getLength() != 32) {
            error(errSyntaxError, -1, "Invalid encryption key length");
            // this is non-fatal -- see below
        }
    } else if(encRevision <= 6) {
        // the spec says 48 bytes, but Acrobat pads them out longer
        if(ownerKeyObj.getString()->getLength() < 48 || userKeyObj.getString()->getLength() < 48 ||
           !ownerEncObj.isString() || ownerEncObj.getString()->getLength() != 32 ||
           !userEncObj.isString() || userEncObj.getString()->getLength() != 32) {
            error(errSyntaxError, -1, "Invalid encryption key length");
            goto done;
        }
    }
    ...
    if(encVersion >= 1 && encVersion <= 2 && encRevision >= 2 && encRevision <= 3) {
        ...
        error(errUnimplemented, -1, "Unsupported version/revision ({0:d}/{1:d}) of Standard security handler",
...
void *StandardSecurityHandler::makeAuthData(GString *ownerPassword, GString *userPassword) {
    return new StandardAuthData(ownerPassword ? ownerPassword->copy() : (GString *)NULL,
...
GBool StandardSecurityHandler::authorize(void *authData) {
    ...
    if(!Decrypt::makeFileKey(encVersion, encRevision, fileKeyLength, ownerKey, userKey, ownerEnc,

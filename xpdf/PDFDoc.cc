...
#ifdef _WIN32
PDFDoc::PDFDoc(wchar_t *fileNameA, int fileNameLen, GString *ownerPassword, GString *userPassword, PDFCore *coreA) {
...
PDFDoc::PDFDoc(char *fileNameA, GString *ownerPassword, GString *userPassword, PDFCore *coreA) {
#ifdef _WIN32
    OSVERSIONINFO version;
#endif
    Object obj;
...
GBool PDFDoc::setup2(GString *ownerPassword, GString *userPassword, GBool repairXRef) {
    // read xref table
...
void PDFDoc::checkHeader() {
    ...
    if(!(hdrBuf[i + 5] >= '0' && hdrBuf[i + 5] <= '9') || pdfVersion > supportedPDFVersionNum + 0.0001) {
        error(errSyntaxWarning, -1, "PDF version {0:s} -- xpdf supports version {1:s} (continuing anyway)",
    ...
    if((encrypted = encrypt.isDict())) {
        if((secHdlr = SecurityHandler::make(this, &encrypt))) {
            if(secHdlr->isUnencrypted()) {
                ...
                xref->setEncryption(secHdlr->getPermissionFlags(), secHdlr->getOwnerPasswordOk(),
                                    secHdlr->getFileKey(), secHdlr->getFileKeyLength(),
                                    secHdlr->getEncVersion(), secHdlr->getEncAlgorithm());
                ret = gTrue;
            } else {
...
void PDFDoc::displayPage(OutputDev *out,
    ...
    catalog->getPage(page)->display(out, hDPI, vDPI, rotate, useMediaBox, crop, printing,
...
void PDFDoc::displayPageSlice(OutputDev *out,
                              ...
                              GBool (*abortCheckCbk)(void *data),
                              void *abortCheckCbkData) {
    catalog->getPage(page)->displaySlice(out, hDPI, vDPI, rotate, useMediaBox, crop, sliceX, sliceY,
                                         sliceW, sliceH, printing, abortCheckCbk, abortCheckCbkData);
}
...
GBool PDFDoc::isLinearized() {
    ...
    lin = gFalse;
    obj1.initNull();
    parser = new Parser(xref, new Lexer(xref, str->makeSubStream(str->getStart(), gFalse, 0, &obj1)), gTrue);
    parser->getObj(&obj1);
    parser->getObj(&obj2);
    parser->getObj(&obj3);
    parser->getObj(&obj4);

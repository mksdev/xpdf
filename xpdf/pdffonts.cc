...
static const char *fontTypeNames[] = {"unknown",          "Type 1",       "Type 1C",
                                      "Type 1C (OT)",     "Type 3",       "TrueType",
                                      "TrueType (OT)",    "CID Type 0",   "CID Type 0C",
                                      "CID Type 0C (OT)", "CID TrueType", "CID TrueType (OT)"};
...
int main(int argc, char *argv[]) {
    ...
    if(showFontLoc || showFontLocPS) {
        printf("name                                           type              emb sub uni prob "
               "object ID location\n");
        printf("---------------------------------------------- ----------------- --- --- --- ---- "
               "--------- --------\n");
    } else {
...
void scanFont(GfxFont *font, PDFDoc *doc) {
    Ref fontRef, embRef;
    ...
    // print the font info
    printf("%-46s %-17s %-3s %-3s %-3s %-4s", name ? name->getCString() : "[none]",
           fontTypeNames[font->getType()], emb ? "yes" : "no", subset ? "yes" : "no",
           hasToUnicode ? "yes" : "no", font->problematicForUnicode() ? " X" : "");
...
GBool checkObject(Object *in, Object *out) {
    int objNum;

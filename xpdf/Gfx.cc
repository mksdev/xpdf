...
Operator Gfx::opTab[] = {
    ...
    {"SCN",
     -33,
     {tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
      tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
      tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
      tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN},
...
void GfxResources::lookupColorSpace(const char *name, Object *obj) {
    ...
    if(!strcmp(name, "DeviceGray") || !strcmp(name, "DeviceRGB") || !strcmp(name, "DeviceCMYK")) {
        obj->initNull();
        return;
    }
    for(resPtr = this; resPtr; resPtr = resPtr->next) {
...
GBool Gfx::checkForContentStreamLoop(Object *ref) {
    ...
    if(ref->isRef()) {
        for(i = 0; i < contentStreamStack->getLength(); ++i) {
            ...
            if(objPtr->isRef()) {
                if(ref->getRefNum() == objPtr->getRefNum() && ref->getRefGen() == objPtr->getRefGen()) {
                ...
                for(j = 0; j < objPtr->arrayGetLength(); ++j) {
                    ...
                    if(obj1.isRef()) {
                        if(ref->getRefNum() == obj1.getRefNum() && ref->getRefGen() == obj1.getRefGen()) {
                            error(errSyntaxError, -1, "Loop in content streams");
                            obj1.free();
                            return gTrue;
                        }
    ...
    while(!obj.isEOF()) {
        ...
        if(obj.isCmd()) {
            ...
            if(errCount > contentStreamErrorLimit) {
                error(errSyntaxError, -1, "Too many errors - giving up on this content stream");
                break;
            }
...
void Gfx::opConcat(Object args[], int numArgs) {
    state->concatCTM(args[0].getNum(), args[1].getNum(), args[2].getNum(), args[3].getNum(),
                     args[4].getNum(), args[5].getNum());
    out->updateCTM(state, args[0].getNum(), args[1].getNum(), args[2].getNum(), args[3].getNum(),
                   args[4].getNum(), args[5].getNum());
    fontChanged = gTrue;
}
...
void Gfx::opSetExtGState(Object args[], int numArgs) {
    ...
    // font
    if(obj1.dictLookup("Font", &obj2)->isArray() && obj2.arrayGetLength() == 2) {
    ...
    if(!obj2.isNull()) {
        if(!state->getIgnoreColorOps()) {
            ...
            error(errSyntaxWarning, getPos(),
                  "Ignoring transfer function setting"
                  " in uncolored Type 3 char or tiling pattern");
        }
    }
    obj2.free();
    ...
    if(!obj1.dictLookup("SMask", &obj2)->isNull()) {
        if(obj2.isName("None")) {
            ...
            if(obj3.isName("Alpha")) {
                ...
                error(errSyntaxError, getPos(), "Missing S (subtype) entry in soft mask");
                alpha = gFalse;
            }
            obj3.free();
            funcs[0] = NULL;
            ...
            if(obj2.dictLookup("G", &obj3)->isStream()) {
                if(obj3.streamGetDict()->lookup("Group", &obj4)->isDict()) {
                    ...
                    doSoftMask(&obj3, &objRef3, alpha, blendingColorSpace, gTrue, knockout, funcs[0], &backdropColor);
                    objRef3.free();
                    if(funcs[0]) {
                        delete funcs[0];
                    }
...
void Gfx::doSoftMask(Object *str,
                     Object *strRef,
                     GBool alpha,
                     GfxColorSpace *blendingColorSpace,
                     GBool isolated,
    ...
    // draw it
    ++formDepth;
    drawForm(strRef, resDict, m, bbox, gTrue, gTrue, blendingColorSpace, isolated, knockout, alpha,
             transferFunc, backdropColor);
    --formDepth;
...
void Gfx::opSetRenderingIntent(Object args[], int numArgs) {
    ...
    if(state->getIgnoreColorOps()) {
        error(errSyntaxWarning, getPos(),
              "Ignoring rendering intent setting"
              " in uncolored Type 3 char or tiling pattern");
        return;
    }
...
void Gfx::opSetFillColorN(Object args[], int numArgs) {
    ...
    if(state->getFillColorSpace()->getMode() == csPattern) {
        if(numArgs == 0 || !args[numArgs - 1].isName()) {
            error(errSyntaxError, getPos(), "Invalid arguments in 'scn' command");
            return;
        }
        if(numArgs > 1) {
            if(!((GfxPatternColorSpace *)state->getFillColorSpace())->getUnder() ||
               numArgs - 1 != ((GfxPatternColorSpace *)state->getFillColorSpace())->getUnder()->getNComps()) {
...
void Gfx::opSetStrokeColorN(Object args[], int numArgs) {
    ...
    if(state->getStrokeColorSpace()->getMode() == csPattern) {
        ...
        if(numArgs > 1) {
            if(!((GfxPatternColorSpace *)state->getStrokeColorSpace())->getUnder() ||
               numArgs - 1 != ((GfxPatternColorSpace *)state->getStrokeColorSpace())->getUnder()->getNComps()) {
                error(errSyntaxError, getPos(), "Incorrect number of arguments in 'SCN' command");
                return;
            }
            for(i = 0; i < numArgs - 1 && i < gfxColorMaxComps; ++i) {
...
void Gfx::doPatternImageMask(Object *ref, Stream *str, int width, int height, GBool invert, GBool inlineImg, GBool interpolate) {
    saveState();

    out->setSoftMaskFromImageMask(state, ref, str, width, height, invert, inlineImg, interpolate);
...
void Gfx::doTilingPatternFill(GfxTilingPattern *tPat, GBool stroke, GBool eoFill, GBool text) {
    ...
    if(out->useTilingPatternFill()) {
        m1[4] = m[4];
        m1[5] = m[5];
        out->tilingPatternFill(state, this, tPat->getContentStreamRef(), tPat->getPaintType(),
                               tPat->getTilingType(), tPat->getResDict(), m1, bbox, xi0, yi0, xi1,
                               yi1, xstep, ystep);
    } else {
        abortCheckCounter = 0;
        for(yi = yi0; yi < yi1; ++yi) {
            for(xi = xi0; xi < xi1; ++xi) {
                ...
                drawForm(tPat->getContentStreamRef(), tPat->getResDict(), m1, bbox);
            }
        }
    }
...
void Gfx::doShadingPatternFill(GfxShadingPattern *sPat, GBool stroke, GBool eoFill, GBool text) {
    GfxShading *shading;
    GfxState *savedState;
    double *ctm, *btm, *ptm;
...
void Gfx::doGouraudTriangleShFill(GfxGouraudTriangleShading *shading) {
    ...
    for(i = 0; i < shading->getNTriangles(); ++i) {
        ...
        gouraudFillTriangle(x0, y0, color0, x1, y1, color1, x2, y2, color2, shading, 0);
    }
}

void Gfx::gouraudFillTriangle(double x0,
                              double y0,
                              double *color0,
                              double x1,
                              double y1,
    ...
    for(i = 0; i < nComps; ++i) {
        if(abs(c0.c[i] - c1.c[i]) > gouraudColorDelta || abs(c1.c[i] - c2.c[i]) > gouraudColorDelta) {
            break;
        }
    }
    ...
    if(i == nComps || depth == gouraudMaxDepth ||
       (fabs(dx0) < 0.5 && fabs(dy0) < 0.5 && fabs(dx1) < 0.5 && fabs(dy1) < 0.5 &&
        ...
        gouraudFillTriangle(x0, y0, color0, x01, y01, color01, x20, y20, color20, shading, depth + 1);
        gouraudFillTriangle(x01, y01, color01, x1, y1, color1, x12, y12, color12, shading, depth + 1);
        gouraudFillTriangle(x01, y01, color01, x12, y12, color12, x20, y20, color20, shading, depth + 1);
        gouraudFillTriangle(x20, y20, color20, x12, y12, color12, x2, y2, color2, shading, depth + 1);
    }
}
...
void Gfx::fillPatch(GfxPatch *patch, GfxPatchMeshShading *shading, int depth) {
    ...
    if(!stop) {
        ...
        for(i = 0; i < nComps; ++i) {
            if(abs(c00.c[i] - c01.c[i]) > patchColorDelta || abs(c01.c[i] - c11.c[i]) > patchColorDelta ||
               abs(c11.c[i] - c10.c[i]) > patchColorDelta || abs(c10.c[i] - c00.c[i]) > patchColorDelta) {
                break;
            }
        }
        if(i == nComps) {
    ...
    if(stop) {
        ...
        state->curveTo(patch->x[0][1], patch->y[0][1], patch->x[0][2], patch->y[0][2],
                       patch->x[0][3], patch->y[0][3]);
        state->curveTo(patch->x[1][3], patch->y[1][3], patch->x[2][3], patch->y[2][3],
                       patch->x[3][3], patch->y[3][3]);
        state->curveTo(patch->x[3][2], patch->y[3][2], patch->x[3][1], patch->y[3][1],
                       patch->x[3][0], patch->y[3][0]);
        state->curveTo(patch->x[2][0], patch->y[2][0], patch->x[1][0], patch->y[1][0],
        ...
        for(i = 0; i < shading->getNComps(); ++i) {
            ...
            patch00.color[0][1][i] = 0.5 * (patch->color[0][0][i] + patch->color[0][1][i]);
            patch01.color[0][0][i] = patch00.color[0][1][i];
            patch01.color[0][1][i] = patch->color[0][1][i];
            patch01.color[1][1][i] = 0.5 * (patch->color[0][1][i] + patch->color[1][1][i]);
            patch11.color[0][1][i] = patch01.color[1][1][i];
            patch11.color[1][1][i] = patch->color[1][1][i];
            patch11.color[1][0][i] = 0.5 * (patch->color[1][1][i] + patch->color[1][0][i]);
            patch10.color[1][1][i] = patch11.color[1][0][i];
            patch10.color[1][0][i] = patch->color[1][0][i];
            patch10.color[0][0][i] = 0.5 * (patch->color[1][0][i] + patch->color[0][0][i]);
            patch00.color[1][0][i] = patch10.color[0][0][i];
            patch00.color[1][1][i] = 0.5 * (patch00.color[1][0][i] + patch01.color[1][1][i]);
...
void Gfx::opSetTextMatrix(Object args[], int numArgs) {
    state->setTextMat(args[0].getNum(), args[1].getNum(), args[2].getNum(), args[3].getNum(),
                      args[4].getNum(), args[5].getNum());
    state->textMoveTo(0, 0);
    out->updateTextMat(state);
    out->updateTextPos(state);
...
void Gfx::doShowText(GString *s) {
    ...
    if(globalParams->isDroppedFont(font->getName() ? font->getName()->getCString() : "")) {
        doIncCharCount(s);
        return;
    }
    ...
    if(font->getType() == fontType3 && out->interpretType3Chars()) {
        ...
        len = s->getLength();
        while(len > 0) {
            n = font->getNextChar(p, len, &code, u, (int)(sizeof(u) / sizeof(Unicode)), &uLen, &dx,
                                  &dy, &originX, &originY);
        ...
        while(len > 0) {
            ...
            out->drawChar(state, state->getCurX() + riseX, state->getCurY() + riseY, tdx, tdy,
                          tOriginX, tOriginY, code, n, u, uLen);
            state->shift(tdx, tdy);
            p += n;
            len -= n;
        }
        ...
        nChars = nSpaces = 0;
        while(len > 0) {
            n = font->getNextChar(p, len, &code, u, (int)(sizeof(u) / sizeof(Unicode)), &uLen, &dx2,
                                  &dy2, &originX, &originY);
            dx += dx2;
            dy += dy2;
            if(n == 1 && *p == ' ') {
    ...
    try {
        ...
        if(obj2.isName("Image")) {
            ...
            if(ocState) {
                ...
                out->psXObject(obj1.getStream(), obj3.isStream() ? obj3.getStream() : (Stream *)NULL);
            }
        } else if(obj2.isName()) {
...
GBool Gfx::doImage(Object *ref, Stream *str, GBool inlineImg) {
    ...
    if(mask) {
        ...
        if(!ocState) {
            ...
            if(state->getFillColorSpace()->getMode() == csPattern) {
                doPatternImageMask(ref, str, width, height, invert, inlineImg, interpolate);
            } else {
                out->drawImageMask(state, ref, str, width, height, invert, inlineImg, interpolate);
            }
        }
        ...
        if(smaskObj.isStream()) {
            ...
            for(i = 0; i + 1 < maskObj.arrayGetLength() && i + 1 < 2 * gfxColorMaxComps; i += 2) {
        ...
        if(state->getIgnoreColorOps() || !ocState) {
            ...
            if(haveSoftMask) {
                out->drawSoftMaskedImage(state, ref, str, width, height, colorMap, maskStr, maskWidth, maskHeight,
                                         maskColorMap, haveMatte ? matte : (double *)NULL, interpolate);
                delete maskColorMap;
            } else if(haveExplicitMask) {
                out->drawMaskedImage(state, ref, str, width, height, colorMap, maskStr, maskWidth,
                                     maskHeight, maskInvert, interpolate);
            } else {
                out->drawImage(state, ref, str, width, height, colorMap,
                               haveColorKeyMask ? maskColors : (int *)NULL, inlineImg, interpolate);
            }
        }
...
void Gfx::doForm(Object *strRef, Object *str) {
    ...
    // draw it
    ++formDepth;
    drawForm(strRef, resDict, m, bbox, transpGroup, gFalse, blendingColorSpace, isolated, knockout);
    --formDepth;
...
void Gfx::drawForm(Object *strRef,
                   Dict *resDict,
                   double *matrix,
                   double *bbox,
                   GBool transpGroup,
...
void Gfx::opSetCacheDevice(Object args[], int numArgs) {
    state->setIgnoreColorOps(gTrue);
    out->type3D1(state, args[0].getNum(), args[1].getNum(), args[2].getNum(), args[3].getNum(),
                 args[4].getNum(), args[5].getNum());
}
...
void Gfx::opEndMarkedContent(Object args[], int numArgs) {
    ...
    if(markedContentStack->getLength() > 0) {
        mc = (GfxMarkedContent *)markedContentStack->del(markedContentStack->getLength() - 1);
        mcKind = mc->kind;
        delete mc;
        if(mcKind == gfxMCOptionalContent) {
            if(markedContentStack->getLength() > 0) {
                mc = (GfxMarkedContent *)markedContentStack->get(markedContentStack->getLength() - 1);
                ocState = mc->ocState;
            } else {
                ocState = gTrue;
            }
...
void Gfx::drawAnnot(Object *strRef, AnnotBorderStyle *borderStyle, double xMin, double yMin, double xMax, double yMax) {
    Dict *dict, *resDict;
    ...
    // draw the border
    if(borderStyle && borderStyle->getWidth() > 0 && borderStyle->getNumColorComps() > 0) {

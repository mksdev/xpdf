...
class TileDesc {
public:
    TileDesc(int pageA, int rotateA, double dpiA, int txA, int tyA, int twA, int thA)
        : page(pageA), rotate(rotateA), dpi(dpiA), tx(txA), ty(tyA), tw(twA), th(thA) {}

    GBool matches(TileDesc *tile) {
        return page == tile->page && rotate == tile->rotate && dpi == tile->dpi && tx == tile->tx &&
               ty == tile->ty && tw == tile->tw && th == tile->th;
    }
...
public:
    PlacedTileDesc(int pageA, int rotateA, double dpiA, int txA, int tyA, int twA, int thA, int pxA, int pyA)
...
public:
    ...
    GBool cvtUserToWindow(int pg, double xu, double yu, int *xw, int *yw);

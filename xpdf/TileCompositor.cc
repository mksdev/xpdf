...
TileCompositor::TileCompositor(DisplayState *stateA, TileMap *tileMapA, TileCache *tileCacheA) {
    state = stateA;
...
SplashBitmap *TileCompositor::getBitmap(GBool *finished) {
    ...
    if(!bitmap || bitmap->getWidth() != state->getWinW() || bitmap->getHeight() != state->getWinH()) {
        ...
        bitmap = new SplashBitmap(state->getWinW(), state->getWinH(), state->getBitmapRowPad(),
                                  state->getColorMode(), gFalse, gTrue, NULL);
    }
    clearBitmap();
    ...
    for(i = 0; i < tiles->getLength(); ++i) {
        ...
        if(w > 0 && h > 0) {
            if((tileBitmap = tileCache->getTileBitmap(tile, &tileFinished))) {
                blit(tileBitmap, xSrc, ySrc, bitmap, xDest, yDest, w, h, !tileFinished);
            } else {
        ...
        case splashModeRGB8:
            if(state->getMatteColor()[0] == state->getMatteColor()[1] &&
               state->getMatteColor()[1] == state->getMatteColor()[2]) {
                if(rowSize < 0) {
                    memset(data + rowSize * (h - 1), state->getMatteColor()[0], -rowSize * h);
                } else {
...
void TileCompositor::blit(SplashBitmap *srcBitmap,
                          int xSrc,
                          int ySrc,
                          SplashBitmap *destBitmap,
                          int xDest,
                          int yDest,
                          int w,
                          int h,
                          GBool compositeWithPaper) {

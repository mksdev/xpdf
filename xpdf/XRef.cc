...
ObjectStream::ObjectStream(XRef *xref, int objStrNumA) {
    ...
    for(i = 0; i < nObjects; ++i) {
        ...
        if(i == nObjects - 1) {
            ...
            str = new EmbedStream(objStr.getStream(), &obj1, gTrue, offsets[i + 1] - offsets[i]);
        }
        lexer = new Lexer(xref, str);
        parser = new Parser(xref, lexer, gFalse);
...
XRef::XRef(BaseStream *strA, GBool repair) {
    ...
    if(repair) {
        ...
        xrefTablePos = (GFileOffset *)gmallocn(xrefTablePosLen, sizeof(GFileOffset));
...
GBool XRef::readXRef(GFileOffset *pos, XRefPosSet *posSet) {
    ...
    if(i + 4 < n && buf[i] == 'x' && buf[i + 1] == 'r' && buf[i + 2] == 'e' && buf[i + 3] == 'f' &&
    ...
    while(1) {
        ...
        for(i = first; i < first + n; ++i) {
            ...
            if(entries[i].offset == (GFileOffset)-1) {
                ...
                // instead of 0.
                if(i == 1 && first == 1 && entries[1].offset == 0 && entries[1].gen == 65535 &&
...
GBool XRef::constructXRef() {
    ...
    while(1) {
        ...
        if(!strncmp(p, "trailer", 7)) {
            obj.initNull();
            parser = new Parser(NULL, new Lexer(NULL, str->makeSubStream(pos + 7, gFalse, 0, &obj)), gFalse);
            ...
            if(streamEndsLen == streamEndsSize) {
                streamEndsSize += 64;
                streamEnds = (GFileOffset *)greallocn(streamEnds, streamEndsSize, sizeof(GFileOffset));
            }
...
void XRef::setEncryption(int permFlagsA, GBool ownerPasswordOkA, Guchar *fileKeyA, int keyLengthA, int encVersionA, CryptAlgorithm encAlgorithmA) {
    int i;
...
GBool XRef::getEncryption(int *permFlagsA, GBool *ownerPasswordOkA, int *keyLengthA, int *encVersionA, CryptAlgorithm *encAlgorithmA) {
    if(!encrypted) {
        return gFalse;
    }
...
Object *XRef::fetch(int num, int gen, Object *obj, int recursion) {
    ...
    switch(e->type) {
        ...
        case xrefEntryUncompressed:
            ...
            obj1.initNull();
            parser =
                new Parser(this, new Lexer(this, str->makeSubStream(start + e->offset, gFalse, 0, &obj1)), gTrue);
            parser->getObj(&obj1, gTrue);
            parser->getObj(&obj2, gTrue);
            parser->getObj(&obj3, gTrue);
            if(!obj1.isInt() || obj1.getInt() != num || !obj2.isInt() || obj2.getInt() != gen ||
               !obj3.isCmd("obj")) {
                obj1.free();
                obj2.free();
                obj3.free();
            ...
            parser->getObj(obj, gFalse, encrypted ? fileKey : (Guchar *)NULL, encAlgorithm,
...
GBool XRef::getObjectStreamObject(int objStrNum, int objIdx, int objNum, Object *obj) {
    ObjectStream *objStr;

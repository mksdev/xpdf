...
void ImageOutputDev::tilingPatternFill(GfxState *state,
                                       Gfx *gfx,
                                       Object *strRef,
                                       int paintType,
                                       int tilingType,
...
void ImageOutputDev::drawImageMask(GfxState *state, Object *ref, Stream *str, int width, int height, GBool invert, GBool inlineImg, GBool interpolate) {
    ...
    if(dumpRaw && !inlineImg) {
        ...
        // open the image file
        sprintf(fileName, "%s-%04d.%s", fileRoot, imgNum, getRawFileExtension(str));
        ++imgNum;
...
void ImageOutputDev::drawImage(GfxState *state,
    ...
    if(dumpRaw && !inlineImg) {
        ...
        // dump JPEG file
              ...
              (colorMap->getNumPixelComps() == 1 || colorMap->getNumPixelComps() == 3) && !inlineImg) {
...
void ImageOutputDev::drawMaskedImage(GfxState *state,
    ...
    drawImageMask(state, ref, maskStr, maskWidth, maskHeight, maskInvert, gFalse, interpolate);
}
...
void ImageOutputDev::drawSoftMaskedImage(GfxState *state,
                                         ...
                                         GBool interpolate) {
    drawImage(state, ref, str, width, height, colorMap, NULL, gFalse, interpolate);
    drawImage(state, ref, maskStr, maskWidth, maskHeight, maskColorMap, NULL, gFalse, interpolate);
}
...
void ImageOutputDev::writeImageInfo(int width, int height, GfxState *state, GfxImageColorMap *colorMap) {
    const char *mode;
    ...
    if(colorMap) {
        mode = GfxColorSpace::getColorSpaceModeName(colorMap->getColorSpace()->getMode());
    ...
    printf("%s: page=%d width=%d height=%d hdpi=%.2f vdpi=%.2f %s%s bpc=%d\n", fileName, curPageNum,
           width, height, hdpi, vdpi, mode ? "colorspace=" : "mask", mode ? mode : "", bpc);
}

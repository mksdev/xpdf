...
static Guchar passwordPad[32] = {0x28, 0xbf, 0x4e, 0x5e, 0x4e, 0x75, 0x8a, 0x41, 0x64, 0x00, 0x4e,
    ...
    *ownerPasswordOk = gFalse;
    ...
    if(encRevision == 5 || encRevision == 6) {
        ...
        if(ownerPassword) {
            ...
            if(makeFileKey2(encVersion, encRevision, keyLength, ownerKey, userKey, permissions,
                            fileID, userPassword2, fileKey, encryptMetadata)) {
                *ownerPasswordOk = gTrue;
        ...
        return makeFileKey2(encVersion, encRevision, keyLength, ownerKey, userKey, permissions,
                            fileID, userPassword, fileKey, encryptMetadata);
    }
}
...
static Guint rcon[11] = {0x00000000, // unused
                         0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
                         0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000};

static inline Guint subWord(Guint x) {
    return (sbox[x >> 24] << 24) | (sbox[(x >> 16) & 0xff] << 16) | (sbox[(x >> 8) & 0xff] << 8) |
           sbox[x & 0xff];
}
...
void aesKeyExpansion(DecryptAESState *s, Guchar *objKey, int objKeyLen, GBool decrypt) {
    ...
    for(i = 0; i < 4; ++i) {
        s->w[i] = (objKey[4 * i] << 24) + (objKey[4 * i + 1] << 16) + (objKey[4 * i + 2] << 8) +
                  objKey[4 * i + 3];
    }
    for(i = 4; i < 44; ++i) {
        temp = s->w[i - 1];
...
static void md5ProcessBlock(MD5State *state) {
    ...
    for(i = 0; i < 16; ++i) {
        x[i] = state->buf[4 * i] | (state->buf[4 * i + 1] << 8) | (state->buf[4 * i + 2] << 16) |
               (state->buf[4 * i + 3] << 24);
    }
...
static Guint sha256K[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
...
static void sha512HashBlock(Guchar *blk, SHA512Uint64 *H) {
    ...
    // 1. prepare the message schedule
    for(t = 0; t < 16; ++t) {
        W[t] = ((SHA512Uint64)blk[t * 8] << 56) | ((SHA512Uint64)blk[t * 8 + 1] << 48) |
               ((SHA512Uint64)blk[t * 8 + 2] << 40) | ((SHA512Uint64)blk[t * 8 + 3] << 32) |
               ((SHA512Uint64)blk[t * 8 + 4] << 24) | ((SHA512Uint64)blk[t * 8 + 5] << 16) |

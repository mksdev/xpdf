...
// (for Splash).
static SplashStrokeAdjustMode mapStrokeAdjustMode[3] = {splashStrokeAdjustOff, splashStrokeAdjustNormal,
...
static void splashOutBlendOverlay(SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend, SplashColorMode cm) {
    ...
    for(i = 0; i < splashColorModeNComps[cm]; ++i) {
        ...
        blend[i] = dest[i] < 0x80 ? (Guchar)((src[i] * 2 * dest[i]) / 255) :
...
static void splashOutBlendHardLight(SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend, SplashColorMode cm) {
    ...
    for(i = 0; i < splashColorModeNComps[cm]; ++i) {
        // the spec says "if Cs <= 0.5" -- note that 0x80 is 128/255=0.5020
        blend[i] = src[i] < 0x80 ? (Guchar)((dest[i] * 2 * src[i]) / 255) :
...
static void splashOutBlendSoftLight(SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend, SplashColorMode cm) {
    ...
    for(i = 0; i < splashColorModeNComps[cm]; ++i) {
        // the spec says "if Cs <= 0.5" -- note that 0x80 is 128/255=0.5020
        if(src[i] < 0x80) {
            blend[i] = (Guchar)(dest[i] - (255 - 2 * src[i]) * dest[i] * (255 - dest[i]) / (255 * 255));
        } else {
...
static void splashOutBlendDifference(SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend, SplashColorMode cm) {
    ...
    for(i = 0; i < splashColorModeNComps[cm]; ++i) {
        blend[i] = dest[i] < src[i] ? (Guchar)(src[i] - dest[i]) : (Guchar)(dest[i] - src[i]);
    }
}
...
static void clipColor(int rIn, int gIn, int bIn, Guchar *rOut, Guchar *gOut, Guchar *bOut) {
...
static void setLum(Guchar rIn, Guchar gIn, Guchar bIn, int lum, Guchar *rOut, Guchar *gOut, Guchar *bOut) {
    int d;
...
static void setSat(Guchar rIn, Guchar gIn, Guchar bIn, int sat, Guchar *rOut, Guchar *gOut, Guchar *bOut) {
...
public:
    ...
    void setOblique(double obliqueA) {
        oblique = obliqueA;
    }
    double getOblique() {
        return oblique;
    }
    void setSubstIdx(int substIdxA) {
        substIdx = substIdxA;
    }
    int getSubstIdx() {
        return substIdx;
...
public:
    T3FontCache(Ref *fontID,
                ...
                GBool aa);
    ~T3FontCache();
    GBool matches(Ref *idA, double m11A, double m12A, double m21A, double m22A) {
        return fontID.num == idA->num && fontID.gen == idA->gen && m11 == m11A && m12 == m12A &&
               m21 == m21A && m22 == m22A;
    }
...
T3FontCache::T3FontCache(Ref *fontIDA,
                         double m11A,
                         double m12A,
                         double m21A,
                         double m22A,
    ...
    for(cacheSets = type3FontCacheMaxSets;
        cacheSets > 1 && cacheSets * cacheAssoc * glyphSize > type3FontCacheSize; cacheSets >>= 1)
        ;
...
SplashOutputDev::SplashOutputDev(SplashColorMode colorModeA,
    ...
    bitmap = new SplashBitmap(1, 1, bitmapRowPad, colorMode, colorMode != splashModeMono1, bitmapTopDown, NULL);
...
#if HAVE_FREETYPE_H
        globalParams->getEnableFreeType(), globalParams->getDisableFreeTypeHinting() ? splashFTNoHinting : 0,
#endif
...
void SplashOutputDev::startPage(int pageNum, GfxState *state) {
    ...
    if(!bitmap || w != bitmap->getWidth() || h != bitmap->getHeight()) {
        ...
        bitmap = new SplashBitmap(w, h, bitmapRowPad, colorMode, colorMode != splashModeMono1,
                                  bitmapTopDown, NULL);
    }
    splash = new Splash(bitmap, vectorAntialias, NULL, &screenParams);
    splash->setMinLineWidth(globalParams->getMinLineWidth());
...
void SplashOutputDev::updateTransfer(GfxState *state) {
    ...
    if(transfer[0] && transfer[0]->getInputSize() == 1 && transfer[0]->getOutputSize() == 1) {
        if(transfer[1] && transfer[1]->getInputSize() == 1 && transfer[1]->getOutputSize() == 1 &&
           transfer[2] && transfer[2]->getInputSize() == 1 && transfer[2]->getOutputSize() == 1 &&
           transfer[3] && transfer[3]->getInputSize() == 1 && transfer[3]->getOutputSize() == 1) {
            for(i = 0; i < 256; ++i) {
                x = i / 255.0;
...
void SplashOutputDev::doUpdateFont(GfxState *state) {
    ...
    if((fontFile = fontEngine->getFontFile(id))) {
        ...
        if(fontLoc->locType == gfxFontLocEmbedded) {
            ...
            if(!(extFontFile = fopen(fontLoc->path->getCString(), "rb"))) {
                error(errSyntaxError, -1, "Couldn't open external font file '{0:t}'", fontLoc->path);
                delete fontLoc;
                goto err2;
            }
            fontBuf = new GString();
        ...
        switch(fontLoc->fontType) {
            ...
            case fontType1C:
                ...
                if(!(fontFile = fontEngine->loadType1CFont(id,
#if LOAD_FONTS_FROM_MEM
                                                           fontBuf,
#else
                                                           fileName->getCString(), fileName == tmpFileName,
            ...
            case fontType1COT:
                ...
                if(!(fontFile = fontEngine->loadOpenTypeT1CFont(
                         ...
                         fileName->getCString(), fileName == tmpFileName,
#endif
                         codeToGID, (const char **)((Gfx8BitFont *)gfxFont)->getEncoding()))) {
                    error(errSyntaxError, -1, "Couldn't create a font for '{0:s}'",
                          gfxFont->getName() ? gfxFont->getName()->getCString() : "(unnamed)");
                    delete fontLoc;
                    goto err2;
            ...
            case fontTrueTypeOT:
#if LOAD_FONTS_FROM_MEM
                if((ff = FoFiTrueType::make(fontBuf->getCString(), fontBuf->getLength(), fontNum))) {
#else
            ...
            case fontCIDType0C:
                ...
                if(!(fontFile = fontEngine->loadCIDFont(id,
#if LOAD_FONTS_FROM_MEM
                                                        fontBuf,
#else
                                                        fileName->getCString(), fileName == tmpFileName,
    ...
    if(substIdx >= 0 && substIdx < 12) {
        ...
        for(code = 0; code < 256; ++code) {
            if((name = ((Gfx8BitFont *)gfxFont)->getCharName(code)) && name[0] && !name[1] &&
...
void SplashOutputDev::stroke(GfxState *state) {
    ...
    setOverprintMask(state, state->getStrokeColorSpace(), state->getStrokeOverprint(),
...
void SplashOutputDev::fill(GfxState *state) {
    ...
    setOverprintMask(state, state->getFillColorSpace(), state->getFillOverprint(),
...
void SplashOutputDev::tilingPatternFill(GfxState *state,
                                        Gfx *gfx,
                                        Object *strRef,
                                        int paintType,
                                        int tilingType,
    ...
    if(tileXMax - tileXMin + 0.5 > (double)INT_MAX || tileYMax - tileYMin + 0.5 > (double)INT_MAX ||
       tileW > INT_MAX / tileH || tileSize > 1000000) {
        mat1[0] = mat[0];
        mat1[1] = mat[1];
        mat1[2] = mat[2];
        mat1[3] = mat[3];
    ...
    if(tilingType == 2 || idet == 0) {
        ...
        ix = (int)((yStepX * (tileYMin - clipYC) - (tileXMin - clipXC) * yStepY) * idet + 0.5);
        iy = (int)((xStepX * (clipYC - tileYMin) - (clipXC - tileXMin) * xStepY) * idet + 0.5);
    ...
    origSplash = splash;
    bitmap = tileBitmap =
        new SplashBitmap(tileW, tileH, bitmapRowPad, colorMode, gTrue, bitmapTopDown, origBitmap);
    ...
    for(iy = iyMin; iy < iyMax; ++iy) {
        for(ix = ixMin; ix < ixMax; ++ix) {
            ...
            splash->composite(tileBitmap, 0, 0, x, y, tileW, tileH, gFalse, gFalse);
        }
    }
...
GBool SplashOutputDev::axialShadedFill(GfxState *state, GfxAxialShading *shading) {
    ...
    tBitmap = new SplashBitmap(bitmapWidth, bitmapHeight, 1, srcMode, gTrue, gTrue, bitmap);
    ...
    if(dZero) {
        ...
        nColors = (int)(1.5 * sqrt((xx1 - xx0) * (xx1 - xx0) + (yy1 - yy0) * (yy1 - yy0)));
        if(nColors < 16) {
            nColors = 16;
...
GBool SplashOutputDev::radialShadedFill(GfxState *state, GfxRadialShading *shading) {
    ...
    if((!ext0 && !ext1) || (enclosed && !(r0 > r1 ? ext0 : ext1))) {
    ...
    // pre-compute colors along the axis
    nColors = (int)sqrt((double)(bitmapWidth * bitmapWidth + bitmapHeight * bitmapHeight));
    if(nColors < 16) {
        nColors = 16;
    } else if(nColors > 1024) {
        nColors = 1024;
    ...
    // circle
    if(enclosed && ((r0 > r1 && ext0) || (r1 > r0 && ext1))) {
        if(r0 > r1) {
            sColor = sColors;
            uxMin = x0 - r0;
            uxMax = x0 + r0;
    ...
    for(y = byMin; y < byMax; ++y) {
        dataPtr = tBitmap->getDataPtr() + y * tBitmap->getRowSize() + bxMin * nComps;
...
void SplashOutputDev::computeShadingColor(GfxState *state, SplashColorMode mode, GfxColor *color, SplashColorPtr sColor) {
    GfxGray gray;
    GfxRGB rgb;
#if SPLASH_CMYK
    GfxCMYK cmyk;
...
SplashPath *SplashOutputDev::convertPath(GfxState *state, GfxPath *path, GBool dropEmptySubpaths) {
    SplashPath *sPath;
    GfxSubpath *subpath;
    int n, i, j;
        ...
        if(subpath->getNumPoints() > n) {
            ...
            while(j < subpath->getNumPoints()) {
                if(subpath->getCurve(j)) {
                    sPath->curveTo((SplashCoord)subpath->getX(j), (SplashCoord)subpath->getY(j),
                                   (SplashCoord)subpath->getX(j + 1), (SplashCoord)subpath->getY(j + 1),
                                   (SplashCoord)subpath->getX(j + 2), (SplashCoord)subpath->getY(j + 2));
                    j += 3;
                } else {
...
void SplashOutputDev::drawChar(GfxState *state,
                               double x,
                               double y,
                               double dx,
                               double dy,
...
GBool SplashOutputDev::beginType3Char(GfxState *state, double x, double y, double dx, double dy, CharCode code, Unicode *u, int uLen) {
    GfxFont *gfxFont;
    Ref *fontID;
    double *ctm, *bbox;
    T3FontCache *t3Font;
    ...
    if(!(nT3Fonts > 0 && t3FontCache[0]->matches(fontID, ctm[0], ctm[1], ctm[2], ctm[3]))) {
        ...
        if(i >= nT3Fonts) {
            ...
            t3FontCache[0] =
                new T3FontCache(fontID, ctm[0], ctm[1], ctm[2], ctm[3], (int)floor(xMin - xt) - 2,
                                (int)floor(yMin - yt) - 2, (int)ceil(xMax) - (int)floor(xMin) + 4,
                                (int)ceil(yMax) - (int)floor(yMin) + 4, validBBox, colorMode != splashModeMono1);
        }
    }
...
void SplashOutputDev::type3D1(GfxState *state, double wx, double wy, double llx, double lly, double urx, double ury) {
    ...
    if(xMin - xt < t3Font->glyphX || yMin - yt < t3Font->glyphY ||
       xMax - xt > t3Font->glyphX + t3Font->glyphW || yMax - yt > t3Font->glyphY + t3Font->glyphH) {
        if(t3Font->validBBox) {
    ...
    if(colorMode == splashModeMono1) {
        ...
        bitmap = new SplashBitmap(t3Font->glyphW, t3Font->glyphH, 1, splashModeMono1, gFalse, gTrue, bitmap);
        splash = new Splash(bitmap, gFalse, t3GlyphStack->origSplash->getImageCache(),
        ...
        bitmap = new SplashBitmap(t3Font->glyphW, t3Font->glyphH, 1, splashModeMono8, gFalse, gTrue, bitmap);
        splash = new Splash(bitmap, vectorAntialias, t3GlyphStack->origSplash->getImageCache(),
                            t3GlyphStack->origSplash->getScreen());
        color[0] = 0x00;
        splash->clear(color);
        color[0] = 0xff;
...
void SplashOutputDev::drawType3Glyph(GfxState *state, T3FontCache *t3Font, T3FontCacheTag *tag, Guchar *data) {
...
void SplashOutputDev::drawImageMask(GfxState *state,
                                    Object *ref,
                                    Stream *str,
                                    int width,
                                    int height,
    ...
    splash->fillImageMask(imgTag, &imageMaskSrc, &imgMaskData, width, height, mat,
...
void SplashOutputDev::setSoftMaskFromImageMask(GfxState *state,
    ...
    imgTag = makeImageTag(ref);
    maskSplash->fillImageMask(imgTag, &imageMaskSrc, &imgMaskData, width, height, mat, gFalse, interpolate);
    delete imgTag;
    delete imgMaskData.imgStr;
    str->close();
    delete maskSplash;
...
GBool SplashOutputDev::imageSrc(void *data, SplashColorPtr colorLine, Guchar *alphaLine) {
...
GBool SplashOutputDev::alphaImageSrc(void *data, SplashColorPtr colorLine, Guchar *alphaLine) {
    SplashOutImageData *imgData = (SplashOutImageData *)data;
    Guchar *p0, *p, *aq;
    SplashColorPtr q, col;
    Guchar alpha;
...
void SplashOutputDev::drawImage(GfxState *state,
    ...
    setOverprintMask(state, colorMap->getColorSpace(), state->getFillOverprint(),
                     state->getOverprintMode(), NULL);
    ...
    imgTag = makeImageTag(ref);
    splash->drawImage(imgTag, src, &imgData, srcMode, maskColors ? gTrue : gFalse, width, height, mat, interpolate);
    if(inlineImg) {
...
GBool SplashOutputDev::maskedImageSrc(void *data, SplashColorPtr colorLine, Guchar *alphaLine) {
...
void SplashOutputDev::drawMaskedImage(GfxState *state,
                                      Object *ref,
                                      Stream *str,
                                      int width,
                                      int height,
    ...
    if(maskWidth > width || maskHeight > height) {
        ...
        maskDecode.free();
        drawSoftMaskedImage(state, ref, str, width, height, colorMap, maskStr, maskWidth,
                            maskHeight, maskColorMap, NULL, interpolate);
        delete maskColorMap;
        ...
        imgMaskData.y = 0;
        maskBitmap = new SplashBitmap(width, height, 1, splashModeMono1, gFalse, gTrue, bitmap);
        ...
        maskSplash->fillImageMask(NULL, &imageMaskSrc, &imgMaskData, maskWidth, maskHeight, mat, gTrue, interpolate);
        delete imgMaskData.imgStr;
        maskStr->close();
        delete maskSplash;
        ...
        imgTag = makeImageTag(ref);
        splash->drawImage(imgTag, &maskedImageSrc, &imgData, srcMode, gTrue, width, height, mat, interpolate);
...
GBool SplashOutputDev::softMaskMatteImageSrc(void *data, SplashColorPtr colorLine, Guchar *alphaLine) {
...
void SplashOutputDev::drawSoftMaskedImage(GfxState *state,
                                          Object *ref,
                                          Stream *str,
                                          int width,
                                          int height,
    ...
    if(matte && width == maskWidth && height == maskHeight) {
        ...
        matteImgData.maskStr = new ImageStream(maskStr, maskWidth, maskColorMap->getNumPixelComps(),
        ...
        matteImgData.y = 0;
        imgTag = makeImageTag(ref);
        splash->drawImage(imgTag, &softMaskMatteImageSrc, &matteImgData, srcMode, gTrue, width,
        ...
        imgMaskData.imgStr = new ImageStream(maskStr, maskWidth, maskColorMap->getNumPixelComps(),
                                             maskColorMap->getBits());
        imgMaskData.imgStr->reset();
        ...
        maskSplash->setEnablePathSimplification(globalParams->getEnablePathSimplification());
        clearMaskRegion(state, maskSplash, 0, 0, 1, 1);
        maskSplash->drawImage(NULL, &imageSrc, &imgMaskData, splashModeMono8, gFalse, maskWidth,
                              maskHeight, mat, interpolate);
        delete imgMaskData.imgStr;
        maskStr->close();
        ...
        imgTag = makeImageTag(ref);
        splash->drawImage(imgTag, &imageSrc, &imgData, srcMode, gFalse, width, height, mat, interpolate);
...
void SplashOutputDev::beginTransparencyGroup(GfxState *state,
                                             double *bbox,
                                             GfxColorSpace *blendingColorSpace,
                                             GBool isolated,
                                             GBool knockout,
    ...
    if(forSoftMask && isolated && !knockout && blendingColorSpace) {
        if(blendingColorSpace->getMode() == csDeviceGray || blendingColorSpace->getMode() == csCalGray ||
           (blendingColorSpace->getMode() == csICCBased && blendingColorSpace->getNComps() == 1)) {
            colorMode = splashModeMono8;
        } else if(blendingColorSpace->getMode() == csDeviceRGB || blendingColorSpace->getMode() == csCalRGB ||
                  (blendingColorSpace->getMode() == csICCBased && blendingColorSpace->getNComps() == 3)) {
            //~ does this need to use BGR8?
            colorMode = splashModeRGB8;
#if SPLASH_CMYK
        } else if(blendingColorSpace->getMode() == csDeviceCMYK ||
                  (blendingColorSpace->getMode() == csICCBased && blendingColorSpace->getNComps() == 4)) {
    ...
    // create the temporary bitmap
    bitmap = new SplashBitmap(w, h, bitmapRowPad, colorMode, gTrue, bitmapTopDown, transpGroup->origBitmap);
    splash = new Splash(bitmap, vectorAntialias, transpGroup->origSplash->getImageCache(),
    ...
    if(!isolated && transpGroup->origBitmap->getAlphaPtr() &&
       transpGroup->origSplash->getInNonIsolatedGroup() && colorMode != splashModeMono1) {
        // when drawing a non-isolated group into another non-isolated group,
        // compute a backdrop bitmap with corrected alpha values
        backdropBitmap =
            new SplashBitmap(w, h, bitmapRowPad, colorMode, gTrue, bitmapTopDown, transpGroup->origBitmap);
        transpGroup->origSplash->blitCorrectedAlpha(backdropBitmap, tx, ty, 0, 0, w, h);
        transpGroup->backdropBitmap = backdropBitmap;
        splash->setInTransparencyGroup(backdropBitmap, 0, 0, !isolated, knockout);
    } else {
        backdropBitmap = transpGroup->origBitmap;
        transpGroup->backdropBitmap = NULL;
        splash->setInTransparencyGroup(backdropBitmap, tx, ty, !isolated, knockout);
    }
...
void SplashOutputDev::paintTransparencyGroup(GfxState *state, double *bbox) {
    ...
    if(xDest < bitmap->getWidth() && yDest < bitmap->getHeight() && w > 0 && h > 0) {
        splash->setOverprintMask(0xffffffff);
        splash->composite(tBitmap, xSrc, ySrc, xDest, yDest, w, h, gFalse, !isolated);
    }
...
void SplashOutputDev::setSoftMask(GfxState *state, double *bbox, GBool alpha, Function *transferFunc, GfxColor *backdropColor) {
    ...
    if(!alpha && tBitmap->getMode() != splashModeMono1) {
        ...
        if(transpGroupStack->blendingColorSpace) {
            tSplash = new Splash(tBitmap, vectorAntialias, transpGroupStack->origSplash->getImageCache(),
                                 transpGroupStack->origSplash->getScreen());
            tSplash->setStrokeAdjust(mapStrokeAdjustMode[globalParams->getStrokeAdjust()]);
            tSplash->setEnablePathSimplification(globalParams->getEnablePathSimplification());
    ...
    softMask = new SplashBitmap(bitmap->getWidth(), bitmap->getHeight(), 1, splashModeMono8, gFalse,
                                gTrue, bitmap);
            ...
            for(x = 0; x < tBitmap->getWidth(); ++x) {
                if(alpha) {
                    ...
                    switch(tBitmap->getMode()) {
                        ...
                        case splashModeRGB8:
                        case splashModeBGR8:
                            lum = (0.3 / 255.0) * color[0] + (0.59 / 255.0) * color[1] +
                                  (0.11 / 255.0) * color[2];
                            break;
#if SPLASH_CMYK
                        case splashModeCMYK8:
                            lum = (1 - color[3] / 255.0) - (0.3 / 255.0) * color[0] -
                                  (0.59 / 255.0) * color[1] - (0.11 / 255.0) * color[2];
                            if(lum < 0) {
                                lum = 0;
                            }
                            break;
...
#if LOAD_FONTS_FROM_MEM
            if(!(ff = FoFiTrueType::make(fontBuf->getCString(), fontBuf->getLength(), fontLoc->fontNum))) {
#else
            if(!(ff = FoFiTrueType::load(fontLoc->path->getCString(), fontLoc->fontNum))) {
#endif
                delete fontLoc;
                delete id;
                return NULL;

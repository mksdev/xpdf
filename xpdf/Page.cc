...
Page::Page(PDFDoc *docA, int numA, Dict *pageDict, PageAttrs *attrsA) {
    ...
    if(!(annots.isRef() || annots.isArray() || annots.isNull())) {
        error(errSyntaxError, -1, "Page annotations object (page {0:d}) is wrong type ({1:s})", num,
              annots.getTypeName());
        annots.free();
        goto err2;
    }
    ...
    if(!(contents.isRef() || contents.isArray() || contents.isNull())) {
        error(errSyntaxError, -1, "Page contents object (page {0:d}) is wrong type ({1:s})", num,
...
void Page::displaySlice(OutputDev *out,
    ...
    if(!out->checkPageSlice(this, hDPI, vDPI, rotate, useMediaBox, crop, sliceX, sliceY, sliceW,
    ...
    if(globalParams->getPrintCommands()) {
        ...
        printf("***** MediaBox = ll:%g,%g ur:%g,%g\n", mediaBox->x1, mediaBox->y1, mediaBox->x2,
               mediaBox->y2);
    ...
    gfx = new Gfx(doc, out, num, attrs->getResourceDict(), hDPI, vDPI, &box,
                  crop ? cropBox : (PDFRectangle *)NULL, rotate, abortCheckCbk, abortCheckCbkData);
...
#ifndef PDF_PARSER_ONLY
void Page::getDefaultCTM(double *ctm, double hDPI, double vDPI, int rotate, GBool useMediaBox, GBool upsideDown) {
    GfxState *state;
    int i;

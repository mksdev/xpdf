...
GBool WebFont::writeTTF(const char *fontFilePath) {
    ...
    ffTrueType->writeTTF(writeToFile, out, NULL, NULL, cmapTable, cmapTableLength);
    fclose(out);
    gfree(cmapTable);
    return gTrue;
}
...
GBool WebFont::writeOTF(const char *fontFilePath) {
    ...
    if(ffType1C) {
        ...
        ffType1C->convertToOpenType(writeToFile, out, nWidths, widths, cmapTable, cmapTableLength);
        fclose(out);
        gfree(cmapTable);
        gfree(widths);
...
Gushort *WebFont::makeType1CWidths(int *codeToGID, int nCodes, int *nWidths) {
    Gushort *widths;
    Gushort width;
    int widthsLen, gid, i;
    ...
    for(i = 0; i < nCodes; ++i) {
        ...
        width = (Gushort)(((Gfx8BitFont *)gfxFont)->getWidth((Guchar)i) * 1000 + 0.5);
        if(width == 0) {
            continue;
        }
        widths[gid] = width;
...
Gushort *WebFont::makeCIDType0CWidths(int *codeToGID, int nCodes, int *nWidths) {
    Gushort *widths;
    Gushort width;
    int widthsLen, gid, i;
    ...
    for(i = 0; i < nCodes; ++i) {
        ...
        width = (Gushort)(((GfxCIDFont *)gfxFont)->getWidth((CID)i) * 1000 + 0.5);
        if(width == 0) {
            continue;
        }
        widths[gid] = width;
...
Guchar *WebFont::makeUnicodeCmapTable(int *codeToGID, int nCodes, int *unicodeCmapLength) {
    int *unicodeToGID;
    Guchar *cmapTable;

...
static FontStyleTagInfo fontStyleTags[] = {{"Roman", 5, gFalse, gFalse},
...
static SubstFontInfo substFonts[16] = {{0.833}, {0.833}, {0.889}, {0.889}, {0.788}, {0.722},
                                       {0.833}, {0.778}, {0.600}, {0.600}, {0.600}, {0.600}};
...
static int privateUnicodeMap[privateUnicodeMapEnd - privateUnicodeMapStart + 1] = {
...
enum VerticalAlignment { vertAlignBaseline, vertAlignSub, vertAlignSuper, vertAlignTop };

static const char *vertAlignNames[] = {"baseline", "sub", "super", "top"};
...
public:
    HTMLGenFontDefn(Ref fontIDA, GString *fontFaceA, GString *fontSpecA, double scaleA)
        : fontID(fontIDA), fontFace(fontFaceA), fontSpec(fontSpecA), scale(scaleA), used(gFalse) {}
    ~HTMLGenFontDefn() {
        delete fontFace;
        delete fontSpec;
    }
...
int HTMLGen::convertPage(int pg,
                         const char *pngURL,
                         const char *htmlDir,
    ...
    doc->displayPage(splashOut, pg, backgroundResolution, backgroundResolution, 0, gFalse, gTrue, gFalse);
    ...
    png_set_IHDR(png, pngInfo, bitmap->getWidth(), bitmap->getHeight(), 8, PNG_COLOR_TYPE_RGB,
...
int HTMLGen::findDirSpan(GList *words, int firstWordIdx, int primaryDir, int *spanDir) {
    ...
    for(nextWordIdx = firstWordIdx + primaryDir;
        (primaryDir >= 0) ? nextWordIdx < words->getLength() : nextWordIdx >= 0; nextWordIdx += primaryDir) {
...
// append them to <s>.
void HTMLGen::appendSpans(GList *words,
                          int firstWordIdx,
                          int lastWordIdx,
                          int primaryDir,
                          int spanDir,
    ...
    for(wordIdx = firstWordIdx; (spanDir >= 0) ? wordIdx <= lastWordIdx : wordIdx >= lastWordIdx;
        wordIdx += spanDir) {
        ...
        if(!word0 || word1->getFontInfo() != word0->getFontInfo() ||
           word1->getFontSize() != word0->getFontSize() || word1->isInvisible() != word0->isInvisible() ||
           vertAlign1 != vertAlign0 || r1 != r0 || g1 != g0 || b1 != b0) {
            if(word0) {
                s->append("</span>");
            }
            for(i = 0; i < fonts->getLength(); ++i) {
            ...
            s->appendf("<span id=\"f{0:d}\"{1:s} "
                       "style=\"font-size:{2:d}px;vertical-align:{3:s};{4:s}color:rgba({5:d},{6:d},"
                       "{7:d},{8:d});\">",
                       i, dirTag, (int)(fontScales[i] * word1->getFontSize() * zoom),
                       vertAlignNames[vertAlign1], (dropCapLine && wordIdx == 0) ? "line-height:75%;" : "",
                       (int)(r1 * 255), (int)(g1 * 255), (int)(b1 * 255), invisible ? 0 : 1);
        }
        ...
        for(i = (spanDir >= 0) ? 0 : word1->getLength() - 1;
            (spanDir >= 0) ? i < word1->getLength() : i >= 0; i += spanDir) {
            u = word1->getChar(i);
            if(u >= privateUnicodeMapStart && u <= privateUnicodeMapEnd &&
...
HTMLGenFontDefn *HTMLGen::getFontFile(TextFontInfo *font, const char *htmlDir) {
    ...
    if(webFont->canWriteTTF()) {
        ...
        if(webFont->writeTTF(fontPath->getCString())) {
            fontFace =
                GString::format("@font-face {{ font-family: ff{0:d}; src: url(\"{1:t}\"); }}\n",
                                nextFontFaceIdx, fontFile);
            getFontDetails(font, &family, &weight, &style, &scale);
            fontSpec =
                GString::format("font-family:ff{0:d},{1:s}; font-weight:{2:s}; font-style:{3:s};",
    ...
    if(fontName2) {
        ...
        for(fst = fontStyleTags; fst->tag; ++fst) {
            if(n > fst->tagLen && !strcasecmp(fontName2 + n - fst->tagLen, fst->tag)) {
                ...
                italic = fst->italic;
                n -= fst->tagLen;
                if(n > 1 && (fontName2[n - 1] == '-' || fontName2[n - 1] == ',' ||
                             fontName2[n - 1] == '.' || fontName2[n - 1] == '_')) {
                    --n;
                }
                break;
            }

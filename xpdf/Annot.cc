...
Annot::Annot(PDFDoc *docA, Dict *dict, Ref *refA) {
    ...
    if(dict->lookup("C", &obj1)->isArray() &&
       (obj1.arrayGetLength() == 1 || obj1.arrayGetLength() == 3 || obj1.arrayGetLength() == 4)) {
    ...
    obj1.free();
    borderStyle = new AnnotBorderStyle(borderType, borderWidth, borderDash, borderDashLength,
...
void Annot::generateFreeTextAppearance() {
    ...
    if(borderStyle->getWidth() != 0) {
        ...
        appearBuf->appendf("{0:.4f} {1:.4f} {2:.4f} {3:.4f} re s\n", 0.5 * lineWidth,
                           0.5 * lineWidth, xMax - xMin - lineWidth, yMax - yMin - lineWidth);
    }
...
void Annot::drawLineArrow(AnnotLineEndType lineEnd, double x, double y, double dx, double dy, double w, GBool fill) {
    switch(lineEnd) {
        ...
        case annotLineEndSlash:
            w *= lineEndSize1;
            appearBuf->appendf("{0:.4f} {1:.4f} m\n",
                               x + 0.5 * w * cos(lineArrowAngle) * dy - 0.5 * w * sin(lineArrowAngle) * dx,
                               y - 0.5 * w * cos(lineArrowAngle) * dx - 0.5 * w * sin(lineArrowAngle) * dy);
            appearBuf->appendf("{0:.4f} {1:.4f} l\n",
                               x - 0.5 * w * cos(lineArrowAngle) * dy + 0.5 * w * sin(lineArrowAngle) * dx,
                               y + 0.5 * w * cos(lineArrowAngle) * dx + 0.5 * w * sin(lineArrowAngle) * dy);
            appearBuf->append("S\n");
...
Annots::Annots(PDFDoc *docA, Object *annotsObj) {
    ...
    if(annotsObj->isArray()) {
        ...
        // in any form field.
        drawWidgetAnnots = !doc->getCatalog()->getForm() || doc->getCatalog()->getForm()->getNumFields() == 0;
        for(i = 0; i < annotsObj->arrayGetLength(); ++i) {
            ...
            if(obj1.isDict()) {
                if(drawWidgetAnnots || !obj1.dictLookup("Subtype", &obj2)->isName("Widget")) {

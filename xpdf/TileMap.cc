    ...
    switch(state->getDisplayMode()) {
        ...
        case displaySingle:
            ...
            for(y = y0 / tileH1; y <= y1 / tileH1; ++y) {
                for(x = x0 / tileW1; x <= x1 / tileW1; ++x) {
                    ...
                    tiles->append(new PlacedTileDesc(page, state->getRotate(), pageDPI1, tx, ty, tw,
                                                     th, tx - state->getScrollX() + offsetX,
            ...
            while(page <= state->getDoc()->getNumPages() &&
                ...
                for(y = y0 / tileH1; y <= y1 / tileH1; ++y) {
                    for(x = x0 / tileW1; x <= x1 / tileW1; ++x) {
                        ...
                        tiles->append(new PlacedTileDesc(page, state->getRotate(), pageDPI1, tx, ty,
                                                         tw, th, tx - state->getScrollX() + offsetX,
        ...
        case displaySideBySideSingle:
            ...
            if(pageW1 + sideBySidePageSpacing + pageW2 < state->getWinW()) {
                offsetX = (state->getWinW() - (pageW1 + sideBySidePageSpacing + pageW2)) / 2;
            } else {
                offsetX = 0;
            }
            ...
            if(page + 1 <= state->getDoc()->getNumPages()) {
                ...
                if((x1 = state->getScrollX() + state->getWinW() - 1 - offsetX2) >= pageW2) {
                    x1 = pageW2 - 1;
                } else if(x1 < 0) {
                    x1 = -tileW2;
                }
                if((y1 = state->getScrollY() + state->getWinH() - 1 - offsetY) >= pageH2) {
                    y1 = pageH2 - 1;
                } else if(y1 < 0) {
                    y1 = -tileH2;
                }
                for(y = y0 / tileH2; y <= y1 / tileH2; ++y) {
                    for(x = x0 / tileW2; x <= x1 / tileW2; ++x) {
                        ...
                        tiles->append(new PlacedTileDesc(page + 1, state->getRotate(), pageDPI2, tx, ty,
                                                         tw, th, tx - state->getScrollX() + offsetX2,
                                                         ty - state->getScrollY() + offsetY));
                    }
                }
            }
        ...
        case displaySideBySideContinuous:
            ...
            while(page <= state->getDoc()->getNumPages() &&
                ...
                if(maxW + sideBySidePageSpacing + maxW2 < state->getWinW()) {
                    offsetX = (state->getWinW() - (maxW + sideBySidePageSpacing + maxW2)) / 2;
                } else {
                    offsetX = 0;
                }
                offsetX += maxW - pageW1;
                ...
                if(page <= state->getDoc()->getNumPages()) {
                    ...
                    for(y = y0 / tileH2; y <= y1 / tileH2; ++y) {
                        for(x = x0 / tileW2; x <= x1 / tileW2; ++x) {
                            ...
                            tiles->append(new PlacedTileDesc(page, state->getRotate(), pageDPI2, tx, ty,
                                                             tw, th, tx - state->getScrollX() + offsetX2,
                                                             ty - state->getScrollY() + pageY[page - 1] + offsetY));
                        }
                    }
            ...
            while(page <= state->getDoc()->getNumPages() &&
                ...
                for(y = y0 / tileH1; y <= y1 / tileH1; ++y) {
                    for(x = x0 / tileW1; x <= x1 / tileW1; ++x) {
                        ...
                        tiles->append(new PlacedTileDesc(page, state->getRotate(), pageDPI1, tx, ty, tw, th,
...
GBool TileMap::cvtDevToWindow(int pg, int xd, int yd, int *xw, int *yw) {
    ...
    switch(state->getDisplayMode()) {
        ...
        case displaySideBySideSingle:
            if(!(pg == state->getScrollPage() || (pg == state->getScrollPage() + 1 &&
...
void TileMap::getWindowPageRange(int x, int y, int w, int h, int *firstPage, int *lastPage) {
    GList *tiles;
    PlacedTileDesc *tile;
    int i;
...
int TileMap::getFirstPageTop() {
    ...
    switch(state->getDisplayMode()) {
        ...
        case displayContinuous:
            page = findContinuousPage(state->getScrollY());
            if(page < state->getDoc()->getNumPages() && pageY[page - 1] < state->getScrollY()) {
                return page + 1;
            } else {
                return page;
            }
        ...
        case displayHorizontalContinuous:
            page = findHorizContinuousPage(state->getScrollX());
            if(page < state->getDoc()->getNumPages() && pageX[page - 1] < state->getScrollX()) {
                return page + 1;
            } else {
                return page;
            }
...
int TileMap::getLastPage() {
    ...
    switch(state->getDisplayMode()) {
        ...
        default:
            return state->getScrollPage();
        case displayContinuous:
            if(!cvtWindowToDev(state->getWinW() / 2, state->getWinH() - 1, &pg, &x, &y)) {
        ...
        case displayHorizontalContinuous:
            x = state->getWinW() - 1;
            y = state->getWinH() / 2;
            if(!cvtWindowToDev(state->getWinW() - 1, state->getWinH() / 2, &pg, &x, &y)) {
                return state->getDoc()->getNumPages();
            }
            return pg;
    }
...
void TileMap::updatePageParams() {
    ...
    for(page = 1; page <= state->getDoc()->getNumPages(); ++page) {
        ...
        if(state->displayModeIsSideBySide()) {
            ...
            rotPageBoxHMax = (rotPageBoxH > rotPageBoxH2) ? rotPageBoxH : rotPageBoxH2;

            // compute resolution
            if(state->getZoom() == zoomPage) {
                hDPI = ((state->getWinW() - sideBySidePageSpacing) / (rotPageBoxW + rotPageBoxW2)) * 72.0;
                ...
                dpi = ((state->getWinW() - sideBySidePageSpacing) / (rotPageBoxW + rotPageBoxW2)) * 72.0;
                // allow for some floating point jitter
                dpi -= 0.01;
...
void TileMap::updateContinuousModeParams() {
    ...
    switch(state->getDisplayMode()) {
        case displayContinuous:
            if(!pageX) {
                pageX = pageY = (int *)gmallocn(state->getDoc()->getNumPages(), sizeof(int));
            }
            y = 0;
            maxW = 0;
            for(page = 1; page <= state->getDoc()->getNumPages(); ++page) {

...
#define gMemDataSize(size) (int)(((((size) + gMemTrlSize - 1) / gMemTrlSize) * gMemTrlSize))

#define gMemDataSize64(size) (size_t)(((((size) + gMemTrlSize - 1) / gMemTrlSize) * gMemTrlSize))
...
static BOOL CALLBACK gMemMutexInitFunc(PINIT_ONCE initOnce, PVOID param, PVOID *context) {
    InitializeCriticalSection(&gMemMutex);
    return TRUE;
}
#define gMemInitMutex InitOnceExecuteOnce(&gMemMutexInitStruct, &gMemMutexInitFunc, NULL, NULL)
...
void gfree(void *p) {
    ...
    if(p) {
        hdr = (GMemHdr *)((char *)p - gMemHdrSize);
        gMemLock;
        if(hdr->magic == gMemMagic && ((hdr->prev == NULL) == (hdr == gMemHead)) &&

...
static const char *formatStrings[] = {"d",    "x",    "o",    "b",    "ud",  "ux",  "uo",  "ub",
    ...
    while(*p0) {
        if(*p0 == '{') {
            ...
            if(*p0 == '{') {
                ...
                for(ft = (GStringFormatType)0; formatStrings[ft]; ft = (GStringFormatType)(ft + 1)) {
                ...
                if(idx == argsLen) {
                    if(argsLen == argsSize) {
                        argsSize *= 2;
                        args = (GStringFormatArg *)greallocn(args, argsSize, sizeof(GStringFormatArg));
                    }
                    switch(ft) {
                        case fmtIntDecimal:
                        case fmtIntHex:
...
#ifdef LLONG_MAX
void GString::formatInt(long long x, char *buf, int bufSize, GBool zeroFill, int width, int base, const char **p, int *len) {
#else
void GString::formatInt(long x, char *buf, int bufSize, GBool zeroFill, int width, int base, const char **p, int *len) {
#endif
...
#ifdef ULLONG_MAX
void GString::formatUInt(unsigned long long x, char *buf, int bufSize, GBool zeroFill, int width, int base, const char **p, int *len) {
#else
void GString::formatUInt(Gulong x, char *buf, int bufSize, GBool zeroFill, int width, int base, const char **p, int *len) {
#endif
    static char vals[17] = "0123456789abcdef";
    int i, j;
...
void GString::formatDouble(double x, char *buf, int bufSize, int prec, GBool trim, const char **p, int *len) {
    GBool neg, started;
    double x2;
    int d, i, j;
...
int GString::cmpN(GString *str, int n) {
    ...
    n1 = length;
    n2 = str->length;
    for(i = 0, p1 = s, p2 = str->s; i < n1 && i < n2 && i < n; ++i, ++p1, ++p2) {
        x = (*p1 & 0xff) - (*p2 & 0xff);
        if(x != 0) {
            return x;
        }

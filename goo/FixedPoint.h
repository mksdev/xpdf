...
public:
    ...
    FixedPoint(const FixedPoint &x) {
        val = x.val;
    }
    FixedPoint(double x) {
        val = (int)(x * (1 << fixptShift) + 0.5);
    }
    FixedPoint(int x) {
        val = x << fixptShift;
    }
    FixedPoint(long x) {
        val = (int)x << fixptShift;
    }
    ...
    bool operator==(FixedPoint x) const {
        return val == x.val;
    }
    ...
    FixedPoint operator+(FixedPoint x) {
        return make(val + x.val);
    }
    FixedPoint operator+(double x) {
        return *this + (FixedPoint)x;
    }
    FixedPoint operator+(int x) {
        return *this + (FixedPoint)x;
    }
    FixedPoint operator+(long x) {
        return *this + (FixedPoint)x;
    }
    ...
    static FixedPoint abs(FixedPoint x) {
        return make(::abs(x.val));
    }

    static int floor(FixedPoint x) {
        return x.val >> fixptShift;
    }

    static int ceil(FixedPoint x) {
        return (x.val & fixptMaskL) ? ((x.val >> fixptShift) + 1) : (x.val >> fixptShift);
    }
